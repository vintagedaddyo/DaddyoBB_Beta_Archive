...
require_once DADDYOBB_ROOT."inc/functions_calendar.php";
...
if($daddyobb->settings['enablecalendar'] == 0)
{
	error($lang->calendar_disabled);
}

if($daddyobb->usergroup['canviewcalendar'] == 0)
{
	error_no_permission();
}
...
$calendar_jump = build_calendar_jump($daddyobb->input['calendar']);

// Add an event
if($daddyobb->input['action'] == "do_addevent" && $daddyobb->request_method == "post")
{
	$query = $db->simple_select("calendars", "*", "cid='".intval($daddyobb->input['calendar'])."'");
	...
	require_once DADDYOBB_ROOT."inc/datahandlers/event.php";
	...
	$event = array(
		...
		"description" => $daddyobb->input['description'],
		"private" => $daddyobb->input['private'],
		"type" => $daddyobb->input['type']
	);

	// Now we add in our date/time info depending on the type of event
	if($daddyobb->input['type'] == "single")
	{
		$event['start_date'] = array(
			"day" => $daddyobb->input['single_day'],
			"month" => $daddyobb->input['single_month'],
			"year" => $daddyobb->input['single_year']
		);
	}
	else if($daddyobb->input['type'] == "ranged")
	{
		$event['start_date'] = array(
			"day" => $daddyobb->input['start_day'],
			"month" => $daddyobb->input['start_month'],
			"year" => $daddyobb->input['start_year'],
			"time" => $daddyobb->input['start_time']
		);
		$event['end_date'] = array(
			"day" => $daddyobb->input['end_day'],
			"month" => $daddyobb->input['end_month'],
			"year" => $daddyobb->input['end_year'],
			"time" => $daddyobb->input['end_time']
		);
		$event['timezone'] = $daddyobb->input['timezone'];
		$event['ignoretimezone'] =	intval($daddyobb->input['ignoretimezone']);
		$repeats = array();
		switch($daddyobb->input['repeats'])
		{
			case 1:
				$repeats['repeats'] = 1;
				$repeats['days'] = $daddyobb->input['repeats_1_days'];
				break;
			case 2:
				$repeats['repeats'] = 2;
				break;
			case 3:
				$repeats['repeats'] = 3;
				$repeats['weeks'] = $daddyobb->input['repeats_3_weeks'];
				if(!is_array($daddyobb->input['repeats_3_days']))
				{
					$daddyobb->input['repeats_3_days'] = array();
				}
				ksort($daddyobb->input['repeats_3_days']);
				$days = array();
				foreach($daddyobb->input['repeats_3_days'] as $weekday => $value)
				{
					if($value != 1) continue;
					$days[] = $weekday;
				}
			...
			case 4:
				$repeats['repeats'] = 4;
				if($daddyobb->input['repeats_4_type'] == 1)
				{
					$repeats['day'] = $daddyobb->input['repeats_4_day'];
					$repeats['months'] = $daddyobb->input['repeats_4_months'];
				}
				else
				{
					$repeats['months'] = $daddyobb->input['repeats_4_months2'];
					$repeats['occurance'] = $daddyobb->input['repeats_4_occurance'];
					$repeats['weekday'] = $daddyobb->input['repeats_4_weekday'];
				}
				break;
			case 5:
				$repeats['repeats'] = 5;
				if($daddyobb->input['repeats_5_type'] == 1)
				{
					$repeats['day'] = $daddyobb->input['repeats_5_day'];
					$repeats['month'] = $daddyobb->input['repeats_5_month'];
					$repeats['years'] = $daddyobb->input['repeats_5_years'];
				}
				else
				{
					$repeats['occurance'] = $daddyobb->input['repeats_5_occurance'];
					$repeats['weekday'] = $daddyobb->input['repeats_5_weekday'];
					$repeats['month'] = $daddyobb->input['repeats_5_month2'];
					$repeats['years'] = $daddyobb->input['repeats_5_years'];
				}
				break;
			default:
	...
	if(!$eventhandler->validate_event())
	{
		...
		$daddyobb->input['action'] = "addevent";
	}
	else
	{
...
if($daddyobb->input['action'] == "addevent")
{
	$query = $db->simple_select("calendars", "*", "cid='".intval($daddyobb->input['calendar'])."'");
	$calendar = $db->fetch_array($query);
	...
	if($daddyobb->settings['bbcodeinserter'] != 0 && (!$daddyobb->user['uid'] || $daddyobb->user['showcodebuttons'] != 0) && $calendar['allowmycode'] == 1)
	{
	...
	// Previous selections
	$name = $description = '';
	if(isset($daddyobb->input['name']))
	{
		$name = htmlspecialchars_uni($daddyobb->input['name']);
	}
	
	if(isset($daddyobb->input['description']))
	{
		$description = htmlspecialchars_uni($daddyobb->input['description']);
	}
	
	if($daddyobb->request_method == "post")
	{
		$single_day = $daddyobb->input['single_day'];
		$single_month[$daddyobb->input['single_month']] = " selected=\"selected\"";
		$single_year = $daddyobb->input['single_year'];
		$start_day = $daddyobb->input['start_day'];
		$start_month[$daddyobb->input['start_month']] = " selected=\"selected\"";
		$start_year = $daddyobb->input['start_year'];
		$start_time = htmlspecialchars_uni($daddyobb->input['start_time']);
		$end_day = $daddyobb->input['end_day'];
		$end_month[$daddyobb->input['end_month']] = " selected=\"selected\"";
		$end_year = $daddyobb->input['end_year'];
		$end_time = htmlspecialchars_uni($daddyobb->input['end_time']);
		if($daddyobb->input['type'] == "single")
		{
		...
		if($daddyobb->input['repeats'])
		{
			$repeats_sel[$daddyobb->input['repeats']] = " selected=\"selected\"";
		}
		$repeats_1_days = intval($daddyobb->input['repeats_1_days']);
		$repeats_3_weeks = intval($daddyobb->input['repeats_3_weeks']);
		if(is_array($daddyobb->input['repeats_3_days']))
		{
			foreach($daddyobb->input['repeats_3_days'] as $day => $val)
			{
				if($val != 1) continue;
				$day = intval($day);
		...
		$repeats_4_day = intval($daddyobb->input['repeats_4_day']);
		$repeats_4_months = intval($daddyobb->input['repeats_4_months']);
		$repeats_4_occurance[$daddyobb->input['repeats_4_occurance']] = "selected=\"selected\"";
		$repeats_4_weekday[$daddyobb->input['repeats_4_weekday']] = "selected=\"selected\"";
		$repeats_4_months2 = intval($daddyobb->input['repeats_4_months2']);
		if($daddyobb->input['repeats_5_type'] == 1)
		{
		...
		$repeats_5_day = intval($daddyobb->input['repeats_5_day']);
		$repeats_5_month[$daddyobb->input['repeats_5_month']] = "selected=\"selected\"";
		$repeats_5_years = intval($daddyobb->input['repeats_5_years']);
		$repeats_5_occurance[$daddyobb->input['repeats_5_occurance']] = "selected=\"selected\"";
		$repeats_5_weekday[$daddyobb->input['repeats_5_weekday']] = "selected=\"selected\"";
		$repeats_5_month2[$daddyobb->input['repeats_5_month2']] = "selected=\"selected\"";
		$repeats_5_years2 = intval($daddyobb->input['repeats_5_years2']);

		$timezone = $daddyobb->input['timezone'];
	}
	else
	{
		if($daddyobb->input['day'])
		{
			$single_day = $start_day = $end_day = intval($daddyobb->input['day']);
		}
		else
		{
			$single_day = $start_day = $end_day = my_date("j");
		}
		if($daddyobb->input['month'])
		{
			$month = intval($daddyobb->input['month']);
		}
		else
		{
			$month = my_date("n");
		...
		if($daddyobb->input['year'])
		{
			$single_year = $start_year = $end_year = intval($daddyobb->input['year']);
		}
		else
		{
		...
		$repeats_5_years2 = 1;
		$timezone = $daddyobb->user['timezone'];
	}
	...
	if($daddyobb->input['ignoretimezone'] == 1)
	{
		$ignore_timezone = "checked=\"checked\"";
	}

	if($daddyobb->input['private'] == 1)
	{
	...
	while($calendar_option = $db->fetch_array($query))
	{
		if($calendar_permissions[$calendar['cid']]['canviewcalendar'] == 1)
		{
			...
			if($calendar_option['cid'] == $daddyobb->input['calendar'])
			{
...
// Edit an event
if($daddyobb->input['action'] == "do_editevent" && $daddyobb->request_method == "post")
{
	$query = $db->simple_select("events", "*", "eid='".intval($daddyobb->input['eid'])."'");
	...
	if(($event['uid'] != $daddyobb->user['uid'] || $daddyobb->user['uid'] == 0) && $calendar_permissions['canmoderateevents'] != 1)
	{
		error_no_permission();
	}
	...
	if(!$eventhandler->validate_event())
	{
		$event_errors = $eventhandler->get_friendly_errors();
		$event_errors = inline_error($event_errors);
		$daddyobb->input['action'] = "editevent";
	}
	else
	{
...
if($daddyobb->input['action'] == "editevent")
{
	$query = $db->simple_select("events", "*", "eid='".intval($daddyobb->input['eid'])."'");
	$event = $db->fetch_array($query);
	...
	{
		$daddyobb->input['calendar'] = $event['cid'];
...
if($daddyobb->input['action'] == "do_move" && $daddyobb->request_method == "post")
{
	// Verify incoming POST request
	verify_post_check($daddyobb->input['my_post_key']);
	...
	$query = $db->simple_select("calendars", "*", "cid='".intval($daddyobb->input['new_calendar'])."'");
	...
	if($calendar_permissions[$daddyobb->input['new_calendar']]['canviewcalendar'] != 1)
	{
		error_no_permission();
	}
...
// Approve an event
if($daddyobb->input['action'] == "approve")
{
	// Verify incoming POST request
	verify_post_check($daddyobb->input['my_post_key']);
...
// Unapprove an event
if($daddyobb->input['action'] == "unapprove")
{
	// Verify incoming POST request
	verify_post_check($daddyobb->input['my_post_key']);
...
if($daddyobb->input['action'] == "event")
{
	$query = $db->query("
		SELECT u.*, e.*
		FROM ".TABLE_PREFIX."events e
		LEFT JOIN ".TABLE_PREFIX."users u ON (u.uid=e.uid)
		WHERE e.eid='".intval($daddyobb->input['eid'])."'
	");
	$event = $db->fetch_array($query);

	if(!is_numeric($event['eid']) || ($event['private'] == 1 && $event['uid'] != $daddyobb->user['uid']))
	{
		error($lang->error_invalidevent);
	}
	...
	if($event['endtime'] > 0 && $event['endtime'] != $event['starttime'])
	{
		...
		{
			$time_period = gmdate($daddyobb->settings['dateformat'], $event['starttime_user']);
			// Event runs all day
			if($start_time != 0000 && $end_time != 2359)
			{
				$time_period .= ", ".gmdate($daddyobb->settings['timeformat'], $event['starttime_user'])." - ".gmdate($daddyobb->settings['timeformat'], $event['endtime_user']);
			}
			else
			{
		...
		{
			$time_period = gmdate($daddyobb->settings['dateformat'], $event['starttime_user']).", ".gmdate($daddyobb->settings['timeformat'], $event['starttime_user']);
			$time_period .= " - ";
			$time_period .= gmdate($daddyobb->settings['dateformat'], $event['endtime_user']).", ".gmdate($daddyobb->settings['timeformat'], $event['endtime_user']);
		}
	}
	else
	{
	...
	if($calendar_permissions['canmoderateevents'] == 1 || ($daddyobb->user['uid'] > 0 && $daddyobb->user['uid'] == $event['uid']))
	{
	...
	if($daddyobb->usergroup['canaddevents'] == 1)
	{
...
if($daddyobb->input['action'] == "dayview")
{
	// Showing a particular calendar
	if($daddyobb->input['calendar'])
	{
		$query = $db->simple_select("calendars", "*", "cid='".intval($daddyobb->input['calendar'])."'");
		$calendar = $db->fetch_array($query);
	}
	...
	// Incoming year?
	if($daddyobb->input['year'] && $daddyobb->input['year'] <= my_date("Y")+5)
	{
		$year = intval($daddyobb->input['year']);
	}
	else
	{
		$year = my_date("Y");
	...
	// Then the month
	if($daddyobb->input['month'] >=1 && $daddyobb->input['month'] <= 12)
	{
		$month = intval($daddyobb->input['month']);
	}
	else
	...
	// And day?
	if($daddyobb->input['day'] && $daddyobb->input['day'] <= gmdate("t", gmmktime(0, 0, 0, $month, 1, $year)))
	{
		$day = intval($daddyobb->input['day']);
	}
	else
	{
		$day = my_date("j");
	...
	if($calendar['showbirthdays'])
	{
		...
		if($birthday_list)
		{
			$bdaydate = my_date($daddyobb->settings['dateformat'], gmmktime(0, 0, 0, $month, $day, $year), 0, 0);
...
if($daddyobb->input['action'] == "weekview")
{
	// Showing a particular calendar
	if($daddyobb->input['calendar'])
	{
	...
	if(!$daddyobb->input['week'])
	{
		...
		$daddyobb->input['week'] = gmmktime(0, 0, 0, $month, $start_day, $year);
	}
	
	// This is where we've come from and where we're headed
	$week_from = explode("-", gmdate("j-n-Y", $daddyobb->input['week']));
	$friendly_week_from = gmdate($daddyobb->settings['dateformat'], $daddyobb->input['week']);
	$week_to_stamp = gmmktime(0, 0, 0, $week_from[1], $week_from[0]+6, $week_from[2]);
	$week_to = explode("-", gmdate("j-n-Y-t", $week_to_stamp));
	$friendly_week_to = gmdate($daddyobb->settings['dateformat'], $week_to_stamp);
			...
			foreach($events_cache["$weekday_day-$weekday_month-$weekday_year"] as $event)
			{
				...
				if($event['endtime'] > 0 && $event['endtime'] != $event['starttime'])
				{
					...
					else if($start_day == $weekday_date)
					{
						...
						{
							$time_period = $lang->starts.gmdate($daddyobb->settings['timeformat'], $event['starttime_user']);
						}
					}
					...
					else if($end_day == $weekday_date)
					{
						...
						{
							$time_period = $lang->finishes.gmdate($daddyobb->settings['timeformat'], $event['endtime_user']);
						}
					}
...
if($daddyobb->input['action'] == "yearview")
{
}

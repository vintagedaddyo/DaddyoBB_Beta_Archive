...
switch($action)
{
	...
	case "announcement":
		...
		archive_header($announcement['subject'], $announcement['subject'], $daddyobb->settings['bburl']."/announcements.php?aid={$id}");

		// Format announcement contents.
		$announcement['startdate'] = my_date($daddyobb->settings['dateformat'].", ".$daddyobb->settings['timeformat'], $announcement['startdate']);
	...
	case "thread":
		...
		archive_header($thread['subject'], $thread['subject'], $daddyobb->settings['bburl']."/".get_thread_link($thread['tid']));
		...
		while($post = $db->fetch_array($query))
		{
			$post['date'] = my_date($daddyobb->settings['dateformat'].", ".$daddyobb->settings['timeformat'], $post['dateline'], "", 0);
			...
			if(is_array($acache[$post['pid']]))
			{
				...
				{
					$post['message'] = str_replace("[attachment={$attachment['aid']}]", "[<a href=\"".$daddyobb->settings['bburl']."/attachment.php?aid={$attachment['aid']}\">attachment={$attachment['aid']}</a>]", $post['message']);
				}
			}
	...
	case "forum":
		...
		{
			archive_header(strip_tags($forum['name']), $forum['name'], $daddyobb->settings['bburl']."/".get_forum_link($id)."");
			archive_error($lang->error_nothreads);
		}

		// Build the archive header.
		archive_header(strip_tags($forum['name']), $forum['name'], $daddyobb->settings['bburl']."/".get_forum_link($id.""), 1);
	...
	case "index":
		...
		archive_header("", $daddyobb->settings['bbname_orig'], $daddyobb->settings['bburl']."/index.php");

		$plugins->run_hooks("archive_index_start");

		echo "<div class=\"listing forumlist\">\n<div class=\"header\">{$daddyobb->settings['bbname']}</div>\n<div class=\"forums\">\n<ul>\n";
		echo $forums;
		echo "\n</ul>\n</div>\n</div>";
...
{
	global $db, $forumpermissions, $daddyobb, $lang, $archiveurl, $base_url;
	...
	if(is_array($fcache[$pid]))
	{
		foreach($fcache[$pid] as $key => $main)
		{
			foreach($main as $key => $forum)
			{
				...
				if(($perms['canview'] == 1 || $daddyobb->settings['hideprivateforums'] == 0) && $forum['active'] != 0)
				{
					if($forum['linkto'])
					{

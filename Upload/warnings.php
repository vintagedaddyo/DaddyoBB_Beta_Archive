...
if($daddyobb->settings['enablewarningsystem'] == 0)
{
...
if($daddyobb->input['action'] == "do_warn" && $daddyobb->request_method == "post")
{
	...
	if($daddyobb->usergroup['maxwarningsday'] != 0)
	{
		$timecut = TIME_NOW-60*60*24;
		$query = $db->simple_select("warnings", "COUNT(wid) AS given_today", "issuedby='{$daddyobb->user['uid']}' AND dateline>'$timecut'");
		$given_today = $db->fetch_field($query, "given_today");
		if($given_today >= $daddyobb->usergroup['maxwarningsday'])
		{
			error($lang->sprintf($lang->reached_max_warnings_day, $daddyobb->usergroup['maxwarningsday']));
		}
	}

	$user = get_user(intval($daddyobb->input['uid']));
	if(!$user['uid'])
	{
	...
	if($user['warningpoints'] >= $daddyobb->settings['maxwarningpoints'])
	{
	...
	{
		$post = get_post(intval($daddyobb->input['pid']));
	...
	if(!trim($daddyobb->input['notes']))
	{
		$warn_errors[] = $lang->error_no_note;
	}

	// Using a predefined warning type
	if($daddyobb->input['type'] != "custom")
	{
		$query = $db->simple_select("warningtypes", "*", "tid='".intval($daddyobb->input['type'])."'");
	...
	{
		if($daddyobb->settings['allowcustomwarnings'] == 0)
		{
			$warn_errors[] = $lang->error_cant_custom_warn;
		}
		else
		{
			if(!$daddyobb->input['custom_reason'])
			{
				$warn_errors[] = $lang->error_no_custom_reason;
			}
			else
			{
				$warning_title = $daddyobb->input['custom_reason'];
			}
			if(!is_numeric($daddyobb->input['custom_points']) || $daddyobb->input['custom_points'] > $daddyobb->settings['maxwarningpoints'] || $daddyobb->input['custom_points'] < 0)
			{
				$warn_errors[] = $lang->sprintf($lang->error_invalid_custom_points, $daddyobb->settings['maxwarningpoints']);
			}
			else
			{
				$points = round((int)$daddyobb->input['custom_points']);
			}
			// Build expiry date
			if($daddyobb->input['expires'])
			{
				$warning_expires = intval($daddyobb->input['expires']);
				if($daddyobb->input['expires_period'] == "hours")
				{
					$warning_expires = $warning_expires*3600;
				}
				else if($daddyobb->input['expires_period'] == "days")
				{
					$warning_expires = $warning_expires*86400;
				}
				else if($daddyobb->input['expires_period'] == "weeks")
				{
					$warning_expires = $warning_expires*604800;
				}
				else if($daddyobb->input['expires_period'] == "months")
				{
					$warning_expires = $warning_expires*2592000;
				}
				// Add on current time and we're there!
				if($daddyobb->input['expires_period'] != "never" && $warning_expires)
				{
	...
	if(!$warn_errors && $daddyobb->input['send_pm'] == 1 && $group_permissions['canusepms']  != 0 && $user['receivepms'] != 0 && $daddyobb->settings['enablepms'] != 0)
	{
		...
		$pm = array(
			"subject" => $daddyobb->input['pm_subject'],
			"message" => $daddyobb->input['pm_message'],
			"fromid" => $daddyobb->user['uid'],
			"toid" => array($user['uid'])
		);

		$pm['options'] = array(
			"signature" => $daddyobb->input['pm_options']['signature'],
			"disablesmilies" => $daddyobb->input['pm_options']['disablesmilies'],
			"savecopy" => $daddyobb->input['pm_options']['savecopy'],
			"readreceipt" => $daddyobb->input['pm_options']['readreceipt']
		);
	...
	if(!is_array($warn_errors))
	{
		...
		$current_level = round($user['warningpoints']/$daddyobb->settings['maxwarningpoints']*100);
		$new_warning_level = round(($user['warningpoints']+$points)/$daddyobb->settings['maxwarningpoints']*100);
	...
	if($warn_errors)
	{
		...
		$daddyobb->input['action'] = "warn";
	}
}

// Warn a user
if($daddyobb->input['action'] == "warn")
{
	if($daddyobb->usergroup['canwarnusers'] != 1)
	{
		error_no_permission();
	...
	if($daddyobb->input['pid'])
	{
		...
		$query = $db->query("
			...
			WHERE w.pid='{$daddyobb->input['pid']}'
		...
		while($warning = $db->fetch_array($query))
		{
			...
			$date_issued = my_date($daddyobb->settings['dateformat'], $warning['dateline']).", ".my_date($daddyobb->settings['timeformat'], $warning['dateline']);
			...
			if($warning['expired'] != 1)
			{
				...
				{
					$expires = my_date($daddyobb->settings['dateformat'], $warning['expires']).", ".my_date($daddyobb->settings['timeformat'], $warning['expires']);
				}
			}
			else
			{
	...
	if($warn_errors)
	{
		$notes = htmlspecialchars_uni($daddyobb->input['notes']);
		$type_checked[$daddyobb->input['type']] = "checked=\"checked\"";
		$pm_subject = htmlspecialchars_uni($daddyobb->input['pm_subject']);
		$message = htmlspecialchars_uni($daddyobb->input['pm_message']);
		if($daddyobb->input['send_pm'])
		{
			$send_pm_checked = "checked=\"checked\"";
		}
		$custom_reason = htmlspecialchars_uni($daddyobb->input['custom_reason']);
		$custom_points = intval($daddyobb->input['custom_points']);
		$expires = intval($daddyobb->input['expires']);
		$expires_period[$daddyobb->input['expires_period']] = "selected=\"selected\"";
	}
	else
	{
		...
		$message = $lang->sprintf($lang->warning_pm_message, $user['username'], $daddyobb->settings['bbname']);
	}
	...
	while($type = $db->fetch_array($query))
	{
		...
		$new_warning_level = round(($user['warningpoints']+$type['points'])/$daddyobb->settings['maxwarningpoints']*100);
		if($new_warning_level > 100)
		{
			$new_warning_level = 100;
		}
	...
	if($group_permissions['canusepms']  != 0 && $daddyobb->user['receivepms'] != 0 && $daddyobb->settings['enablepms'] != 0)
	{
...
// Revoke a warning
if($daddyobb->input['action'] == "do_revoke" && $daddyobb->request_method == "post")
{
	// Verify incoming POST request
	verify_post_check($daddyobb->input['my_post_key']);
	...
	$query = $db->simple_select("warnings", "*", "wid='".intval($daddyobb->input['wid'])."'");
	...
	else
	{
		...
		if($warning['expired'] != 1)
		{
			...
			$current_level = round($user['warningpoints']/$daddyobb->settings['maxwarningpoints']*100);
			$new_warning_level = round($new_warning_points/$daddyobb->settings['maxwarningpoints']*100);
		...
		$updated_warning = array(
			...
			"revokedby" => $daddyobb->user['uid'],
			"revokereason" => $db->escape_string($daddyobb->input['reason'])
		);
...
if($daddyobb->input['action'] == "view")
{
	...
	$query = $db->query("
		...
		WHERE w.wid='".intval($daddyobb->input['wid'])."'
	");
	$warning = $db->fetch_array($query);
	...
	else
	{
		...
		{
			$revoked_date = '('.my_date($daddyobb->settings['dateformat'], $warning['expires']).' '.my_date($daddyobb->settings['timeformat'], $warning['expires']).')';
	...
	{
		$date_revoked = my_date($daddyobb->settings['dateformat'], $warning['daterevoked']).", ".my_date($daddyobb->settings['timeformat'], $warning['daterevoked']);
...
if(!$daddyobb->input['action'])
{
	...
	if($user['warningpoints'] > $daddyobb->settings['maxwarningpoints'])
	{
		$user['warningpoints'] = $daddyobb->settings['maxwarningpoints'];
	}
	
	if($warning_level > 0)
	{
		$lang->current_warning_level = $lang->sprintf($lang->current_warning_level, $warning_level, $user['warningpoints'], $daddyobb->settings['maxwarningpoints']);
	}
	else
	{

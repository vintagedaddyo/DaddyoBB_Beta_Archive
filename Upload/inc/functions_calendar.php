...
{
	global $events_cache, $daddyobb, $templates, $theme, $monthnames;
...
function get_calendar_permissions($cid=0)
{
	...
	$group_permissions = array(
		"canviewcalendar" => $daddyobb->usergroup['canviewcalendar'],
		"canaddevents" => $daddyobb->usergroup['canaddevents'],
		"canbypasseventmod" => $daddyobb->usergroup['canbypasseventmod'],
		"canmoderateevents" => $daddyobb->usergroup['canmoderateevents']
	);
...
function get_events($calendar, $start, $end, $unapproved=0, $private=1)
{
	...
	$query = $db->query("
		...
		WHERE e.cid='{$calendar}' {$visible_where} AND ((e.endtime>={$start} AND e.starttime<={$end}) OR (e.endtime=0 AND e.starttime>={$start} AND e.starttime<={$end})) AND ((e.uid='{$daddyobb->user['uid']}' AND private='1') OR private!='1')

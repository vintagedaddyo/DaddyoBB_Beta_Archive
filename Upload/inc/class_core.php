...
class DaddyoBB {
	/**
	 * The friendly version number of DaddyoBB we're running.
	 *
	 * @var string
	 */
	...
	/**
	 * The version code of DaddyoBB we're running.
	 *
	 * @var integer
	 */
	var $version_code = "1000";
	...
	function DaddyoBB()
	{
		...
		if(isset($daddyobb->input['debug']) || preg_match("#[?&]debug=1#", $_SERVER['REQUEST_URI']))
		{
		...
		if(isset($this->input['action']) && $this->input['action'] == "daddyobb_logo")
		{
			require_once dirname(__FILE__)."/daddyobb_group.php";
			output_logo();
		}
	...
	function trigger_generic_error($code)
	{
		...
		switch($code)
		{
			...
			case "install_directory":
				$message = "The install directory (install/) still exists on your server and is not locked. To access DaddyoBB please either remove this directory or create an empty file in it called 'lock'.";
				break;
			...
			case "board_not_upgraded":
				...
				break;
			case "sql_load_error":
				$message = "DaddyoBB was unable to load the SQL extension. Please contact the DaddyoBB Group for support. <a href=\"http://www.mybboard.net\">DaddyoBB Website</a>";
				break;
			default:
				$message = "DaddyoBB has experienced an internal error. Please contact the DaddyoBB Group for support. <a href=\"http://www.mybboard.net\">DaddyoBB Website</a>";
		}
		$error_handler->trigger($message, DADDYOBB_GENERAL);
	}
...
/**
 * Do this here because the core is used on every DaddyoBB page
 */

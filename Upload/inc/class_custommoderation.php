		...
		else
		{
			...
			if($post_options['splitposts'] > 0 || $post_options['splitposts'] == -2) // Split posts
			{
				...
				if(!empty($post_options['splitpostsaddreply'])) // Add reply to new thread
				{
					...
					{
						$post_options['splitpostsreplysubject'] = str_ireplace('{username}', $daddyobb->user['username'], $thread_options['replysubject']);
		...
		else
		{
			...
			if(!empty($thread_options['addreply'])) // Add reply to thread
			{
				...
				while($thread = $db->fetch_array($query))
				{
					...
					{
						$thread_options['replysubject'] = str_ireplace('{username}', $daddyobb->user['username'], $thread_options['replysubject']);

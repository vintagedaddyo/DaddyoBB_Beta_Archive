...
class pluginSystem
{
	...
	function load()
	{
		...
		if(is_array($pluginlist['active']))
		{
			...
			{
				if($plugin != "" && file_exists(DADDYOBB_ROOT."inc/plugins/".$plugin.".php"))
				{
					require_once DADDYOBB_ROOT."inc/plugins/".$plugin.".php";
				}
			}
		}
	}
	...
	/**
	 * Establishes if a particular plugin is compatible with this version of DaddyoBB.
	 *
	...
	function is_compatible($plugin)
	{
		...
		if(!file_exists(DADDYOBB_ROOT."inc/plugins/".$plugin.".php"))
		{
			return true;
		}
		...
		foreach($compatibility as $version)
		{
			...
			if(preg_match("#{$version}#i", $daddyobb->version_code))
			{
				return true;
			}
		}

...
class UserDataHandler extends DataHandler
{
	...
	*/
	...
	function verify_username()
	{
		...
		if(($daddyobb->settings['maxnamelength'] != 0 && my_strlen($username) > $daddyobb->settings['maxnamelength']) || ($daddyobb->settings['minnamelength'] != 0 && my_strlen($username) < $daddyobb->settings['minnamelength']))
		{
			$this->set_error('invalid_username_length', array($daddyobb->settings['minnamelength'], $daddyobb->settings['maxnamelength']));
			return false;
		}
	...
	function verify_usertitle()
	{
		...
		if($daddyobb->settings['customtitlemaxlength'] != 0 && my_strlen($usertitle) > $daddyobb->settings['customtitlemaxlength'])
		{
			$this->set_error('invalid_usertitle_length', $daddyobb->settings['customtitlemaxlength']);
			return false;
		}
	...
	*/
	function verify_password()
	{
		...
		if(my_strlen($user['password']) < $daddyobb->settings['minpasswordlength'])
		{
			$this->set_error('invalid_password_length', array($daddyobb->settings['minpasswordlength'], $daddyobb->settings['maxpasswordlength']));
			return false;
		}
	...
	*/
	function verify_email()
	{
		...
		if($daddyobb->settings['allowmultipleemails'] == 0 && !defined("IN_ADMINCP"))
		{
	...
	*/
	function verify_referrer()
	{
		...
		if($daddyobb->settings['usereferrals'] == 1 && $user['referrer'] != '')
		{
	...
	*/
	function verify_options()
	{
		...
		$this->verify_yesno_option($options, 'vcard', 1);
		...
		$this->verify_yesno_option($options, 'enablevms', 1);
		$this->verify_yesno_option($options, 'limitvms', 1);
		...
		{
			if($daddyobb->settings['threadusenetstyle'])
			{
		...
		if($this->method == "insert" || (array_key_exists('tpp', $options) && $daddyobb->settings['usertppoptions']))
		{
		...
		if($this->method == "insert" || (array_key_exists('ppp', $options) && $daddyobb->settings['userpppoptions']))
		{
	...
	*/
	function insert_user()
	{
		...
		$this->user_insert_data = array(
			...
			"profilepic" => $db->escape_string($user['profilepic']),
			...
			"limitvms" => intval($user['limitvms']),
			"enablevms" => intval($user['enablevms']),
	...
	*/
	function update_user()
	{
		...
		{
      $this->user_update_data['profilepic'];
		}
		...
		if(isset($user['enablevms']))
		{
      $this->user_update_data['enablevms'] = intval($user['enablevms']);
		}
		if(isset($user['limitvms']))
		{
      $this->user_update_data['limitvms'] = intval($user['limitvms']);
		}

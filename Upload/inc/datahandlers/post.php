...
class PostDataHandler extends DataHandler
{
	...
	*/
	...
	function verify_message()
	{
		...
		else if(my_strlen($post['message']) > $daddyobb->settings['maxmessagelength'] && $daddyobb->settings['maxmessagelength'] > 0 && !is_moderator($post['fid'], "", $post['uid']))
		{
			$this->set_error("message_too_long", array($daddyobb->settings['maxmessagelength']));
			return false;
		}

		// And if we've got a minimum message length do we meet that requirement too?
		else if(my_strlen($post['message']) < $daddyobb->settings['minmessagelength'] && $daddyobb->settings['minmessagelength'] > 0 && !is_moderator($post['fid'], "", $post['uid']))
		{
			$this->set_error("message_too_short", array($daddyobb->settings['minmessagelength']));
			return false;
		}
		return true;
	}
	...
	*/
	function verify_post_flooding()
	{
		...
		// Check if post flooding is enabled within DaddyoBB or if the admin override option is specified.
		if($daddyobb->settings['postfloodcheck'] == 1 && $post['uid'] != 0 && $this->admin_override == false)
		{
			...
			if(TIME_NOW-$user['lastpost'] <= $daddyobb->settings['postfloodsecs'] && !is_moderator($post['fid'], "", $user['uid']))
			{
				// Oops, user has been flooding - throw back error message.
				$time_to_wait = ($daddyobb->settings['postfloodsecs'] - (TIME_NOW-$user['lastpost'])) + 1;
				if($time_to_wait == 1)
				{
	...
	function verify_post_merge($simple_mode=false)
	{
		...
		if(empty($daddyobb->settings['postmergemins']))
		{
			return true;
		}
		
		// Assign a default separator if none is specified
		if(trim($daddyobb->settings['postmergesep']) == "")
		{
			$daddyobb->settings['postmergesep'] = "[hr]";
		}
		
		// Check to see if this person is in a usergroup that is excluded
		if(trim($daddyobb->settings['postmergeuignore']) != "")
		{
			$gids = explode(',', $daddyobb->settings['postmergeuignore']);
			$gids = array_map('intval', $gids);
			
			
			$user_usergroups = explode(',', $daddyobb->user['usergroup'].",".$daddyobb->user['additionalgroups']);
		...
		if((intval($daddyobb->settings['postmergemins']) != 0 && trim($daddyobb->settings['postmergemins']) != "") && (TIME_NOW-$thread['lastpost']) > (intval($daddyobb->settings['postmergemins'])*60))
		{
			return true;
		}
		
		if(strstr($daddyobb->settings['postmergefignore'], ','))
		{
			$fids = explode(',', $daddyobb->settings['postmergefignore']);
		...
		else if(trim($daddyobb->settings['postmergefignore']) != "" && $thread['fid'] == intval($daddyobb->settings['postmergefignore']))
		{
			return true;
		}
	...
	*/
	function verify_image_count()
	{
		...
		if($post['savedraft'] != 1 && $daddyobb->settings['maxpostimages'] != 0 && $permissions['cancp'] != 1)
		{
			...
			if($image_count > $daddyobb->settings['maxpostimages'])
			{
				// Throw back a message if over the count with the number of images as well as the maximum number of images per post.
				$this->set_error("too_many_images", array(1 => $image_count, 2 => $daddyobb->settings['maxpostimages']));
				return false;
			}
		}
	}
	...
	*/
	...
	{
		global $db, $daddyobb, $plugins, $cache, $lang;
		...
		else
		{
			...
			if($daddyobb->user['uid'] == $post['uid'] && $daddyobb->user['moderateposts'] == 1)
			{
				$visible = 0;
			}
		}
		...
		if($this->method != "update" && $visible == 1)
		{
			...
			if($double_post !== true && $double_post['visible'] == $visible)
			{
				...
				$double_post['message'] .= $daddyobb->settings['postmergesep']."\n".$post['message'];
				$update_query = array(
		...
		if($visible == 1)
		{
			...
			$excerpt = my_substr($excerpt, 0, $daddyobb->settings['subscribeexcerpt']).$lang->emailbit_viewthread;
			...
			while($subscribedmember = $db->fetch_array($query))
			{
				...
				elseif($daddyobb->settings['orig_bblanguage'])
				{
					$uselang = $daddyobb->settings['orig_bblanguage'];
				}
				else
				{
					$uselang = "english";
				...
				if($uselang == $daddyobb->settings['bblanguage'])
				{
				...
				else
				{
					if(!isset($langcache[$uselang]['emailsubject_subscription']))
					{
						...
						$userlang->set_path(DADDYOBB_ROOT."inc/languages");
				...
				$emailmessage = $lang->sprintf($emailmessage, $subscribedmember['username'], $post['username'], $daddyobb->settings['bbname'], $subject, $excerpt, $daddyobb->settings['bburl'], str_replace("&amp;", "&", get_thread_link($thread['tid'], 0, "newpost")), $thread['tid'], $subscribedmember['subscriptionkey']);
				$new_email = array(
	...
	function insert_thread()
	{
		...
		else
		{
			...
			// Are posts from this user being moderated? Change visibility
			if($daddyobb->user['uid'] == $thread['uid'] && $daddyobb->user['moderateposts'] == 1)
			{
				$visible = 0;
			}
		}
		...
		if(!$thread['savedraft'])
		{
			...
			if($visible == 1)
			{
				...
				$excerpt = my_substr($thread['message'], 0, $daddyobb->settings['subscribeexcerpt']).$lang->emailbit_viewthread;
				...
				while($subscribedmember = $db->fetch_array($query))
				{
					...
					else if($daddyobb->settings['bblanguage'])
					{
						$uselang = $daddyobb->settings['bblanguage'];
					}
					else
					{
						$uselang = "english";
					...
					$emailmessage = $lang->sprintf($emailmessage, $subscribedmember['username'], $thread['username'], $forum['name'], $daddyobb->settings['bbname'], $thread['subject'], $excerpt, $daddyobb->settings['bburl'], get_thread_link($this->tid), $thread['fid']);
					$new_email = array(
						"mailto" => $db->escape_string($subscribedmember['email']),
						"mailfrom" => '',
						"subject" => $db->escape_string($emailsubject),
	...
	function update_post()
	{
		...
		if(($daddyobb->settings['showeditedby'] == 1 && !is_moderator($post['fid'], "caneditposts", $post['edit_uid'])) || ($daddyobb->settings['showeditedbyadmin'] == 1 && is_moderator($post['fid'], "caneditposts", $post['edit_uid'])))
		{

...
class PMDataHandler extends DataHandler
{
	...
	function verify_recipient()
	{
		...
		foreach($recipients as $user)
		{
			...
			if($recipient_permissions['pmquota'] != "0" && $user['totalpms'] >= $recipient_permissions['pmquota'] && $recipient_permissions['cancp'] != 1 && $sender_permissions['cancp'] != 1 && !$pm['saveasdraft'] && !$this->admin_override)
			{
				...
				elseif($daddyobb->settings['bblanguage'])
				{
				...
				if($uselang == $daddyobb->settings['bblanguage'] || !$uselang)
				{
				...
				$emailmessage = $lang->sprintf($emailmessage, $user['username'], $daddyobb->settings['bbname'], $daddyobb->settings['bburl']);
				$emailsubject = $lang->sprintf($emailsubject, $daddyobb->settings['bbname']);
	...
	*/
	function verify_pm_flooding()
	{
		...
		if($daddyobb->settings['postfloodcheck'] == 1 && $pm['fromid'] != 0 && $this->admin_override == false)
		{
			// Fetch the senders profile data.
			$sender = get_user($pm['fromid']);
			...
			if(TIME_NOW-$sender['lastpm'] <= $daddyobb->settings['postfloodsecs'] && !is_moderator("", "", $pm['fromid']))
			{
				// Oops, user has been flooding - throw back error message.
				$time_to_wait = ($daddyobb->settings['postfloodsecs'] - (TIME_NOW-$sender['lastpm'])) + 1;
				if($time_to_wait == 1)
				{
	...
	function insert_pm()
	{
		global $db, $daddyobb, $plugins, $lang;
		...
		foreach($pm['recipients'] as $recipient)
		{
			...
			if($recipient['pmnotify'] == 1 && $recipient['lastactive'] > $lastpm['dateline'])
			{
				...
				if($uselang == $daddyobb->settings['bblanguage'] && !empty($lang->emailsubject_newpm))
				{
				...
				{
					$pm['sender']['username'] = 'DaddyoBB Engine';
				}
				
				$emailmessage = $lang->sprintf($emailmessage, $recipient['username'], $pm['sender']['username'], $daddyobb->settings['bbname'], $daddyobb->settings['bburl']);
				$emailsubject = $lang->sprintf($emailsubject, $daddyobb->settings['bbname']);

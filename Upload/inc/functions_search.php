...
{
	global $db, $pforumcache, $permissioncache, $daddyobb, $selecteddone, $forumlist, $forumlistbits, $theme, $templates, $lang, $forumpass;
	$pid = intval($pid);
	...
	if(is_array($pforumcache[$pid]))
	{
		foreach($pforumcache[$pid] as $key => $main)
		{
			foreach($main as $key => $forum)
			{
				...
				if(($perms['canview'] == 1 || $daddyobb->settings['hideprivateforums'] == 0) && $perms['cansearch'] != 0)
				{
...
{
	global $db, $forum_cache, $permissioncache, $daddyobb, $unsearchableforums, $unsearchable, $templates, $forumpass;
...
function perform_search_mysql($search)
{
	...
	if($daddyobb->settings['minsearchword'] < 1)
	{
		$daddyobb->settings['minsearchword'] = 3;
	}
	...
	if($keywords)
	{
		...
		if(preg_match("# and|or #", $keywords))
		{
			...
			foreach($keywords_exp as $phrase)
			{
				...
				if(!$inquote)
				{
					...
					for($i=0; $i < $count_matches; ++$i)
					{
						...
						else
						{
							...
							if(my_strlen($word) < $daddyobb->settings['minsearchword'])
							{
								$lang->error_minsearchlength = $lang->sprintf($lang->error_minsearchlength, $daddyobb->settings['minsearchword']);
				...
				else
				{
					...
					if(my_strlen($phrase) < $daddyobb->settings['minsearchword'])
					{
						$lang->error_minsearchlength = $lang->sprintf($lang->error_minsearchlength, $daddyobb->settings['minsearchword']);
						error($lang->error_minsearchlength);
					}
		...
		else
		{
			...
			if(my_strlen($keywords) < $daddyobb->settings['minsearchword'])
			{
				$lang->error_minsearchlength = $lang->sprintf($lang->error_minsearchlength, $daddyobb->settings['minsearchword']);
				error($lang->error_minsearchlength);
			}
				...
				switch($db->type)
				{
					case "pgsql":
						$query = $db->query("
							...
							LEFT JOIN ".TABLE_PREFIX."forumpermissions p ON (f.fid=p.fid AND p.gid='".$daddyobb->user['usergroup']."')
...
function perform_search_mysql_ft($search)
{
	...
	{
		$daddyobb->settings['minsearchword'] = $min_length;
	}
	// Otherwise, could not fetch - default back to MySQL fulltext default setting
	else
	{
		$daddyobb->settings['minsearchword'] = 4;
	}

...
{
	global $fcache, $moderatorcache, $forumpermissions, $theme, $daddyobb, $templates, $bgcolor, $collapsed, $lang, $showdepth, $plugins, $parser, $forum_viewers;
	...
	foreach($fcache[$pid] as $parent)
	{
		foreach($parent as $forum)
		{
			...
			if($permissions['canview'] != 1 && $daddyobb->settings['hideprivateforums'] == 1)
			{
				continue;
			}
			...
			$hideinfo = false;
			if($forum['password'] != '' && $daddyobb->cookies['forumpass'][$forum['fid']] != md5($daddyobb->user['uid'].$forum['password']))
			{
				$hideinfo = true;
			}
			...
			else if($depth == 3)
			{
				if($donecount < $daddyobb->settings['subforumsindex'])
				{
					...
					if($daddyobb->settings['subforumsstatusicons'] == 1)
					{
			...
			if($forum['linkto'] == '')
			{
				...
				elseif($hideinfo != true)
				{
					...
					$lastpost_date = my_date($daddyobb->settings['dateformat'], $lastpost_data['lastpost']);
					$lastpost_time = my_date($daddyobb->settings['timeformat'], $lastpost_data['lastpost']);
				...
				if($daddyobb->settings['showforumviewing'] != 0 && $forum['viewers'] > 0)
				{
					if($forum['viewers'] == 1)
					{
...
{
	global $daddyobb, $lang, $db, $unread_forums;
	...
	else
	{
		...
		if(!$forum_read)
		{
			$forum_read = $daddyobb->user['lastvisit'];
		}

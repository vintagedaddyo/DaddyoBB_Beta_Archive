...
{
	global $db, $altbg, $theme, $daddyobb, $postcounter;
	...
	{
		$unapproved_shade = 'trow_shaded';
	}
	else
	{
		$unapproved_shade = 'trow1';
	}
	$post['fid'] = $fid;
	switch($post_type)
	{
		...
		case 2: // Private message
			global $message, $pmid;
			$parser_options['allow_html'] = $daddyobb->settings['pmsallowhtml'];
			$parser_options['allow_mycode'] = $daddyobb->settings['pmsallowmycode'];
			$parser_options['allow_smilies'] = $daddyobb->settings['pmsallowsmilies'];
			$parser_options['allow_imgcode'] = $daddyobb->settings['pmsallowimgcode'];
			$parser_options['me_username'] = $post['username'];
			$parser_options['filter_badwords'] = 1;
			$id = $pmid;
			break;
	...
	{ // Used to show the # of the post
		if($page > 1)
		{
			...
			$postcounter = $daddyobb->settings['postsperpage']*($page-1);
		}
		else
		{
			$postcounter = 0;
	...
	elseif($daddyobb->input['mode'] == "threaded")
	{
		$post_extra_style = "border-top-width: 0;";
	}
	else
	...
	$post['postdate'] = my_date($daddyobb->settings['dateformat'], $post['dateline']);
	$post['posttime'] = my_date($daddyobb->settings['timeformat'], $post['dateline']);
	...
	{ // This post was made by a registered user
		...
		if($post['lastactive'] > $timecut && ($post['invisible'] != 1 || $daddyobb->usergroup['canviewwolinvis'] == 1) && $post['lastvisit'] != $post['lastactive'])
		{
			eval("\$post['onlinestatus'] = \"".$templates->get("postbit_online")."\";");
		}
		else
		{
			if($post['away'] == 1 && $daddyobb->settings['allowaway'] != 0)
			{
		...
		if($post['avatar'] != "" && ($daddyobb->user['showavatars'] != 0 || !$daddyobb->user['uid']))
		{
			...
			{
				list($max_width, $max_height) = explode("x", my_strtolower($daddyobb->settings['postmaxavatarsize']));
		...
		if($post['hideemail'] != 1 && $daddyobb->usergroup['cansendemail'] == 1)
		{
		...
		$post['userregdate'] = my_date($daddyobb->settings['regdateformat'], $post['regdate']);

		// Work out the reputation this user has (only show if not announcement)
		if($post_type != 3 && $usergroup['usereputationsystem'] != 0 && $daddyobb->settings['enablereputation'] == 1)
		{
		...
		if($post_type != 3 && $daddyobb->settings['enablewarningsystem'] != 0 && $usergroup['canreceivewarnings'] != 0 && ($daddyobb->usergroup['canwarnusers'] != 0 || ($daddyobb->user['uid'] == $post['uid'] && $daddyobb->settings['canviewownwarning'] != 0)))
		{
			$warning_level = round($post['warningpoints']/$daddyobb->settings['maxwarningpoints']*100);
			...
			if($daddyobb->usergroup['canwarnusers'] != 0 && $post['uid'] != $daddyobb->user['uid'] && ($post_type == 0 || $post_type == 2))
			{
		...
		//Shall we show the messenger icons?
		if($post['icq'] || $post['msn'] || $post['yahoo'] || $post['aim'])
		{
			$user_icons = "<div>";
			if($post['icq'])
			{
				$user_icons .= "<a href=\"javascript:;\" onclick=\"DaddyoBB.popupWindow('misc.php?action=imcenter&amp;imtype=icq&amp;uid={$post['uid']}', 'imcenter', 450, 300);\"><img src=\"{$theme['imgdir']}/messenger/im_icq.gif\"></a>";
			}
			if($post['aim'])
			{
				$user_icons .= "<a href=\"javascript:;\" onclick=\"DaddyoBB.popupWindow('misc.php?action=imcenter&amp;imtype=aimq&amp;uid={$post['uid']}', 'imcenter', 450, 300);\"><img src=\"{$theme['imgdir']}/messenger/im_aim.gif\"></a>";
			}
			if($post['msn'])
			{
				$user_icons .= "<a href=\"javascript:;\" onclick=\"DaddyoBB.popupWindow('misc.php?action=imcenter&amp;imtype=msn&amp;uid={$post['uid']}', 'imcenter', 450, 300);\"><img src=\"{$theme['imgdir']}/messenger/im_msn.gif\"></a>";
			}
			if($post['yahoo'])
			{
				$user_icons .= "<a href=\"javascript:;\" onclick=\"DaddyoBB.popupWindow('misc.php?action=imcenter&amp;imtype=yahoo&amp;uid={$post['uid']}', 'imcenter', 450, 300);\"><img src=\"{$theme['imgdir']}/messenger/im_yahoo.gif\"></a>";
			}
			$user_icons .= "</div>";
		}
		else
	...
	if(!$post_type)
	{
		...
		if($post['edituid'] != 0 && $post['edittime'] != 0 && $post['editusername'] != "" && ($daddyobb->settings['showeditedby'] != 0 && $usergroup['cancp'] == 0 || $daddyobb->settings['showeditedbyadmin'] != 0 && $usergroup['cancp'] == 1))
		{
			$post['editdate'] = my_date($daddyobb->settings['dateformat'], $post['edittime']);
			$post['edittime'] = my_date($daddyobb->settings['timeformat'], $post['edittime']);
			$post['editedprofilelink'] = build_profile_link($post['editusername'], $post['edituid']);
			if($post['editreason'])
			{
				$post['editnote'] = $lang->sprintf($lang->postbit_edited_reason, $post['editdate'], $post['edittime'], $post['editedprofilelink'], $post['editreason']);
			}
			else
			{
		...
		if((is_moderator($fid, "caneditposts") || ($forumpermissions['caneditposts'] == 1 && $daddyobb->user['uid'] == $post['uid'])) && $daddyobb->user['uid'] != 0)
		{
		...
		if((is_moderator($fid, "candeleteposts") || $can_delete == 1) && $daddyobb->user['uid'] != 0)
		{
		...
		if($ismod)
		{
			if(my_strpos($daddyobb->cookies[$inlinecookie], "|".$post['pid']."|"))
			{
		...
		if($forumpermissions['canpostreplys'] != 0 && ($thread['closed'] != 1 || is_moderator($fid)) && $daddyobb->settings['multiquote'] != 0 && $forum['open'] != 0 && !$post_type)
		{
		...
		if($daddyobb->settings['logip'] != "no")
		{
			if($daddyobb->settings['logip'] == "show")
			{
				eval("\$post['iplogged'] = \"".$templates->get("postbit_iplogged_show")."\";");
			}
			else if($daddyobb->settings['logip'] == "hide" && $ismod)
			{
	...
	if($daddyobb->input['highlight'])
	{
		...
		{
			$highlight_cache = build_highlight_array($daddyobb->input['highlight']);
		}
		if(is_array($highlight_cache))
		{
	...
	if($post['includesig'] != 0 && $post['username'] && $post['signature'] != "" && ($daddyobb->user['uid'] == 0 || $daddyobb->user['showsigs'] != 0))
	{
	    //fetch posters permission
	    $poster_perms = user_permissions($post['uid']);
    
		$sig_parser = array(
			"allow_html" => $poster_perms['sigallowhtml'],
			"allow_mycode" => $poster_perms['sigallowmycode'],
			"allow_smilies" => $poster_perms['sigallowsmilies'],
			"allow_imgcode" => $poster_perms['sigallowimgcode'],
...
{
	global $attachcache, $daddyobb, $theme, $templates, $forumpermissions, $lang;
		...
		foreach($attachcache[$id] as $aid => $attachment)
		{
			...
			{ // There is an attachment thats visible!
				...
				if(stripos($post['message'], "[attachment=".$attachment['aid']."]") !== false)
				{
					...
					if($attachment['thumbnail'] != "SMALL" && $attachment['thumbnail'] != "" && $daddyobb->settings['attachthumbnails'] == "yes")
					{
						eval("\$attbit = \"".$templates->get("postbit_attachments_thumbnails_thumbnail")."\";");
					}
					elseif((($attachment['thumbnail'] == "SMALL" && $forumpermissions['candlattachments'] == 1) || $daddyobb->settings['attachthumbnails'] == "no") && $isimage)
					{

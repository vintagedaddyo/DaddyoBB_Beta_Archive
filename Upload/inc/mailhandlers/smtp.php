...
if(!defined('DADDYOBB_SSL'))
{
	define('DADDYOBB_SSL', 1);
}

if(!defined('DADDYOBB_TLS'))
{
	define('DADDYOBB_TLS', 2);
}
...
class SmtpMail extends MailHandler
{
	...
	function __construct()
	{
		...
		$protocol = '';
		switch($daddyobb->settings['secure_smtp'])
		{
			case DADDYOBB_SSL:
				$protocol = 'ssl://';
				break;
		...
		if(empty($daddyobb->settings['smtp_host']))
		{
			$this->host = @ini_get('SMTP');
		}
		else
		{
			$this->host = $daddyobb->settings['smtp_host'];
		}
		...
		if(empty($daddyobb->settings['smtp_port']) && !empty($protocol) && !@ini_get('smtp_port'))
		{
			$this->port = $this->secure_port;
		}
		else if(empty($daddyobb->settings['smtp_port']) && @ini_get('smtp_port'))
		{
			$this->port = @ini_get('smtp_port');
		}
		else if(!empty($daddyobb->settings['smtp_port']))
		{
			$this->port = $daddyobb->settings['smtp_port'];
		}
	
		$this->password = $daddyobb->settings['smtp_pass'];
		$this->username = $daddyobb->settings['smtp_user'];
	}
	...
	function connect()
	{
		...
		if(is_resource($this->connection))
		{
			...
			if(!empty($this->username) && !empty($this->password))
			{
				...
				{
					$this->fatal_error("DaddyoBB was unable to authenticate you against the SMTP server");
					return false;
				}
			}
			else
	...
	function auth($auth_methods)
	{
		...
		{
			$this->fatal_error("The SMTP server does not support any of the AUTH methods that DaddyoBB supports");
			return false;
		}

...
class datacache
{
	...
	function cache()
	{
		...
		switch($daddyobb->config['cache_store'])
		{
			// Disk cache
			case "files":
				require_once DADDYOBB_ROOT."/inc/cachehandlers/disk.php";
				$this->handler = new diskCacheHandler;
				break;
			// Memcache cache
			case "memcache":
				require_once DADDYOBB_ROOT."/inc/cachehandlers/memcache.php";
				$this->handler = new memcacheCacheHandler;
				break;
			// eAccelerator cache
			case "eaccelerator":
				require_once DADDYOBB_ROOT."/inc/cachehandlers/eaccelerator.php";
				$this->handler = new eacceleratorCacheHandler;
				break;
			// Xcache cache
			case "xcache":
				require_once DADDYOBB_ROOT."/inc/cachehandlers/xcache.php";
	...
	/**
	 * Update the DaddyoBB version in the cache.
	 *
	 */
	function update_version()
	{
		...
		$version = array(
			"version" => $daddyobb->version,
			"version_code" => $daddyobb->version_code
		);
	...
	function update_most_replied_threads()
	{
		...
		$query = $db->simple_select("threads", "tid, subject, replies, fid", "visible='1'", array('order_by' => 'replies', 'order_dir' => 'DESC', 'limit_start' => 0, 'limit' => $daddyobb->settings['statslimit']));
	...
	function update_most_viewed_threads()
	{
		...
		$query = $db->simple_select("threads", "tid, subject, views, fid", "visible='1'", array('order_by' => 'views', 'order_dir' => 'DESC', 'limit_start' => 0, 'limit' => $daddyobb->settings['statslimit']));
		while($thread = $db->fetch_array($query))
		{
			$threads[] = $thread;
		}

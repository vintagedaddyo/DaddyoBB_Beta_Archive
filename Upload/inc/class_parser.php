...
class postParser
{
	...
	{
		global $plugins, $daddyobb;

		// Set base URL for parsing smilies
		$this->base_url = $daddyobb->settings['bburl'];
		...
		if($options['allow_mycode'])
		{
			...
			if(count($code_matches) > 0)
			{
				foreach($code_matches as $text)
				{
					...
					$message = preg_replace("#\<daddyobb-code>\n?#", $code, $message, 1);
				}
			}
		}
	...
	function cache_mycode()
	{
		...
		$standard_mycode['align']['replacement'] = "<div align=\"text-align: $1;\">$2</div>";
	...
	function parse_smilies($message, $allow_html=0)
	{
        ...
        $message = preg_replace("#\[(url(=[^\]]*])?\].*?\[\/url\]|quote=([^\]]*)?\])#si", "<daddyobb-bad-sm>", $message);
		...
		if(count($bad_matches[0]) > 0)
		{
			...
			{
				$message = preg_replace("#<daddyobb-bad-sm>#", $match, $message, 1);
			}
		}
	...
	*/
	...
	{
		global $lang, $templates, $theme, $daddyobb;
		...
		if($text_only == false)
		{
			$replace = array(
				...
				"<div class=\"smallfont\" style=\"margin-bottom: 2px;\">$lang->quote:</div><table border=\"0\" cellpadding=\"6\" cellspacing=\"0\" width=\"100%\">
					<tbody><tr><td class=\"trow2\" style=\"border: 1px inset ;\">\n<i>$1</i></td></tr></tbody></table>\n"
			);
		}
		else
		{
			$replace = array(
				...
				"<div class=\"smallfont\" style=\"margin-bottom: 2px;\">$lang->quote:</div><table border=\"0\" cellpadding=\"6\" cellspacing=\"0\" width=\"100%\">
				<tbody><tr><td class=\"trow2\" style=\"border: 1px inset ;\">	<i>$1</i></td></tr></tbody></table>\n"
			);
		}
	...
	*/
	function mycode_parse_post_quotes($message, $username, $text_only=false)
	{
		...
		{
			$pid = intval($match[1]);
			$url = $daddyobb->settings['bburl']."/".get_post_link($pid)."#pid$pid";
		...
		if(intval($match[1]))
		{
			if($match[1] < TIME_NOW)
			{
				$postdate = my_date($daddyobb->settings['dateformat'], intval($match[1]));
				$posttime = my_date($daddyobb->settings['timeformat'], intval($match[1]));
		...
		else
		{
			...
			if(!$delete_quote)
			{
				$span = " @ <span class=\"time\">{$date}</span>";
			}
			
			return "<div class=\"smallfont\" style=\"margin-bottom: 2px;\">Quote:</div>
								<table border=\"0\" cellpadding=\"6\" cellspacing=\"0\" width=\"100%\">
								<tbody><tr><td class=\"trow2\" style=\"border: 1px inset ;\">$lang->originally_posted_by<b>$username</b>{$span}{$linkback}
								<i>{$message}</i></td></tr></tbody></table>\n";
		}
	}

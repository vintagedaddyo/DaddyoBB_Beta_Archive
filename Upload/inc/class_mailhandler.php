...
class MailHandler
{
	...
	function set_common_headers()
	{
		...
		if(!trim($this->from))
		{
			...
			{
				$this->from = $daddyobb->settings['adminemail'];
			}
			else
			{
				$this->from = '"'.$this->utf8_encode($daddyobb->settings['bbname']).'"';
				$this->from .= " <{$daddyobb->settings['adminemail']}>";
			}
		}
		...
		elseif($daddyobb->settings['returnemail'])
		{
			$this->headers .= "Return-Path: {$daddyobb->settings['returnemail']}{$this->delimiter}";
			$this->headers .= "Reply-To: {$daddyobb->settings['adminemail']}{$this->delimiter}";
		}
		else
		{
			$this->headers .= "Return-Path: {$daddyobb->settings['adminemail']}{$this->delimiter}";
			$this->headers .= "Reply-To: {$daddyobb->settings['adminemail']}{$this->delimiter}";
		}
		...
		$this->headers .= "X-Mailer: DaddyoBB{$this->delimiter}";
		if(defined("IN_ADMINCP"))
		{
			$_SERVER['PHP_SELF'] = str_replace($daddyobb->config['admin_dir']."/", "admin-", $_SERVER['PHP_SELF']);
		}
		$this->headers .= "X-DaddyoBB-Script: {$http_host}{$_SERVER['PHP_SELF']}{$this->delimiter}";

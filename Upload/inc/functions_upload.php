...
function remove_attachment($pid, $posthash, $aid)
{
	...
	@unlink($daddyobb->settings['uploadspath']."/".$attachment['attachname']);
	if($attachment['thumbnail'])
	{
		@unlink($daddyobb->settings['uploadspath']."/".$attachment['thumbnail']);
	}

	$date_directory = explode('/', $attachment['attachname']);
	if(@is_dir($daddyobb->settings['uploadspath']."/".$date_directory[0]))
	{
		@rmdir($daddyobb->settings['uploadspath']."/".$date_directory[0]);
	}
...
{
	global $daddyobb, $plugins;
	
	if(defined('IN_ADMINCP'))
	{
		$avatarpath = '../'.$daddyobb->settings['avataruploadpath'];
	}
	else
	{
		$avatarpath = $daddyobb->settings['avataruploadpath'];
...
{
	global $db, $daddyobb, $lang, $plugins;
	...
	{
		list($maxwidth, $maxheight) = @explode("x", $daddyobb->settings['maxavatardims']);
		if(($maxwidth && $img_dimensions[0] > $maxwidth) || ($maxheight && $img_dimensions[1] > $maxheight))
		{
			// Automatic resizing enabled?
			if($daddyobb->settings['avatarresizing'] == "auto" || ($daddyobb->settings['avatarresizing'] == "user" && $daddyobb->input['auto_resize'] == 1))
			{
			...
			{
				$ret['error'] = $lang->sprintf($lang->error_avatartoobig, $maxwidth, $maxheight);
				if($daddyobb->settings['avatarresizing'] == "user")
				{
	...
	if($avatar['size'] > ($daddyobb->settings['avatarsize']*1024) && $daddyobb->settings['avatarsize'] > 0)
	{
		@unlink($avatarpath."/".$filename);
	...
	$ret = array(
		"avatar" => $daddyobb->settings['avataruploadpath']."/".$filename,
...
{
	global $db, $theme, $templates, $posthash, $pid, $tid, $forum, $daddyobb, $lang, $plugins, $cache;
	
	$posthash = $db->escape_string($daddyobb->input['posthash']);
	...
	if($daddyobb->usergroup['attachquota'] > 0)
	{
		...
		if($usage > ($daddyobb->usergroup['attachquota']*1024))
		{
	...
	$month_dir = gmdate("Ym");
	if(!@is_dir($daddyobb->settings['uploadspath']."/".$month_dir))
	{
		@mkdir($daddyobb->settings['uploadspath']."/".$month_dir);
	...
	$filename = "post_".$daddyobb->user['uid']."_".TIME_NOW."_".md5(uniqid(rand(), true)).".attach";
	
	$file = upload_file($attachment, $daddyobb->settings['uploadspath']."/".$month_dir, $filename);
	
	// Failed to create the attachment in the monthly directory, just throw it in the main directory
	if($file['error'] && $month_dir)
	{
		$file = upload_file($attachment, $daddyobb->settings['uploadspath'].'/', $filename);		
	}
	...
	// Lets just double check that it exists
	if(!file_exists($daddyobb->settings['uploadspath']."/".$filename))
	{
	...
	if($ext == "gif" || $ext == "png" || $ext == "jpg" || $ext == "jpeg" || $ext == "jpe")
	{
		...
		// Check if the uploaded file type matches the correct image type (returned by getimagesize)
		$img_dimensions = @getimagesize($daddyobb->settings['uploadspath']."/".$filename);
		if(!is_array($img_dimensions) || ($img_dimensions[2] != $img_type && !in_array(mime_content_type($filename), $supported_mimes)))
		{
			@unlink($daddyobb->settings['uploadspath']."/".$filename);
			$ret['error'] = $lang->error_uploadfailed;
			return $ret;		
		}		
		require_once DADDYOBB_ROOT."inc/functions_image.php";
		$thumbname = str_replace(".attach", "_thumb.$ext", $filename);
		$thumbnail = generate_thumbnail($daddyobb->settings['uploadspath']."/".$filename, $daddyobb->settings['uploadspath'], $thumbname, $daddyobb->settings['attachthumbh'], $daddyobb->settings['attachthumbw']);
	...
	if($forum['modattachments'] == 1 && !is_moderator($forum['fid'], "", $daddyobb->user['uid']))
	{

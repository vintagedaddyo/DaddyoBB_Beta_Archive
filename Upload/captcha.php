...
if($daddyobb->input['imagehash'] == "test")
{
	$imagestring = "DaddyoBB";
}
elseif($daddyobb->input['imagehash'])
{
	$query = $db->simple_select("captcha", "*", "imagehash='".$db->escape_string(strval($daddyobb->input['imagehash']))."'", array("limit" => 1));
...
if(function_exists("imagefttext"))
{
	...
	$ttfdir  = @opendir(DADDYOBB_ROOT."inc/captcha_fonts");
	if($ttfdir)
	{
		while($file = readdir($ttfdir))
		{
			// If this file is a ttf file, add it to the list
			if(is_file(DADDYOBB_ROOT."inc/captcha_fonts/".$file) && get_extension($file) == "ttf")
			{
				$ttf_fonts[] = DADDYOBB_ROOT."inc/captcha_fonts/".$file;
			}
		}
	}
}

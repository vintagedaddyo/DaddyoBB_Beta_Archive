...
function log_admin_action()
{
	...
	$log_entry = array(
		...
		"dateline" => TIME_NOW,
		"module" => $db->escape_string($daddyobb->input['module']),
		"action" => $db->escape_string($daddyobb->input['action']),
...
function draw_admin_pagination($page, $per_page, $total_items, $url)
{
	...
	$max_links = $daddyobb->settings['maxmultipagelinks'];
...
{
	global $daddyobb, $page, $lang, $modules_dir;
	...
	{	
		...
		$permissions = $func();
		if($permissions['permissions'][$action['action']] && $daddyobb->admin['permissions'][$action['module']][$action['action']] != 1)
		{
...
function get_admin_permissions($get_uid="", $get_gid="")
{
	...
	if(!$gid)
	{
		...
		$gid_array[] = (-1) * intval($daddyobb->user['usergroup']);
		
		if($daddyobb->user['additionalgroups'])
		{
			$additional_groups = explode(',', $daddyobb->user['additionalgroups']);

...
function view_manager($base_url, $type, $fields, $sort_options=array(), $conditions_callback="")
{
	...
	if($daddyobb->input['do'] == "set_default")
	{
		$query = $db->simple_select("adminviews", "vid", "vid='".intval($daddyobb->input['vid'])."'");
		$admin_view = $db->fetch_array($query);
		
		if(!$admin_view['vid'] || $admin_view['visibility'] == 1 && $daddyobb->user['uid'] != $admin_view['uid'])
		{
	...
	if($daddyobb->input['do'] == "add")
	{
		if($daddyobb->request_method == "post")
		{
			...
			if($daddyobb->input['fields_js'])
			{
				$daddyobb->input['fields'] = explode(",", $daddyobb->input['fields_js']);
			}
			if(count($daddyobb->input['fields']) <= 0)
			{
				$errors[] = $lang->error_no_view_fields;
			}

			if(intval($daddyobb->input['perpage']) <= 0)
			{
				$errors[] = $lang->error_invalid_view_perpage;
			}

			if(!in_array($daddyobb->input['sortby'], array_keys($sort_options)))
			{
				$errors[] = $lang->error_invalid_view_sortby;
			}

			if($daddyobb->input['sortorder'] != "asc" && $daddyobb->input['sortorder'] != "desc")
			{
				$errors[] = $lang->error_invalid_view_sortorder;
			}

			if($daddyobb->input['visibility'] == 0)
			{
				$daddyobb->input['visibility'] = 2;
			}
			...
			if(!$errors)
			{
				$new_view = array(
					...
					"type" => $type,
					"visibility" => intval($daddyobb->input['visibility']),
					"fields" => $db->escape_string(serialize($daddyobb->input['fields'])),
					"conditions" => $db->escape_string(serialize($daddyobb->input['conditions'])),
					"sortby" => $db->escape_string($daddyobb->input['sortby']),
					"sortorder" => $db->escape_string($daddyobb->input['sortorder']),
					"perpage" => intval($daddyobb->input['perpage']),
					"view_type" => $db->escape_string($daddyobb->input['view_type'])
				);

				$vid = $db->insert_query("adminviews", $new_view);

				if($daddyobb->input['isdefault'])
				{
		...
		if($daddyobb->input['visibility'] == 2)
		{
		...
		$form_container->output_row($lang->set_as_default_view, "", $form->generate_yes_no_radio("isdefault", $daddyobb->input['isdefault'], array('yes' => 1, 'no' => 0)));
		...
		if(count($sort_options) > 0)
		{
			...
			$form_container->output_row($lang->sort_results_by, "", $form->generate_select_box('sortby', $sort_options, $daddyobb->input['sortby'], array('id' => 'sortby'))." {$lang->in} ".$form->generate_select_box('sortorder', $sort_directions, $daddyobb->input['sortorder'], array('id' => 'sortorder')), 'sortby');
		}
		...
		if($type == "user")
		{
			$form_container->output_row($lang->display_results_as, "", $form->generate_radio_button('view_type', 'table', $lang->table, array('checked' => ($daddyobb->input['view_type'] != "card" ? true : false)))."<br />".$form->generate_radio_button('view_type', 'card', $lang->business_card, array('checked' => ($daddyobb->input['view_type'] == "card" ? true : false))));
		}
		...
		if(is_array($daddyobb->input['fields']))
		{
			foreach($daddyobb->input['fields'] as $field)
			{
				if($fields[$field])
				{
...
//]]>
		...
		$field_select .= "<noscript>".$form->generate_select_box('fields', $field_options, $daddyobb->input['fields'], array('id' => 'fields', 'multiple' => true))."</noscript>\n";
		...
		{
			$conditions_callback($daddyobb->input, $form); 
		}
	...
	else if($daddyobb->input['do'] == "edit")
	{
		...
		else
		{
			...
			$daddyobb->input = $admin_view;
		}

		$form_container = new FormContainer($lang->edit_view);
		$form_container->output_row($lang->view." <em>*</em>", "", $form->generate_text_box('title', $daddyobb->input['title'], array('id' => 'title')), 'title');
...
//]]></script>\n";
	...
	else if($daddyobb->input['do'] == "delete")
	{
	...
	// Export views
	else if($daddyobb->input['do'] == "export")
	{
		$xml = "<?xml version=\"1.0\" encoding=\"ISO-8859-1\"?".">\n";
		$xml = "<adminviews version=\"".$daddyobb->version_code."\" exported=\"".TIME_NOW."\">\n";

		if($daddyobb->input['type'])
		{
			$type_where = "type='".$db->escape_string($daddyobb->input['type'])."'";
		}
		...
		header("Content-disposition: filename=".$daddyobb->settings['bbname']."-views.xml");
	...
	else
	{
		...
		$query = $db->query("
			...
			WHERE v.visibility='2' OR (v.visibility='1' AND v.uid='{$daddyobb->user['uid']}')
			ORDER BY title
		");
		while($view = $db->fetch_array($query))
		{
			...
			if($views > 1)
			{
				$popup->add_item($lang->delete_view, "{$base_url}&amp;action=views&amp;do=delete&amp;vid={$view['vid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_view_deletion}')");
			}
...
function set_default_view($type, $vid)
{
	...
	$query = $db->simple_select("adminoptions", "defaultviews", "uid='{$daddyobb->user['uid']}'");
	...
	if($create == true)
	{
		$updated_admin['uid'] = $daddyobb->user['uid'];
		$db->insert_query("adminoptions", $updated_admin);
	}
	else
	{
		$db->update_query("adminoptions", $updated_admin, "uid='{$daddyobb->user['uid']}'");
	}
}

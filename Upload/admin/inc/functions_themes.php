...
function import_theme_xml($xml, $options=array())
{
	...
	// Do we have DaddyoBB 1.2 template's we're importing?
	$css_120 = "";
	...
	if($css_120)
	{
		...
		$theme['stylesheets']['stylesheet'][0]['attributes'] = array('name' => 'global.css', 'version' => $daddyobb->version_code);
	...
	if($daddyobb->version_code != $version && $options['version_compat'] != 1)
	{
		return -2;
	}
...
function cache_stylesheet($tid, $filename, $stylesheet)
{
	...
	if(!is_dir(DADDYOBB_ROOT."cache/themes/theme{$tid}"))
	{
		if(!@mkdir(DADDYOBB_ROOT."cache/themes/theme{$tid}"))
		{
			return false;
		}
		// Add in empty index.html!
		$fp = @fopen(DADDYOBB_ROOT."cache/themes/theme{$tid}/index.html", "w");
		@fwrite($fp, "");
		@fclose($fp);
	}
	...
	$fp = @fopen(DADDYOBB_ROOT."cache/themes/theme{$tid}/{$filename}", "wb");
	if(!$fp)
	{
		return false;
	}
...
function resync_stylesheet($stylesheet)
{
	...
	if(!file_exists(DADDYOBB_ROOT."cache/themes/theme{$stylesheet['tid']}/{$stylesheet['cachefile']}"))
	{
	...
	else if($stylesheet['sid'] != 1 && @filemtime(DADDYOBB_ROOT."cache/themes/theme{$stylesheet['tid']}/{$stylesheet['cachefile']}") > $stylesheet['lastmodified'])
	{
		$contents = unfix_css_urls(file_get_contents(DADDYOBB_ROOT."cache/themes/theme{$stylesheet['tid']}/{$stylesheet['cachefile']}"));
...
function update_theme_stylesheet_list($tid)
{
	...
	foreach($stylesheets as $name => $stylesheet)
	{
		...
		{
			if(file_exists(DADDYOBB_ROOT."cache/themes/theme{$theme_id}/{$stylesheet['cachefile']}") && filemtime(DADDYOBB_ROOT."cache/themes/theme{$theme_id}/{$stylesheet['cachefile']}") >= $stylesheet['lastmodified'])
			{
...
{
	global $daddyobb, $db, $table, $lang, $page; // Global $table is bad, but it will have to do for now
	static $theme_cache;
	...
	{		
		...
		if($theme['tid'] > 1)
		{
			...
			{
				$popup->add_item($lang->delete_theme, "index.php?module=style/themes&amp;action=delete&amp;tid={$theme['tid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_theme_deletion}')");
			}
			...
			$popup->add_item($lang->force_on_users, "index.php?module=style/themes&amp;action=force&amp;tid={$theme['tid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_theme_forced}')");
		}

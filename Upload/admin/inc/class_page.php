...
/*
 * DaddyoBB Admin CP Page Generation Class
 */
class DefaultPage
{
	...
	{
		global $daddyobb, $admin_session, $lang, $plugins;
		$plugins->run_hooks("admin_page_output_header");
		
		if(!$title)
		{
			$title = $lang->DADDYOBB_admin_panel;
		}
		...
		echo "	<meta name=\"author\" content=\"DaddyoBB Group\" />\n";
		echo "	<meta name=\"copyright\" content=\"Copyright ".COPY_YEAR." DaddyoBB Group.\" />\n";
		echo "	<link rel=\"stylesheet\" href=\"styles/".$this->style."/main.css\" type=\"text/css\" />\n";

		// Load stylesheet for this module if it has one
		if(file_exists(DADDYOBB_ADMIN_DIR."styles/{$this->style}/{$this->active_module}.css"))
		{
...
var cookieDomain = '{$daddyobb->settings['cookiedomain']}';
var cookiePath = '{$daddyobb->settings['cookiepath']}';
var cookiePrefix = '{$daddyobb->settings['cookieprefix']}';
...
//]]>
		...
		echo "	<div id=\"logo\"><h1><span class=\"invisible\">{$lang->DADDYOBB_admin_cp}</span></h1></div>\n";
		echo "	<div id=\"welcome\"><span class=\"logged_in_as\">{$lang->logged_in_as} <a href=\"index.php?module=user/users&amp;action=edit&amp;uid={$daddyobb->user['uid']}\" class=\"username\">{$daddyobb->user['username']}</a></span> | <a href=\"{$daddyobb->settings['bburl']}\" target=\"_blank\" class=\"forum\">{$lang->view_board}</a> | <a href=\"index.php?action=logout\" class=\"logout\">{$lang->logout}</a></div>\n";
	...
	{
		global $daddyobb, $maintimer, $db, $lang, $plugins;
		...
		echo "	<br style=\"clear: both;\" />";
		echo "	</div>\n";
		echo "<div id=\"footer\"><p class=\"generation\">".$lang->sprintf($lang->generated_in, $totaltime, $querycount)."</p><p class=\"powered\">Powered By DaddyoBB. &copy; ".COPY_YEAR." DaddyoBB Group. All Rights Reserved.</p></div>\n";
...
<title>{$lang->DADDYOBB_admin_login}</title>
<meta name="author" content="DaddyoBB Group" />
<meta name="copyright" content="Copyright {$copy_year} DaddyoBB Group." />
...
<div id="container"{$login_container_width}>
	<div id="header">
		<div id="logo">
			<h1><a href="../" title="{$lang->return_to_forum}"><span class="invisible">{$lang->DADDYOBB_acp}</span></a></h1>

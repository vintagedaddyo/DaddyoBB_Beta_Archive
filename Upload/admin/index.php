...
if(!isset($config['admin_dir']) || !file_exists(DADDYOBB_ROOT.$config['admin_dir']."/inc/class_page.php"))
{
	$config['admin_dir'] = "admin";
}

define('DADDYOBB_ADMIN_DIR', DADDYOBB_ROOT.$config['admin_dir'].'/');

define('COPY_YEAR', my_date('Y', TIME_NOW));

require_once DADDYOBB_ADMIN_DIR."inc/class_page.php";
require_once DADDYOBB_ADMIN_DIR."inc/class_form.php";
require_once DADDYOBB_ADMIN_DIR."inc/class_table.php";
require_once DADDYOBB_ADMIN_DIR."inc/functions.php";
require_once DADDYOBB_ROOT."inc/functions_user.php";

if(!file_exists(DADDYOBB_ROOT."inc/languages/".$daddyobb->settings['cplanguage']."/admin/home_dashboard.lang.php"))
{
	$daddyobb->settings['cplanguage'] = "english";
}
$lang->set_language($daddyobb->settings['cplanguage'], "admin");
...
if($daddyobb->input['action'] == "logout")
{
	...
	$db->delete_query("adminsessions", "sid='".$db->escape_string($daddyobb->cookies['adminsid'])."'");
	my_setcookie("adminsid", "");
	$logged_out = true;
}
elseif($daddyobb->input['do'] == "login")
{
	...
	{
		$db->delete_query("adminsessions", "uid='{$daddyobb->user['uid']}'");
		...
		$admin_session = array(
			...
			"loginkey" => $daddyobb->user['loginkey'],
...
else
{
	...
	if(!isset($daddyobb->cookies['adminsid']))
	{
		$login_message = "";
	}
	...
	{
		$query = $db->simple_select("adminsessions", "*", "sid='".$db->escape_string($daddyobb->cookies['adminsid'])."'");
		...
		else
		{
			...
			if(!$daddyobb->user['uid'] || $daddyobb->user['loginkey'] != $admin_session['loginkey'])
			{
				unset($daddyobb->user);
			}
			else
...
if(!$daddyobb->user['usergroup'])
{
	$daddyobbgroups = 1;
}
else
{
	$daddyobbgroups = $daddyobb->user['usergroup'].",".$daddyobb->user['additionalgroups'];
}
$daddyobb->usergroup = usergroup_permissions($daddyobbgroups);

if($daddyobb->usergroup['cancp'] != 1 || !$daddyobb->user['uid'])
{
	unset($daddyobb->user);
}
...
if($daddyobb->user['uid'])
{
	$query = $db->simple_select("adminoptions", "*", "uid='".$daddyobb->user['uid']."'");
	$admin_options = $db->fetch_array($query);
	
	if(!empty($admin_options['cpstyle']) && file_exists(DADDYOBB_ADMIN_DIR."/styles/{$admin_options['cpstyle']}/main.css"))
	{
	...
	$daddyobb->admin['permissions'] = get_admin_permissions($daddyobb->user['uid']);
}

// Load Admin CP style
if(!$cp_style)
{
	if(!empty($daddyobb->settings['cpstyle']) && file_exists(DADDYOBB_ADMIN_DIR."/styles/".$daddyobb->settings['cpstyle']."/main.css"))
	{
		$cp_style = $daddyobb->settings['cpstyle'];
	}
	else
	{
		$cp_style = "default";
...
if(file_exists(DADDYOBB_ADMIN_DIR."/styles/{$cp_style}/style.php"))
{
	require_once DADDYOBB_ADMIN_DIR."/styles/{$cp_style}/style.php";
}
...
if(!$daddyobb->user['uid'] || $logged_out == true)
{
	if($logged_out == true)
	{
...
$modules_dir = DADDYOBB_ADMIN_DIR."modules";
...
while(($module = readdir($dir)) !== false)
{
	if(is_dir($modules_dir."/".$module) && !in_array($module, array(".", "..")) && file_exists($modules_dir."/".$module."/module_meta.php"))
	{
		...
		{
			if(isset($daddyobb->admin['permissions'][$module]))
			{
				$has_permission = true;
			}
		}
...
$current_module = explode("/", $daddyobb->input['module'], 2);
if($daddyobb->input['module'] && $modules[$current_module[0]])
{
...
{
	if(in_array($daddyobb->input['module'], $post_check_ignores))
	{
		$k = array_search($daddyobb->input['module'], $post_check_ignores);
		if(in_array($daddyobb->input['action'], $post_check_ignores[$k]))
		{
			$post_verify = false;
		}
	}
	...
	if($post_verify == true)
	{
		...
		if(!isset($daddyobb->input['my_post_key']) || $daddyobb->post_code != $daddyobb->input['my_post_key'])
		{
			$daddyobb->request_method = "get";

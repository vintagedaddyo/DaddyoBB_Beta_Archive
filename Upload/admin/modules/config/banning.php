...
if($daddyobb->input['action'] == "add" && $daddyobb->request_method == "post")
{
	...
	{
		$new_filter = array(
			"filter" => $db->escape_string($daddyobb->input['filter']),
		...
		// Log admin action
		log_admin_action($fid, $daddyobb->input['filter'], $daddyobb->input['type']);
	...
	else
	{
		if($daddyobb->input['type'] == 1)
		{
			$daddyobb->input['type'] = "ips";
		}
		else if($daddyobb->input['type'] == 2)
		{
			$daddyobb->input['type'] = "usernames";
		}
		else if($daddyobb->input['type'] == 3)
		{
			$daddyobb->input['type'] = "emails";
		}
...
if($daddyobb->input['action'] == "delete")
{
	...
	$query = $db->simple_select("banfilters", "*", "fid='".intval($daddyobb->input['fid'])."'");
...
if(!$daddyobb->input['action'])
{
	...
	$page->output_nav_tabs($sub_tabs, $daddyobb->input['type']);

	$table = new Table;
	if($daddyobb->input['type'] == "usernames")
	{
	...
	else if($daddyobb->input['type'] == "emails")
	{
	...
	while($filter = $db->fetch_array($query))
	{
		...
		if($filter['lastuse'] > 0)
		{
			$last_use = my_date($daddyobb->settings['dateformat'], $filter['lastuse']).", ".my_date($daddyobb->settings['timeformat'], $filter['lastuse']);
		}
		else
		{
			$last_use = $lang->never;
		}
		$date = my_date($daddyobb->settings['dateformat'], $filter['dateline']).", ".my_date($daddyobb->settings['timeformat'], $filter['dateline']);
	...
	if($daddyobb->input['type'] == "usernames")
	{
		...
		$form_container->output_row($lang->username." <em>*</em>", $lang->username_desc, $form->generate_text_box('filter', $daddyobb->input['filter'], array('id' => 'filter')), 'filter');
	...
	else if($daddyobb->input['type'] == "emails")
	{
		...
		$form_container->output_row($lang->email_address." <em>*</em>", $lang->email_address_desc, $form->generate_text_box('filter', $daddyobb->input['filter'], array('id' => 'filter')), 'filter');
		$buttons[] = $form->generate_submit_button($lang->disallow_email_address);
	}
	else
	{
		$form_container = new FormContainer($lang->ban_ip_address);
		$form_container->output_row($lang->ip_address." <em>*</em>", $lang->ip_address_desc, $form->generate_text_box('filter', $daddyobb->input['filter'], array('id' => 'filter')), 'filter');

...
if($daddyobb->input['action'] == "add")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!trim($daddyobb->input['find']))
		{
			$errors[] = $lang->error_missing_text_replacement;
		}

		if(!trim($daddyobb->input['image']))
		{
			$errors[] = $lang->error_missing_path;
		}

		if(!trim($daddyobb->input['disporder']))
		{
		...
		if(!$errors)
		{
			$new_smilie = array(
				...
				"find" => $db->escape_string($daddyobb->input['find']),
				"image" => $db->escape_string($daddyobb->input['image']),
				"disporder" => intval($daddyobb->input['disporder']),
				"showclickable" => $db->escape_string($daddyobb->input['showclickable'])
			);
	...
	{
		$daddyobb->input['image'] = 'images/smilies/';
		$daddyobb->input['showclickable'] = 1;
	}
	
	if(!$daddyobb->input['disporder'])
	{
		$query = $db->simple_select("smilies", "max(disporder) as dispordermax");
		$daddyobb->input['disporder'] = $db->fetch_field($query, "dispordermax")+1;
	}
	...
	$form_container->output_row($lang->text_replace." <em>*</em>", "", $form->generate_text_box('find', $daddyobb->input['find'], array('id' => 'find')), 'find');
	$form_container->output_row($lang->image_path." <em>*</em>", $lang->image_path_desc, $form->generate_text_box('image', $daddyobb->input['image'], array('id' => 'image')), 'image');
	...
	$form_container->output_row($lang->show_clickable." <em>*</em>", $lang->show_clickable_desc, $form->generate_yes_no_radio('showclickable', $daddyobb->input['showclickable']));
	$form_container->end();
...
if($daddyobb->input['action'] == "edit")
{
	...
	$query = $db->simple_select("smilies", "*", "sid='".intval($daddyobb->input['sid'])."'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$db->update_query("smilies", $updated_smilie, "sid = '".intval($daddyobb->input['sid'])."'");
			...
			// Log admin action
			log_admin_action($smilie['sid'], $daddyobb->input['name']);
...
if($daddyobb->input['action'] == "add_multiple")
{
	...
	$form_container = new FormContainer($lang->add_multiple_smilies);
	$form_container->output_row($lang->path_to_images, $lang->path_to_images_desc, $form->generate_text_box('pathfolder', $daddyobb->input['pathfolder'], array('id' => 'pathfolder')), 'pathfolder');
	$form_container->end();
...
if($daddyobb->input['action'] == "mass_edit")
{
	$plugins->run_hooks("admin_config_smilies_mass_edit");
	
	if($daddyobb->request_method == "post")
	{
		foreach($daddyobb->input['name'] as $sid => $name)
		{
			if($daddyobb->input['delete'][$sid] == 1)
			{
			...
			{
				$smilie = array(
					"name" => $db->escape_string($daddyobb->input['name'][$sid]),
					"find" => $db->escape_string($daddyobb->input['find'][$sid]),
					"disporder" => intval($daddyobb->input['disporder'][$sid]),
					"showclickable" => $db->escape_string($daddyobb->input['showclickable'][$sid])
				);
	...
	{
		$daddyobb->input['path'] = 'images/smilies/';
		$daddyobb->input['showclickable'] = 1;
	}
	...
	while($smilie = $db->fetch_array($query))
	{
		...
		$form_container->output_cell($form->generate_check_box("delete[{$smilie['sid']}]", 1, $daddyobb->input['delete']), array("class" => "align_center"));
		$form_container->construct_row();
	}
...
if(!$daddyobb->input['action'])
{
	...
	while($smilie = $db->fetch_array($query))
	{
		...
		$table->construct_cell("<a href=\"index.php?module=config/smilies&amp;action=delete&amp;sid={$smilie['sid']}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_smilie_deletion}')\">{$lang->delete}</a>", array("class" => "align_center"));
		$table->construct_row();
	}

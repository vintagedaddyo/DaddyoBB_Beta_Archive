...
if($daddyobb->input['action'] == "check")
{	
	...
	if($plugins_list)
	{
		...
		{
			require_once DADDYOBB_ROOT."inc/plugins/".$plugin_file;
...
if($daddyobb->input['action'] == "activate" || $daddyobb->input['action'] == "deactivate")
{
	...
	require_once DADDYOBB_ROOT."inc/plugins/$file";
	...
	if($daddyobb->input['action'] == "activate")
	{
		...
		{
			flash_message($lang->sprintf($lang->plugin_incompatible, $daddyobb->version_code), 'error');
			admin_redirect("index.php?module=config/plugins");
		}
	...
	else if($daddyobb->input['action'] == "deactivate")
	{
		...
		if($daddyobb->input['uninstall'] == 1 && function_exists("{$codename}_uninstall"))
		{
...
if(!$daddyobb->input['action'])
{
	...
	$table = new Table;
	$table->construct_header($lang->plugin);
	$table->construct_header($lang->controls, array("colspan" => 2, "class" => "align_center", "width" => 500));
	...
	if(!empty($plugins_list))
	{
		foreach($plugins_list as $plugin_file)
		{
			...
			if($plugins->is_compatible($codename) == false)
			{
				$compatibility_warning = "<span style=\"color: red;\">".$lang->sprintf($lang->plugin_incompatible, $daddyobb->version)."</span>";
			}
			else
			{
			...
			else if($active_plugins[$codename])
			{
				...
				{
					$table->construct_cell("&nbsp;", array("class" => "align_center", "width" => 250));
				}
			}
...
function get_plugins_list()
{
	...
	$dir = @opendir(DADDYOBB_ROOT."inc/plugins/");
	if($dir)
	{

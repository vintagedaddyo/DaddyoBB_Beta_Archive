...
if($daddyobb->input['action'] == "delete_duplicates")
{
	$query = $db->query("
		DELETE s1
...
if($daddyobb->input['action'] == "addgroup")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		$query = $db->simple_select("settinggroups", "title", "name='".$db->escape_string($daddyobb->input['name'])."'");
		...
		if(!$errors)
		{
			...
			// Log admin action
			log_admin_action($gid, $daddyobb->input['name']);
	...
	$form_container->output_row($lang->description, "", $form->generate_text_area('description', $daddyobb->input['description'], array('id' => 'description')), 'description');
	...
	$form_container->output_row($lang->name." <em>*</em>", $lang->group_name_desc, $form->generate_text_box('name', $daddyobb->input['name'], array('id' => 'name')), 'name');
	$form_container->end();
...
// Edit setting group
if($daddyobb->input['action'] == "editgroup")
{
	$plugins->run_hooks("admin_config_settings_editgroup");
	
	$query = $db->simple_select("settinggroups", "*", "gid='".intval($daddyobb->input['gid'])."'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		$query = $db->simple_select("settinggroups", "title", "name='".$db->escape_string($daddyobb->input['name'])."' AND gid != '{$group['gid']}'");
		if($db->num_rows($query) > 0)
		{
			$dup_group_title = $db->fetch_field($query, 'title');
			$errors[] = $lang->sprintf($lang->error_duplicate_group_name, $dup_group_title);
		...
		if(!$errors)
		{
			...
			// Log admin action
			log_admin_action($group['gid'], $daddyobb->input['name']);
...
if($daddyobb->input['action'] == "deletegroup")
{
...
if($daddyobb->input['action'] == "add")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		$query = $db->simple_select("settinggroups", "gid", "gid='".intval($daddyobb->input['gid'])."'");
		...
		$query = $db->simple_select("settings", "title", "name='".$db->escape_string($daddyobb->input['name'])."'");
		if($db->num_rows($query) > 0)
		{
		...
		$daddyobb->input['type'] = str_replace("\n", "", $daddyobb->input['type']);
		if(strtolower(substr($daddyobb->input['type'], 0, 3)) == "php")
		{
			$daddyobb->input['type'] = "";
		}
		...
		if(!$errors)
		{
			...
			{
				$options_code = $daddyobb->input['extra'];
			}
			else if($daddyobb->input['extra'])
			{
				$options_code = "{$daddyobb->input['type']}\n{$daddyobb->input['extra']}";
			}
			else
			{
				$options_code = $daddyobb->input['type'];
			...
			$new_setting = array(
				...
				"value" => $db->escape_string($daddyobb->input['value']),
				"disporder" => intval($daddyobb->input['disporder']),
	...
	$form_container->output_row($lang->group." <em>*</em>", "", $form->generate_select_box("gid", $options, $daddyobb->input['gid'], array('id' => 'gid')), 'gid');
	$form_container->output_row($lang->display_order, "", $form->generate_text_box('disporder', $daddyobb->input['disporder'], array('id' => 'disporder')), 'disporder');
	...
	$form_container->output_row($lang->type." <em>*</em>", "", $form->generate_select_box("type", $setting_types, $daddyobb->input['type'], array('id' => 'type')), 'type');
	$form_container->output_row($lang->extra, $lang->extra_desc, $form->generate_text_area('extra', $daddyobb->input['extra'], array('id' => 'extra')), 'extra', array(), array('id' => 'row_extra'));
	$form_container->output_row($lang->value, "", $form->generate_text_area('value', $daddyobb->input['value'], array('id' => 'value')), 'value');
	$form_container->end();
...
if($daddyobb->input['action'] == "edit")
{
	...
	$query = $db->simple_select("settings", "*", "sid='".intval($daddyobb->input['sid'])."'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		$query = $db->simple_select("settings", "title", "name='".$db->escape_string($daddyobb->input['name'])."' AND sid != '{$setting['sid']}'");
		if($db->num_rows($query) > 0)
		{
			$dup_setting_title = $db->fetch_field($query, 'title');
			$errors[] = $lang->sprintf($lang->error_duplicate_name, $dup_setting_title);
		...
		if(!$errors)
		{
			...
			$db->update_query("settings", $updated_setting, "sid='{$daddyobb->input['sid']}'");
			rebuild_settings();
			
			$plugins->run_hooks("admin_config_settings_edit_commit");

			// Log admin action
			log_admin_action($setting['sid'], $daddyobb->input['title']);
...
if($daddyobb->input['action'] == "manage")
{
	...
	if($daddyobb->request_method == "post")
	{
		if(is_array($daddyobb->input['group_disporder']))
		{
			foreach($daddyobb->input['group_disporder'] as $gid => $new_order)
			{
				$gid = intval($gid);
		...
		if(is_array($daddyobb->input['setting_disporder']))
		{
			foreach($daddyobb->input['setting_disporder'] as $sid => $new_order)
			{
				$sid = intval($sid);
	...
	while($group = $db->fetch_array($query))
	{
		...
		if($group['isdefault'] != 1)
		{
			...
			$popup->add_item($lang->delete_setting_group, "index.php?module=config/settings&amp;action=deletegroup&amp;gid={$group['gid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_setting_group_deletion}')");
		...
		if(is_array($settings_cache[$group['gid']]))
		{
			foreach($settings_cache[$group['gid']] as $setting)
			{
				...
				if($group['isdefault'] != 1 || $setting['isdefault'] != 1)
				{
					...
					$popup->add_item($lang->delete_setting, "index.php?module=config/settings&amp;action=delete&amp;sid={$setting['sid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_setting_deletion}')");
					$table->construct_cell($popup->fetch(), array('class' => 'align_center'));
				}
				else
				{
...
if($daddyobb->input['action'] == "change")
{
	$plugins->run_hooks("admin_config_settings_change");
	
	if($daddyobb->request_method == "post")
	{
		if(is_array($daddyobb->input['upsetting']))
		{
			foreach($daddyobb->input['upsetting'] as $name => $value)
			{
		...
		if($daddyobb->settings['searchtype'] != $daddyobb->input['upsetting']['searchtype'] && $daddyobb->input['upsetting']['searchtype'] == "fulltext")
		{
		...
		if($daddyobb->input['upsetting']['delayedthreadviews'] && $daddyobb->settings['delayedthreadviews'] != $daddyobb->input['upsetting']['delayedthreadviews'])
		{
			if($daddyobb->input['upsetting']['delayedthreadviews'] == 0)
			{
				$updated_task = array(
					"enabled" => 0
				);
		...
		if($daddyobb->input['upsetting']['cookieprefix'] && $daddyobb->input['upsetting']['cookieprefix'] != $daddyobb->settings['cookieprefix'])
		{
			my_unsetcookie("adminsid");
			$daddyobb->settings['cookieprefix'] = $daddyobb->input['upsetting']['cookieprefix'];
	...
	if($daddyobb->input['search'])
	{
		// Search
		
		// Search for settings
		$search = $db->escape_string($daddyobb->input['search']);
		$query = $db->query("
			SELECT s.* 
		...
		{
			if(isset($daddyobb->input['ajax_search']))
			{
	...
	elseif($daddyobb->input['gid'])
	{
		...
		// Cache settings
		$query = $db->simple_select("settings", "*", "gid='".intval($daddyobb->input['gid'])."'", array('order_by' => 'disporder'));
		while($setting = $db->fetch_array($query))
		{
			$cache_settings[$setting['gid']][$setting['sid']] = $setting;
		}
	...
	foreach($cache_groups as $groupinfo)
	{
		...
		foreach($cache_settings[$groupinfo['gid']] as $setting)
		{
			...
			{
				$dir = @opendir(DADDYOBB_ROOT.$config['admin_dir']."/styles");
				while($folder = readdir($dir))
				{
					if($file != "." && $file != ".." && @file_exists(DADDYOBB_ROOT.$config['admin_dir']."/styles/$folder/main.css"))
					{

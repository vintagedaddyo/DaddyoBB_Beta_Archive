...
if($daddyobb->input['action'] == "utf8_conversion")
{
	...
	if($daddyobb->request_method == "post" || ($daddyobb->input['do'] == "all" && !empty($daddyobb->input['table'])))
	{
		@set_time_limit(0);
		...
		if(!$db->table_exists($db->escape_string($daddyobb->input['table'])))
		{
		...
		$table1 = $db->show_create_table($db->escape_string($daddyobb->input['table']));
        preg_match("#CHARSET=([a-zA-Z0-9_]+)\s?#i", $table1, $matches);
		$charset = $matches[1];
		
		$table->construct_cell("<strong>".$lang->sprintf($lang->converting_to_utf8, $daddyobb->input['table'], $charset)."</strong>");
		$table->construct_row();
		
		$table->construct_cell($lang->please_wait);
		$table->construct_row();
		
		$table->output($converting_table." {$daddyobb->input['table']}");
		...
		$db->write_query("ALTER TABLE {$daddyobb->input['table']} DEFAULT CHARACTER SET utf8 COLLATE utf8_general_ci");

		// Fetch any fulltext keys
		if($db->supports_fulltext($daddyobb->input['table']))
		{
			$table_structure = $db->show_create_table($daddyobb->input['table']);
			switch($db->type)
			{
				case "mysql":
				case "mysqli":
					...
					if(is_array($matches))
					{
						...
						{
							$db->write_query("ALTER TABLE {$daddyobb->input['table']} DROP INDEX {$matches[1][$key]}");
		...
		$query = $db->query("SHOW FULL COLUMNS FROM {$daddyobb->input['table']}");
		...
		if(!empty($convert_to_binary))
		{
			...
			$db->write_query("ALTER TABLE {$daddyobb->input['table']} {$convert_to_binary}");
			$db->write_query("ALTER TABLE {$daddyobb->input['table']} {$convert_to_utf8}");
		}
		...
		if(is_array($fulltext_to_create))
		{
			...
			{
				$db->create_fulltext_index($daddyobb->input['table'], $fields, $name);
			}
		}
		...
		// Log admin action
		log_admin_action($daddyobb->input['table']);
		
		flash_message($lang->sprintf($lang->success_table_converted, $daddyobb->input['table']), 'success');
		
		if($daddyobb->input['do'] == "all")
		{
			$old_table_prefix = $db->table_prefix;
			$db->set_table_prefix('');
		
			$tables = $db->list_tables($daddyobb->config['database']['database']);
			...
			{		
				if(substr($tablename, 0, strlen(TABLE_PREFIX)) == TABLE_PREFIX)
				{
					...
					if(fetch_iconv_encoding($matches[1]) == 'utf-8' && $daddyobb->input['table'] != $tablename)
					{
						continue;
					}
					
					$daddyobb_tables[$key] = $tablename;		
				}
			}
			
			asort($daddyobb_tables);
			reset($daddyobb_tables);
			
			$is_next = false;
			$nexttable = "";
			
			foreach($daddyobb_tables as $key => $tablename)
			{
				if($is_next == true)
				{
					$nexttable = $tablename;
				...
				else if($daddyobb->input['table'] == $tablename)
				{
					$is_next = true;
				}
			}
	...
	if($daddyobb->input['table'] || $daddyobb->input['do'] == "all")
	{
		$old_table_prefix = $db->table_prefix;
		$db->set_table_prefix('');
		
		if($daddyobb->input['do'] != "all" && !$db->table_exists($db->escape_string($daddyobb->input['table'])))
		{
			$db->set_table_prefix($old_table_prefix);
			flash_message($lang->error_invalid_table, 'error');
			admin_redirect("index.php?module=tools/system_health&action=utf8_conversion");
		...
		if($daddyobb->input['do'] == "all")
		{
			...
			if(is_array($daddyobb_tables))
			{
				asort($daddyobb_tables);
				reset($daddyobb_tables);
				$nexttable = current($daddyobb_tables);
				$table = $db->show_create_table($db->escape_string($nexttable));
				$daddyobb->input['table'] = $nexttable;
			}
			else
			{
		...
		{
			$table = $db->show_create_table($db->escape_string($daddyobb->input['table']));
		}
		...
		echo $form->generate_hidden_field("table", $daddyobb->input['table']);
		...
		{
			$table->construct_cell("<strong>".$lang->sprintf($lang->convert_to_utf8, $daddyobb->input['table'], $charset)."</strong>");
		}
		...
		{
			$table->output($lang->convert_table.": {$daddyobb->input['table']}");
	...
	if($okay_count == count($daddyobb_tables))
	{
		flash_message($lang->success_all_tables_already_converted, 'success');
		admin_redirect("index.php?module=tools/system_health");
	}
	
	if(!$daddyobb->config['database']['encoding'])
	{
...
if(!$daddyobb->input['action'])
{
	...
	if(is_writable(DADDYOBB_ROOT.'inc/config.php'))
	{
	...
	if(is_writable('.'.$daddyobb->settings['uploadspath']))
	{
	...
	if(is_writable('../'.$daddyobb->settings['avataruploadpath']))
	{
	...
	if(is_writable(DADDYOBB_ROOT.'inc/languages/'))
	{
	...
	if(is_writable(DADDYOBB_ROOT.$config['admin_dir'].'/backups/'))
	{
	...
	if(is_writable(DADDYOBB_ROOT.'/cache/themes/'))
	{
	...
	if($errors)
	{
		$page->output_error("<p><em>{$errors} {$lang->error_chmod}</span></strong> {$lang->chmod_info} <a href=\"http://wiki.mybboard.net/index.php/HowTo_Chmod\" target=\"_blank\">DaddyoBB Wiki</a>.</em></p>");
	}
	else
	{
	...
	$table->construct_cell($daddyobb->settings['uploadspath']);
	$table->construct_cell($message_upload);
	$table->construct_row();
	
	$table->construct_cell("<strong>{$lang->avatar_upload_dir}</strong>");
	$table->construct_cell($daddyobb->settings['avataruploadpath']);

...
if($daddyobb->input['action'] == "view")
{
	...
	if(is_array($warn_errors))
	{
		...
		$daddyobb->input['reason'] = htmlspecialchars_uni($daddyobb->input['reason']);
	}
	...
	if(!$warning['daterevoked'])
	{
		...
		$form_container->output_row("", $lang->revoke_warning_desc, $form->generate_text_area('reason', $daddyobb->input['reason'], array('id' => 'reason')), 'reason');	
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->input['filter']['uid'])
	{
		...
		if(!isset($daddyobb->input['search']['username']))
		{
			...
			$daddyobb->input['search']['username'] = $user['username'];
		}
	}
	...
	if($daddyobb->input['filter']['mod_uid'])
	{
		...
		if(!isset($daddyobb->input['search']['mod_username']))
		{
			...
			$daddyobb->input['search']['mod_username'] = $mod_user['username'];
		}
	}
	...
	{
		$view_page = intval($daddyobb->input['page']);
	}
	$per_page = 20;
	...
	$form_container->output_row($lang->filter_warned_user, "", $form->generate_text_box('filter[username]', $daddyobb->input['filter']['username'], array('id' => 'filter_username')), 'filter_username');	
	$form_container->output_row($lang->filter_issued_by, "", $form->generate_text_box('filter[mod_username]', $daddyobb->input['filter']['mod_username'], array('id' => 'filter_mod_username')), 'filter_mod_username');
	$form_container->output_row($lang->filter_reason, "", $form->generate_text_box('filter[reason]', $daddyobb->input['filter']['reason'], array('id' => 'filter_reason')), 'filter_reason');
	$form_container->output_row($lang->sort_by, "", $form->generate_select_box('filter[sortby]', $sort_by, $daddyobb->input['filter']['sortby'], array('id' => 'filter_sortby'))." {$lang->in} ".$form->generate_select_box('filter[order]', $order_array, $order, array('id' => 'filter_order'))." {$lang->order}", 'filter_order');	

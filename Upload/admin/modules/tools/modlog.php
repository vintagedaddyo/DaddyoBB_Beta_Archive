...
if($daddyobb->input['action'] == 'prune')
{
	...
	if($daddyobb->request_method == 'post')
	{
		...
		if($daddyobb->input['fid'] > 0)
		{
			$where .= " AND fid='".$db->escape_string($daddyobb->input['fid'])."'";
		}
		...
		// Log admin action
		log_admin_action($daddyobb->input['older_than'], $daddyobb->input['uid'], $daddyobb->input['fid'], $num_deleted, $forum_cache[$daddyobb->input['fid']]['name']);
	...
	$form_container->output_row($lang->forum, "", $form->generate_forum_select('fid', $daddyobb->input['fid'], array('id' => 'fid', 'main_option' => $lang->all_forums)), 'fid');	
	$form_container->output_row($lang->forum_moderator, "", $form->generate_select_box('uid', $user_options, $daddyobb->input['uid'], array('id' => 'uid')), 'uid');
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->input['fid'] > 0)
	{
		$where .= " AND l.fid='".intval($daddyobb->input['fid'])."'";
	}
	...
	if($rescount > $perpage)
	{
		echo draw_admin_pagination($pagecnt, $perpage, $rescount, "index.php?module=tools/modlog&amp;perpage=$perpage&amp;uid={$daddyobb->input['uid']}&amp;fid={$daddyobb->input['fid']}&amp;sortby={$daddyobb->input['sortby']}&amp;order={$order}")."<br />";
	}

...
if($daddyobb->input['action'] == "view")
{
	...
	$query = $db->simple_select("maillogs", "*", "mid='".intval($daddyobb->input['mid'])."'");
	...
	if($daddyobb->settings['mail_logging'] == 1)
	{
		$log['message'] = $lang->na;
	}
	else
...
if(!$daddyobb->input['action'])
{
	...
	{
		if($daddyobb->input['from_type'] == "user")
		{
			$daddyobb->input['fromname'] = $daddyobb->input['from_value'];
		}
		else if($daddyobb->input['from_type'] == "email")
		{
			$daddyobb->input['fromemail'] = $daddyobb->input['from_value'];
		}

		if($daddyobb->input['to_type'] == "user")
		{
			$daddyobb->input['toname'] = $daddyobb->input['to_value'];
		}
		else if($daddyobb->input['to_type'] == "email")
		{
			$daddyobb->input['toemail'] = $daddyobb->input['to_value'];
		}
	}
	...
	{
		$additional_sql_criteria .= " AND l.subject LIKE '%".$db->escape_string($daddyobb->input['subject'])."%'";
		$additional_criteria[] = "subject='".htmlspecialchars_uni($daddyobb->input['subject'])."'";
	}

	if($daddyobb->input['fromuid'])
	{
		$query = $db->simple_select("users", "uid, username", "uid='".intval($daddyobb->input['fromuid'])."'");
		$user = $db->fetch_array($query);
		$from_filter = $user['username'];
		$additional_sql_criteria .= " AND l.fromuid='".intval($daddyobb->input['fromuid'])."'";
		$additional_criteria[] = "fromuid='".intval($daddyobb->input['fromuid'])."'";
	}
	else if($daddyobb->input['fromname'])
	{
		$query = $db->simple_select("users", "uid, username", "LOWER(username)='".my_strtolower($daddyobb->input['fromname'])."'");
		$user = $db->fetch_array($query);
		$from_filter = $user['username'];
	...
	if($daddyobb->input['fromemail'])
	{
		$additional_sql_criteria .= " AND l.fromemail LIKE '%".$db->escape_string($daddyobb->input['fromemail'])."%'";
		$additional_criteria[] = "fromemail=".urlencode($daddyobb->input['fromemail']);
		$from_filter = $daddyobb->input['fromemail'];
	}

	if($daddyobb->input['touid'])
	{
		$query = $db->simple_select("users", "uid, username", "uid='".intval($daddyobb->input['touid'])."'");
		$user = $db->fetch_array($query);
		$to_filter = $user['username'];
		$additional_sql_criteria .= " AND l.touid='".intval($daddyobb->input['touid'])."'";
		$additional_criteria[] = "touid='".intval($daddyobb->input['touid'])."'";
	}
	else if($daddyobb->input['toname'])
	{
		$query = $db->simple_select("users", "uid, username", "LOWER(username)='".my_strtolower($daddyobb->input['toname'])."'");
		$user = $db->fetch_array($query);
		$to_filter = $user['username'];
	...
	if($daddyobb->input['toemail'])
	{
		$additional_sql_criteria .= " AND l.toemail LIKE '%".$db->escape_string($daddyobb->input['toemail'])."%'";
		$additional_criteria[] = "toemail='".urlencode($daddyobb->input['toemail'])."'";
		$to_filter = $daddyobb->input['toemail'];
	}
	...
	while($log = $db->fetch_array($query))
	{
		...
		if($log['tid'] > 0)
		{
			...
			$table->construct_cell("<a href=\"javascript:DaddyoBB.popupWindow('index.php?module=tools/maillogs&amp;action=view&amp;mid={$log['mid']}', 'log_entry', 450, 450);\">{$log['subject']}</a><br /><small>{$lang->thread} {$thread_link}</small>");
		...
		else
		{
			...
			$table->construct_cell("<a href=\"javascript:DaddyoBB.popupWindow('index.php?module=tools/maillogs&amp;action=view&amp;mid={$log['mid']}', 'log_entry', 450, 450);\">{$log['subject']}</a>");
			$find_from = "<div class=\"float_right\"><a href=\"index.php?module=tools/maillogs&amp;fromuid={$log['fromuid']}\"><img src=\"styles/{$page->style}/images/icons/find.gif\" title=\"{$lang->find_emails_by_user}\" alt=\"{$lang->find}\" /></a></div>";
			if(!$log['from_username'])
			{
				$table->construct_cell("{$find_from}<div>{$lang->deleted_user}</div>");
	...
	echo "<br />".draw_admin_pagination($daddyobb->input['page'], $per_page, $total_rows, "index.php?module=tools/maillogs&amp;page={page}{$additional_criteria}");
	...
	else if($daddyobb->input['fromemail'])
	{
		$from_type = "email";
	}
	...
	else if($daddyobb->input['toemail'])
	{
		$to_type = "email";
	}

...
if($daddyobb->input['action'] == 'prune')
{
	$plugins->run_hooks("admin_tools_adminlog_prune");
	
	if(!is_super_admin($daddyobb->user['uid']))
	{
	...
	{
		$where = 'dateline < '.(TIME_NOW-(intval($daddyobb->input['older_than'])*86400));
		
		// Searching for entries by a particular user
		if($daddyobb->input['uid'])
		{
			$where .= " AND uid='".intval($daddyobb->input['uid'])."'";
		}
		
		// Searching for entries in a specific module
		if($daddyobb->input['filter_module'])
		{
			$where .= " AND module='".$db->escape_string($daddyobb->input['filter_module'])."'";
		}
		...
		// Log admin action
		log_admin_action($daddyobb->input['older_than'], $daddyobb->input['uid'], $daddyobb->input['filter_module'], $num_deleted);
	...
	$form_container->output_row($lang->module, "", $form->generate_select_box('filter_module', $module_options, $daddyobb->input['filter_module'], array('id' => 'filter_module')), 'filter_module');	
	$form_container->output_row($lang->administrator, "", $form->generate_select_box('uid', $user_options, $daddyobb->input['uid'], array('id' => 'uid')), 'uid');
	if(!$daddyobb->input['older_than'])
	{
		$daddyobb->input['older_than'] = '30';
	}
	$form_container->output_row($lang->date_range, "", $lang->older_than.$form->generate_text_box('older_than', $daddyobb->input['older_than'], array('id' => 'older_than', 'style' => 'width: 30px'))." {$lang->days}", 'older_than');
	$form_container->end();
...
if(!$daddyobb->input['action'])
{
	...
	{
		$pagecnt = intval($daddyobb->input['page']);
	}
	...
	if($rescount > $perpage)
	{
		echo draw_admin_pagination($pagecnt, $perpage, $rescount, "index.php?module=tools/adminlog&amp;perpage=$perpage&amp;uid={$daddyobb->input['uid']}&amp;fid={$daddyobb->input['fid']}&amp;sortby={$daddyobb->input['sortby']}&amp;order={$order}")."<br />";
	}
	...
	$form_container->output_row($lang->module, "", $form->generate_select_box('filter_module', $module_options, $daddyobb->input['filter_module'], array('id' => 'filter_module')), 'filter_module');	
	$form_container->output_row($lang->administrator, "", $form->generate_select_box('uid', $user_options, $daddyobb->input['uid'], array('id' => 'uid')), 'uid');	
	$form_container->output_row($lang->sort_by, "", $form->generate_select_box('sortby', $sort_by, $daddyobb->input['sortby'], array('id' => 'sortby'))." {$lang->in} ".$form->generate_select_box('order', $order_array, $order, array('id' => 'order'))." {$lang->order}", 'order');	
 ...
 * @param array The log item (one row from DADDYOBB_adminlogs)
 * @return string The description
 */
function get_admin_log_action($logitem)
{
	global $lang, $plugins, $daddyobb;
	...
	switch($lang_string)
	{
		...
		case 'admin_log_user_banning_': // banning
			...
			{
				$logitem['data'][2] = my_date($daddyobb->settings['dateformat'], $logitem['data'][2]);

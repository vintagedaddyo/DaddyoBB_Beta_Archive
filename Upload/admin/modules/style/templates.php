...
$expand_str2 = "";
$expand_array = array();
if(isset($daddyobb->input['expand']))
{
	$expand_array = explode("|", $daddyobb->input['expand']);
	$expand_array = array_map("intval", $expand_array);
	$expand_str = "&amp;expand=".implode("|", $expand_array);
	$expand_str2 = "&expand=".implode("|", $expand_array);
}

if($daddyobb->input['action'] == "add_set" || $daddyobb->input['action'] == "add_template" || $daddyobb->input['action'] == "search_replace" || $daddyobb->input['action'] == "find_updated" || (!$daddyobb->input['action'] && !$sid))
{
...
else if(($sid && !$daddyobb->input['action']) || $daddyobb->input['action'] == "edit_set" || $daddyobb->input['action'] == "edit_template")
{
...
if($daddyobb->input['action'] == "add_set")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			$sid = $db->insert_query("templatesets", array('title' => $db->escape_string($daddyobb->input['title'])));
			
			// Log admin action
			log_admin_action($sid, $daddyobb->input['title']);
	...
	$form_container->output_row($lang->title, "", $form->generate_text_box('title', $daddyobb->input['title'], array('id' => 'title')), 'title');
	$form_container->end();
...
if($daddyobb->input['action'] == "add_template")
{
	$plugins->run_hooks("admin_style_templates_add_template");
	
	if($daddyobb->request_method == "post")
	{
		if(empty($daddyobb->input['title']))
		{
			$errors[] = $lang->error_missing_set_title;
		}
		else
		{	
			$query = $db->simple_select("templates", "COUNT(tid) as count", "title='".$db->escape_string($daddyobb->input['title'])."' AND (sid = '-2' OR sid = '{$sid}')");
		...
		if(!$errors)
		{
			$template_array = array(
				...
				'sid' => $sid,
				'template' => $db->escape_string($daddyobb->input['template']),
				'version' => $db->escape_string($daddyobb->version_code),
				'status' => '',
				'dateline' => TIME_NOW
			);
			...
			// Log admin action
			log_admin_action($tid, $daddyobb->input['title'], $sid, $template_sets[$sid]);
			
			flash_message($lang->success_template_saved, 'success');
			
			if($daddyobb->input['continue'])
			{
				admin_redirect("index.php?module=style/templates&action=edit_template&title=".urlencode($daddyobb->input['title'])."&sid=".$sid.$expand_str2);
			}
			else
			{
	...
	{
		$page->extra_header .= '
	<link type="text/css" href="./jscripts/codepress/languages/codepress-daddyobb.css" rel="stylesheet" id="cp-lang-style" />
	<script type="text/javascript" src="./jscripts/codepress/codepress.js"></script>
	<script type="text/javascript">
		CodePress.language = \'daddyobb\';
	</script>';
	}
...
if($daddyobb->input['action'] == "edit_set")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			$query = $db->update_query("templatesets", array('title' => $db->escape_string($daddyobb->input['title'])), "sid='{$sid}'");
	...
	else
	{
		...
		$daddyobb->input['title'] = $db->fetch_field($query, "title");
	}
...
if($daddyobb->input['action'] == "edit_template")
{
	$plugins->run_hooks("admin_style_templates_edit_template");
	
	if(!$daddyobb->input['title'] || !$sid)
	{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			$template_array = array(
				'title' => $db->escape_string($daddyobb->input['title']),
				'sid' => $sid,
				'template' => $db->escape_string(trim($daddyobb->input['template'])),
				'version' => $daddyobb->version_code,
				'status' => '',
				'dateline' => TIME_NOW
			);
			
			$tid = intval($daddyobb->input['tid']);
			if($daddyobb->input['sid'] > 0)
			{
				$query = $db->simple_select("templates", "sid", "title='".$db->escape_string($daddyobb->input['title'])."' AND (sid = '-2' OR sid = '{$sid}')", array('order_by' => 'sid', 'order_dir' => 'desc', 'limit' => 1));
			...
			// Log admin action
			log_admin_action($tid, $daddyobb->input['title'], $sid, $set['title']);
			
			flash_message($lang->success_template_saved, 'success');
			
			if($daddyobb->input['continue'])
			{
				if($daddyobb->input['from'] == "diff_report")
				{
					admin_redirect("index.php?module=style/templates&action=edit_template&title=".urlencode($daddyobb->input['title'])."&sid=".$sid.$expand_str2."&amp;from=diff_report");
				}
				else
				{
					admin_redirect("index.php?module=style/templates&action=edit_template&title=".urlencode($daddyobb->input['title'])."&sid=".$sid.$expand_str2);
	...
	{		
		$query = $db->simple_select("templates", "*", "title='".$db->escape_string($daddyobb->input['title'])."' AND (sid='-2' OR sid='{$sid}')", array('order_by' => 'sid', 'order_dir' => 'DESC', 'limit' => 1));
	...
	$form_container->output_row("", "", $form->generate_text_area('template', $template['template'], array('id' => 'template', 'class' => 'codepress daddyobb', 'style' => 'width: 100%; height: 500px;')));
	$form_container->end();
...
if($daddyobb->input['action'] == "search_replace")
{
	$plugins->run_hooks("admin_style_templates_search_replace");
	
	if($daddyobb->request_method == "post")
	{
		if($daddyobb->input['type'] == "templates")
		{
			// Search and replace in templates
			
			if(!$daddyobb->input['find'])
			{
			...
			else
			{
				...
				$query = $db->simple_select("templates", "tid, title, template, sid", "template LIKE '%".$db->escape_string($daddyobb->input['find'])."%'", array('order_by' => 'sid, title', 'order_dir' => 'ASC'));
				if($db->num_rows($query) == 0)
				{
					$table->construct_cell($lang->sprintf($lang->search_noresults, htmlspecialchars_uni($daddyobb->input['find'])), array("class" => "align_center"));
				...
				else
				{
					...
					foreach($template_list as $sid => $templates)
					{
						...
						$search_header = $lang->sprintf($lang->search_header, htmlspecialchars_uni($daddyobb->input['find']), $template_sets[$sid]);						
						$table->construct_header($search_header, array("colspan" => 2));
		
						foreach($templates as $title => $template)
						{
							// Do replacement
							$newtemplate = str_ireplace($daddyobb->input['find'], $daddyobb->input['replace'], $template['template']);
							if($newtemplate != $template['template'])
							{
								// If the template is different, that means the search term has been found.
								if(trim($daddyobb->input['replace']) != "")
								{
									if($template['sid'] == -2)
									{
				...
				if(trim($daddyobb->input['replace']) != "")
				{
					...
					log_admin_action($daddyobb->input['find'], $daddyobb->input['replace']);
				}
		...
		else
		{
			...
			else
			{
				...
				$query = $db->query("
					...
					WHERE t.title LIKE '%".$db->escape_string($daddyobb->input['title'])."%'
				...
				{
					$table->construct_cell($lang->sprintf($lang->search_noresults_title, htmlspecialchars_uni($daddyobb->input['title'])), array("class" => "align_center"));
				...
				foreach($templatessets as $sid => $templates)
				{
					...
					foreach($templates as $template)
					{
						...
						{					
							if($sid > 0)
							{
								...
								$popup->add_item($lang->revert_to_orig, "index.php?module=style/templates&amp;action=revert&amp;title=".urlencode($template['title'])."&amp;sid={$sid}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')");
							}
						...
						{
							$popup->add_item($lang->delete_template, "index.php?module=style/templates&amp;action=delete_template&amp;title=".urlencode($template['title'])."&amp;sid={$sid}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_deletion}')");
					...
					if($count == 1)
					{
						$table->output($lang->sprintf($lang->search_names_header, htmlspecialchars_uni($daddyobb->input['title'])));
					}
					else if($count > 0)
					{
						$table->output();
	...
	$form_container->output_row($lang->search_for, "", $form->generate_text_area('find', $daddyobb->input['find'], array('id' => 'find', 'class' => 'codepress daddyobb', 'style' => 'width: 100%; height: 200px;')));
	
	$form_container->output_row($lang->replace_with, "", $form->generate_text_area('replace', $daddyobb->input['replace'], array('id' => 'replace', 'class' => 'codepress daddyobb', 'style' => 'width: 100%; height: 200px;')));
	$form_container->end();
	...
	$form_container->output_row($lang->search_for, "", $form->generate_text_box('title', $daddyobb->input['title'], array('id' => 'title')), 'title');
...
if($daddyobb->input['action'] == "find_updated")
{
	$plugins->run_hooks("admin_style_templates_find_updated");

	// Finds templates that are old and have been updated by DaddyoBB
	$compare_version = $daddyobb->version_code;
	$query = $db->query("
...
LEGEND;
	...
	foreach($templates as $sid => $templates)
	{
		...
		{		
			...
			$popup->add_item($lang->revert_to_orig, "index.php?module=style/templates&amp;action=revert&amp;title=".urlencode($template['title'])."&amp;sid={$sid}&amp;from=diff_report&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')");
...
if($daddyobb->input['action'] == "delete_set")
{
...
if($daddyobb->input['action'] == "delete_template")
{
	...
	$query = $db->query("
		...
		FROM ".TABLE_PREFIX."templates t
		LEFT JOIN ".TABLE_PREFIX."templatesets s ON(t.sid=s.sid)
		WHERE t.title='".$db->escape_string($daddyobb->input['title'])."' AND t.sid > '-2' AND t.sid = '{$sid}'
	");
	$template = $db->fetch_array($query);
...
if($daddyobb->input['action'] == "diff_report")
{
	// Compares a template of sid1 with that of sid2, if no sid1, it is assumed -2
	if(!$daddyobb->input['sid1'])
	{
		$daddyobb->input['sid1'] = -2;
	}
	
	if($daddyobb->input['sid2'] == -2)
	{
		$sub_tabs['find_updated'] = array(
			'title' => $lang->find_updated,
			'link' => "index.php?module=style/templates&amp;action=find_updated"
	...
	$sub_tabs['diff_report'] = array(
		'title' => $lang->diff_report,
		'link' => "index.php?module=style/templates&amp;action=diff_report&amp;title=".$db->escape_string($daddyobb->input['title'])."&amp;sid1=".intval($daddyobb->input['sid1'])."&amp;sid2=".intval($daddyobb->input['sid2']),
		'description' => $lang->diff_report_desc
	);
	
	$plugins->run_hooks("admin_style_templates_diff_report");
	
	$query = $db->simple_select("templates", "*", "title='".$db->escape_string($daddyobb->input['title'])."' AND sid='".intval($daddyobb->input['sid1'])."'");
	$template1 = $db->fetch_array($query);

	$query = $db->simple_select("templates", "*", "title='".$db->escape_string($daddyobb->input['title'])."' AND sid='".intval($daddyobb->input['sid2'])."'");
	...
	if($daddyobb->input['sid2'] == -2)
	{
		$sub_tabs['full_edit'] = array(
			...
			'link' => "index.php?module=style/templates&action=edit_template&title=".urlencode($template1['title'])."&sid=".intval($daddyobb->input['sid1'])."&amp;from=diff_report",
		);
	}
	...
	if($template1['template'] == $template2['template'])
	{
		...
		admin_redirect("index.php?module=style/templates&sid=".intval($daddyobb->input['sid2']).$expand_str);
	}
	...
	require_once DADDYOBB_ROOT."inc/3rdparty/diff/Diff.php";	
	require_once DADDYOBB_ROOT."inc/3rdparty/diff/Diff/Renderer/inline.php";
	...
	$page->add_breadcrumb_item($lang->diff_report.": ".$template1['title'], "index.php?module=style/templates&amp;action=diff_report&amp;title=".$db->escape_string($daddyobb->input['title'])."&amp;sid1=".intval($daddyobb->input['sid1'])."&amp;sid2=".intval($daddyobb->input['sid2']));
...
if($daddyobb->input['action'] == "revert")
{
	...
	$query = $db->query("
		...
		WHERE t.title='".$db->escape_string($daddyobb->input['title'])."' AND t.sid > 0 AND t.sid = '".intval($daddyobb->input['sid'])."'
	");
	$template = $db->fetch_array($query);
...
if($daddyobb->input['sid'] && !$daddyobb->input['action'])
{
	...
	if($sid == -1)
	{
		...
		while($template = $db->fetch_array($query))
		{
			...
			$popup->add_item($lang->delete_template, "index.php?module=style/templates&amp;action=delete_template&amp;title=".urlencode($template['title'])."&amp;sid=-1&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_deletion}')");
	...
	if(count($expand_array) > 0)
	{
		...
		$query = $db->simple_select("templates", "*", "(sid='".intval($daddyobb->input['sid'])."' OR sid='-2') {$template_sql}", array('order_by' => 'sid DESC, title', 'order_dir' => 'ASC'));
		while($template = $db->fetch_array($query))
		{
			...
			foreach($templates as $template)
			{
				...
				{					
					if($sid > 0)
					{
						...
						$popup->add_item($lang->revert_to_orig, "index.php?module=style/templates&amp;action=revert&amp;title=".urlencode($template['title'])."&amp;sid={$sid}&amp;my_post_key={$daddyobb->post_code}{$expand_str}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_revertion}')");
					}
				...
				{
					$popup->add_item($lang->delete_template, "index.php?module=style/templates&amp;action=delete_template&amp;title=".urlencode($template['title'])."&amp;sid={$sid}&amp;my_post_key={$daddyobb->post_code}{$expand_str}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_deletion}')");
	...
	foreach($template_sets as $set)
	{
		...
		{	
			...
			if($set['sid'] != 1)
			{
				...
				if(!$themes[$set['sid']])
				{
					$popup->add_item($lang->delete_template_set, "index.php?module=style/templates&amp;action=delete_set&amp;sid={$set['sid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_template_set_deletion}')");
				}
			}

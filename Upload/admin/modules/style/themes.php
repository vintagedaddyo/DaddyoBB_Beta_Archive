...
require_once DADDYOBB_ADMIN_DIR."inc/functions_themes.php";
...
if($daddyobb->input['action'] == "xmlhttp_stylesheet" && $daddyobb->request_method == "post")
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	...
	$query = $db->simple_select("themestylesheets", "*", "name='".$db->escape_string($daddyobb->input['file'])."' AND tid IN ({$parent_list})", array('order_by' => 'tid', 'order_dir' => 'desc', 'limit' => 1));
	...
	$selector_list = get_selectors_as_options($css_array, $daddyobb->input['selector']);
	$editable_selector = $css_array[$daddyobb->input['selector']];
	...
	echo $form->generate_hidden_field("tid", $daddyobb->input['tid'], array('id' => "tid"))."\n";
	echo $form->generate_hidden_field("file", htmlspecialchars_uni($daddyobb->input['selector']), array('id' => "file"))."\n";
	echo $form->generate_hidden_field("selector", htmlspecialchars_uni($daddyobb->input['selector']), array('id' => 'hidden_selector'))."\n";
...
if($daddyobb->input['action'] == "add" || $daddyobb->input['action'] == "import" || !$daddyobb->input['action'])
{
...
if($daddyobb->input['action'] == "import")
{
	if($daddyobb->request_method == "post")
	{
		if(!$_FILES['local_file'] && !$daddyobb->input['url'])
		{
		...
		if(!$errors)
		{
			...
			else if(!empty($daddyobb->input['url']))
			{
				// Get the contents
				$contents = @fetch_remote_file($daddyobb->input['url']);
				if(!$contents)
				{
			...
			{
				$options = array(
					'no_stylesheets' => ($daddyobb->input['import_stylesheets'] ? 0 : 1),
					'no_templates' => ($daddyobb->input['import_templates'] ? 0 : 1),
					'version_compat' => intval($daddyobb->input['version_compat']),
					'parent' => intval($daddyobb->input['tid']),
	...
	else
	{
		...
		$daddyobb->input['import_stylesheets'] = true;
		$daddyobb->input['import_templates'] = true;
	}
	...
	<table cellpadding="4">
		...
		<table cellpadding="4">
				<tr>
					<td>'.$form->generate_text_box("url", $daddyobb->input['file']).'</td>
				</tr>
		</table></dd>
	</dl>
	<script type="text/javascript">
	...
	$form_container->output_row($lang->parent_theme, $lang->parent_theme_desc, $form->generate_select_box('tid', $themes, $daddyobb->input['tid'], array('id' => 'tid')), 'tid');
	$form_container->output_row($lang->new_name, $lang->new_name_desc, $form->generate_text_box('name', $daddyobb->input['name'], array('id' => 'name')), 'name');
	$form_container->output_row($lang->advanced_options, "", $form->generate_check_box('version_compat', '1', $lang->ignore_version_compatibility, array('checked' => $daddyobb->input['version_compat'], 'id' => 'version_compat'))."<br /><small>{$lang->ignore_version_compat_desc}</small><br />".$form->generate_check_box('import_stylesheets', '1', $lang->import_stylesheets, array('checked' => $daddyobb->input['import_stylesheets'], 'id' => 'import_stylesheets'))."<br /><small>{$lang->import_stylesheets_desc}</small><br />".$form->generate_check_box('import_templates', '1', $lang->import_templates, array('checked' => $daddyobb->input['import_templates'], 'id' => 'import_templates'))."<br /><small>{$lang->import_templates_desc}</small>");
...
if($daddyobb->input['action'] == "export")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		$xml .= "<theme name=\"".htmlspecialchars_uni($theme['name'])."\" version=\"".$daddyobb->version_code."\">\r\n";
		$xml .= "\t<properties>\r\n";
		...
		$inherited_load[] = $daddyobb->input['tid'];
		...
		foreach($stylesheets as $filename => $style)
		{
			...
			if($daddyobb->input['custom_theme'] == 1 && $style['tid'] != $daddyobb->input['tid'])
			{
				continue;
			}
			...
			$xml .= "\t\t<stylesheet name=\"{$filename}\" {$attachedto}version=\"{$daddyobb->version_code}\"><![CDATA[{$stylesheet}]]>\r\n\t\t</stylesheet>\r\n";
		
		}
		$xml .= "\t</stylesheets>\r\n";
		
		if($daddyobb->input['include_templates'] != 0)
		{
			$xml .= "\t<templates>\r\n";
	...
	$page->add_breadcrumb_item(htmlspecialchars_uni($theme['name']), "index.php?module=style/themes&amp;action=edit&amp;tid={$daddyobb->input['tid']}");
	...
	$sub_tabs['edit_stylesheets'] = array(
		...
		'link' => "index.php?module=style/themes&amp;action=edit&amp;tid={$daddyobb->input['tid']}",
	);

	$sub_tabs['add_stylesheet'] = array(
		'title' => $lang->add_stylesheet,
		'link' => "index.php?module=style/themes&amp;action=add_stylesheet&amp;tid={$daddyobb->input['tid']}",
	);
	
	$sub_tabs['export_theme'] = array(
		'title' => $lang->export_theme,
		'link' => "index.php?module=style/themes&amp;action=export&amp;tid={$daddyobb->input['tid']}",
	...
	$form_container->output_row($lang->include_custom_only, $lang->include_custom_only_desc, $form->generate_yes_no_radio('custom_theme', $daddyobb->input['custom_theme']), 'custom_theme');
	$form_container->output_row($lang->include_templates, $lang->include_templates_desc, $form->generate_yes_no_radio('include_templates', $daddyobb->input['include_templates']), 'include_templates');
...
if($daddyobb->input['action'] == "add")
{
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			$tid = build_new_theme($daddyobb->input['name'], null, $daddyobb->input['tid']);
			
			// Log admin action
			log_admin_action($daddyobb->input['name'], $tid);
	...
	$form_container->output_row($lang->name, $lang->name_desc, $form->generate_text_box('name', $daddyobb->input['name'], array('id' => 'name')), 'name');
	$form_container->output_row($lang->parent_theme, $lang->parent_theme_desc, $form->generate_select_box('tid', $themes, $daddyobb->input['tid'], array('id' => 'tid')), 'tid');
...
if($daddyobb->input['action'] == "delete")
{
	...
	{		
		...
		{
			@unlink(DADDYOBB_ROOT."cache/themes/theme{$theme['tid']}/{$cachefile['cachefile']}");
		}
		@unlink(DADDYOBB_ROOT."cache/themes/theme{$theme['tid']}/index.html");
		...
		@rmdir(DADDYOBB_ROOT."cache/themes/theme{$theme['tid']}/");
...
if($daddyobb->input['action'] == "edit")
{
	...
	{
		$properties = array(
			'templateset' => intval($daddyobb->input['templateset']),
			'editortheme' => $daddyobb->input['editortheme'],
			'imgdir' => $daddyobb->input['imgdir'],
			'logo' => $daddyobb->input['logo'],
			'tablespace' => intval($daddyobb->input['tablespace']),
			'borderwidth' => intval($daddyobb->input['borderwidth'])
		);
		...
		$allowedgroups = array();
		if(is_array($daddyobb->input['allowedgroups']))
		{
			foreach($daddyobb->input['allowedgroups'] as $gid)
			{
				if($gid == "all")
				{
					$allowedgroups = "all";
		...
		if(!$properties['editortheme'] || !@is_dir(DADDYOBB_ROOT."jscripts/editor_themes/".$properties['editortheme']))
		{
	...
	foreach($stylesheets as $filename => $style)
	{
		...
		if($inherited == "")
		{
			$popup->add_item($lang->delete_revert, "index.php?module=style/themes&amp;action=delete_stylesheet&amp;file=".htmlspecialchars_uni($filename)."&amp;tid={$theme['tid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_stylesheet_deletion}')");
		}
	...
	$options = array();
	$editor_theme_root = DADDYOBB_ROOT."jscripts/editor_themes/";
...
if($daddyobb->input['action'] == "stylesheet_properties")
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			if($daddyobb->input['attach'] == 1)
			{
				// Our stylesheet is attached to custom pages in DaddyoBB
				foreach($daddyobb->input as $id => $value)
				{
					$actions_list = "";
					$attached_to = $value;
					...
					if(strpos($id, 'attached_') !== false)
					{
						...
						if($daddyobb->input['action_'.$attached_id] == 1)
						{
							// We have custom actions for attached files							
							$actions_list = $daddyobb->input['action_list_'.$attached_id];
						}
			...
			if($stylesheet['name'] != $daddyobb->input['name'])
			{
				$update_array['cachefile'] = $db->escape_string(str_replace('/', '', $daddyobb->input['name']));
			}
			...
			if($stylesheet['name'] != $daddyobb->input['name'])
			{
				...
				if(!cache_stylesheet($theme['tid'], str_replace('/', '', $daddyobb->input['name']), $theme['stylesheet']))
				{
					$db->update_query("themestylesheets", array('cachefile' => "css.php?stylesheet={$stylesheet['sid']}"), "sid='{$stylesheet['sid']}'", 1);
				}
				@unlink(DADDYOBB_ROOT."cache/themes/theme{$theme['tid']}/{$stylesheet['cachefile']}");
			}
			...
			// Log admin action
			log_admin_action($stylesheet['sid'], $daddyobb->input['name'], $theme['tid'], $theme['name']);
		
			flash_message($lang->success_stylesheet_properties_updated, 'success');
			admin_redirect("index.php?module=style/themes&action=stylesheet_properties&tid={$theme['tid']}&file=".htmlspecialchars_uni($daddyobb->input['file']));
		}
	}
	
	
	$page->add_breadcrumb_item(htmlspecialchars_uni($theme['name']), "index.php?module=style/themes&amp;action=edit&amp;tid={$daddyobb->input['tid']}");
	$page->add_breadcrumb_item(htmlspecialchars_uni($stylesheet['name'])." {$lang->properties}", "index.php?module=style/themes&amp;action=edit_properties&amp;tid={$daddyobb->input['tid']}");
	...
	if($errors)
	{
		...
		foreach($daddyobb->input as $name => $value)
		{
			if(strpos($name, "attached") !== false)
			{
				...
				if($daddyobb->input['action_'.$id] == 1)
				{
					$applied_to[$value] = explode(',', $daddyobb->input['action_list_'.$id]);
				}
			}
		}
	}
	...
	{
		$daddyobb->input['name'] = $stylesheet['name'];
	}
	...
	$form_container->output_row($lang->file_name, $lang->file_name_desc, $form->generate_text_box('name', $daddyobb->input['name'], array('id' => 'name', 'style' => 'width: 200px;')), 'name');
...
if($daddyobb->input['action'] == "edit_stylesheet" && (!$daddyobb->input['mode'] || $daddyobb->input['mode'] == "simple"))
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
	...
	if($daddyobb->request_method == "post")
	{
		...
		if($daddyobb->input['serialized'] == 1)
		{
			$daddyobb->input['css_bits'] = unserialize($daddyobb->input['css_bits']);
		}

		$css_to_insert = '';
		foreach($daddyobb->input['css_bits'] as $field => $value)
		{
		...
		$new_stylesheet = insert_into_css($css_to_insert, $daddyobb->input['selector'], $new_stylesheet);
	...
	$page->add_breadcrumb_item(htmlspecialchars_uni($theme['name']), "index.php?module=style/themes&amp;action=edit&amp;tid={$daddyobb->input['tid']}");
	$page->add_breadcrumb_item("{$lang->editing} ".htmlspecialchars_uni($stylesheet['name']), "index.php?module=style/themes&amp;action=edit_stylesheet&amp;tid={$daddyobb->input['tid']}&amp;file=".htmlspecialchars_uni($daddyobb->input['file'])."&amp;mode=simple");
	...
	$sub_tabs['edit_stylesheet'] = array(
		...
		'link' => "index.php?module=style/themes&amp;action=edit_stylesheet&amp;tid={$daddyobb->input['tid']}&amp;file=".htmlspecialchars_uni($daddyobb->input['file'])."&amp;mode=simple",
	...
	$sub_tabs['edit_stylesheet_advanced'] = array(
		...
		'link' => "index.php?module=style/themes&amp;action=edit_stylesheet&amp;tid={$daddyobb->input['tid']}&amp;file=".htmlspecialchars_uni($daddyobb->input['file'])."&amp;mode=advanced",
	);
	...
	echo $form->generate_hidden_field("file", htmlspecialchars_uni($daddyobb->input['file']))."\n";	
	...
	{
		$editable_selector = $css_array[$daddyobb->input['selector']];
		$selector = $daddyobb->input['selector'];
	}
	...
	echo $form->generate_hidden_field("tid", $daddyobb->input['tid'], array('id' => "tid"))."\n";
	echo $form->generate_hidden_field("file", htmlspecialchars_uni($daddyobb->input['file']), array('id' => "file"))."\n";
...
Event.observe(window, "load", function() {
//<![CDATA[
    new ThemeSelector("./index.php?module=style/themes&action=xmlhttp_stylesheet", "./index.php?module=style/themes&action=edit_stylesheet", $("selector"), $("stylesheet"), "'.htmlspecialchars_uni($daddyobb->input['file']).'", $("selector_form"), "'.$daddyobb->input['tid'].'");
});
//]]>
</script>';
...
if($daddyobb->input['action'] == "edit_stylesheet" && $daddyobb->input['mode'] == "advanced")
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
	...
	if($daddyobb->request_method == "post")
	{
		...
		// Now we have the new stylesheet, save it
		$updated_stylesheet = array(
			"stylesheet" => $db->escape_string(unfix_css_urls($daddyobb->input['stylesheet'])),
			"lastmodified" => TIME_NOW
		);
		$db->update_query("themestylesheets", $updated_stylesheet, "sid='{$sid}'");
		
		// Cache the stylesheet to the file		
		if(!cache_stylesheet($theme['tid'], $stylesheet['name'], $daddyobb->input['stylesheet']))
		{
			$db->update_query("themestylesheets", array('cachefile' => "css.php?stylesheet={$sid}"), "sid='{$sid}'", 1);
		}
	...
	$page->add_breadcrumb_item(htmlspecialchars_uni($theme['name']), "index.php?module=style/themes&amp;action=edit&amp;tid={$daddyobb->input['tid']}");
	$page->add_breadcrumb_item("{$lang->editing} ".htmlspecialchars_uni($stylesheet['name']), "index.php?module=style/themes&amp;action=edit_stylesheet&amp;tid={$daddyobb->input['tid']}&amp;file=".htmlspecialchars_uni($daddyobb->input['file'])."&amp;mode=advanced");
...
if($daddyobb->input['action'] == "delete_stylesheet")
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
...
if($daddyobb->input['action'] == "add_stylesheet")
{
	// Fetch the theme we want to edit this stylesheet in
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			if($daddyobb->input['add_type'] == 1)
			{
				// Import from a current stylesheet
				$parent_list = make_parent_theme_list($theme['tid']);
				$parent_list = implode(',', $parent_list);
				
				$query = $db->simple_select("themestylesheets", "stylesheet", "name='".$db->escape_string($daddyobb->input['import'])."' AND tid IN ({$parent_list})", array('limit' => 1, 'order_by' => 'tid', 'order_dir' => 'desc'));
				$stylesheet = $db->fetch_field($query, "stylesheet");
			}
			else
			{
				// Custom stylesheet
				$stylesheet = $daddyobb->input['stylesheet'];
			}
			...
			$insert_array = array(
				...
				'cachefile' => $db->escape_string(str_replace('/', '', $daddyobb->input['name'])),
				'lastmodified' => TIME_NOW
			);			
			
			$sid = $db->insert_query("themestylesheets", $insert_array);
			
			if(!cache_stylesheet($theme['tid'], str_replace('/', '', $daddyobb->input['name']), $stylesheet))
			{
				$db->update_query("themestylesheets", array('cachefile' => "css.php?stylesheet={$sid}"), "sid='{$sid}'", 1);
			}
			...
			// Log admin action
			log_admin_action($sid, $daddyobb->input['name'], $theme['tid'], $theme['name']);
		
			flash_message($lang->success_stylesheet_added, 'success');
			admin_redirect("index.php?module=style/themes&action=edit_stylesheet&tid={$daddyobb->input['tid']}&sid={$sid}&file=".urlencode($daddyobb->input['name']));
		}
	}
	...
	if($errors)
	{
		...
		foreach($daddyobb->input as $name => $value)
		{
			if(strpos($name, "attached") !== false)
			{
				...
				$daddyobb->input['applied_to'][$value] = array(0 => 'global');
				
				if($daddyobb->input['action_'.$id] == 1)
				{
					$daddyobb->input['applied_to'][$value] = explode(',', $daddyobb->input['action_list_'.$id]);
				}
			}
		}
	...
	if(is_array($daddyobb->input['applied_to']) && $daddyobb->input['applied_to']['global'][0] != "global")
	{
		$check_actions = "";
		...
		foreach($daddyobb->input['applied_to'] as $name => $actions)
		{
			$short_name = substr($name, 0, -4);
	...
	<dt><label style=\"display: block;\"><input type=\"radio\" name=\"add_type\" value=\"1\" {$add_checked[1]} class=\"adds_check\" onclick=\"checkAction('add');\" style=\"vertical-align: middle;\" /> <strong>{$lang->import_stylesheet_from}</strong></label></dt>
		<dd style=\"margin-top: 4px;\" id=\"add_1\" class=\"adds\">
			<table cellpadding=\"4\">
				<tr>
					<td>".$form->generate_select_box('import', $sheetnames, $daddyobb->input['import'], array('id' => 'import'))."</td>
				</tr>
			</table>
		</dd>
		...
		<span id=\"add_2\" class=\"adds\"><br />".$form->generate_text_area('stylesheet', $daddyobb->input['stylesheet'], array('id' => 'stylesheet', 'style' => 'width: 99%;', 'class' => 'codepress css', 'rows' => '30'))."</span>
	</dl>";
...
if($daddyobb->input['action'] == "set_default")
{
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);
	...
	$db->update_query("themes", array('def' => 1), "tid='".intval($daddyobb->input['tid'])."'");
...
if($daddyobb->input['action'] == "force")
{
	$query = $db->simple_select("themes", "*", "tid='".intval($daddyobb->input['tid'])."'");
	$theme = $db->fetch_array($query);

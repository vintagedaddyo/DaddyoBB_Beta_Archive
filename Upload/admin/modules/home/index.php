...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->request_method == "post" && isset($daddyobb->input['adminnotes']))
	{
		// Update Admin Notes cache
		$update_cache = array(
			"adminmessage" => $daddyobb->input['adminnotes']
		);
	...
	if($update_check['latest_version_code'] > $daddyobb->version_code)
	{
		$lang->new_version_available = $lang->sprintf($lang->new_version_available, "DaddyoBB {$daddyobb->version}", "<a href=\"http://liteoft.info/downloads\" target=\"_blank\">DaddyoBB {$update_check['latest_version']}</a>");
	...
	$table = new Table;
	$table->construct_header($lang->DADDYOBB_server_stats, array("colspan" => 2));
	$table->construct_header($lang->forum_stats, array("colspan" => 2));
	
	$table->construct_cell("<strong>{$lang->DADDYOBB_version}</strong>", array('width' => '25%'));
	$table->construct_cell($daddyobb->version, array('width' => '25%'));

...
$usergroup_permissions = array(
	...
	"avatarmaxsize" => 10,
	"avatarmaxdimensions" => "100x100",
		"avatarmaxsize" => 15,
	"avatarmaxdimensions" => "130x130",
	"sigmaxchars" => 1000,
	"sigallowmycode" => 1,
	"sigallowimgcode" => 1,
	"signumimages" =>  4,
	...
	"maxwarningsday" => 0,
	"canmodcp" => 0,
	"modcanannouncements" => 0,
	"modcanviewmodlogs" => 0,
	"modcanmanagevms" => 0,
	"modcanmanageprofiles" => 0,
	"modcanbanusers" => 0
);
...
if($daddyobb->input['action'] == "export")
{
	...
	$gidwhere = "";
	if($daddyobb->input['gid'])
	{
		$gidwhere = "gid='".intval($daddyobb->input['gid'])."'";
	}
	$xml = "<?xml version=\"1.0\" encoding=\"{$lang->settings['charset']}\"?".">\n";
	$xml = "<usergroups version=\"{$daddyobb->version_code}\" exported=\"".TIME_NOW."\">\n";
	...
	$xml .= "</usergroups>";
	$daddyobb->settings['bbname'] = urlencode($daddyobb->settings['bbname']);

	header("Content-disposition: filename=".$daddyobb->settings['bbname']."-usergroups.xml");
...
if($daddyobb->input['action'] == "approve_join_request")
{
	$plugins->run_hooks("admin_user_groups_approve_join_request");
	
	$query = $db->simple_select("joinrequests", "*", "rid='".$daddyobb->input['rid']."'");
...
if($daddyobb->input['action'] == "deny_join_request")
{
...
if($daddyobb->input['action'] == "join_requests")
{
	$plugins->run_hooks("admin_user_groups_join_requests_start");
	
	$query = $db->simple_select("usergroups", "*", "gid='".intval($daddyobb->input['gid'])."'");
	...
	if($daddyobb->request_method == "post" && is_array($daddyobb->input['users']))
	{		
		$uid_in = implode(",", array_map('intval', $daddyobb->input['users']));
		
		if(isset($daddyobb->input['approve']))
		{
			foreach($daddyobb->input['users'] as $uid)
			{
				$uid = intval($uid);
	...
	while($request = $db->fetch_array($query))
	{
		...
		$table->construct_cell(my_date($daddyobb->settings['dateformat'].", ".$daddyobb->settings['timeformat'], $request['dateline']), array('class' => 'align_center'));
...
if($daddyobb->input['action'] == "add_leader" && $daddyobb->request_method == "post")
{
	...
	$query = $db->simple_select("users", "uid, username", "LOWER(username)='".$db->escape_string(my_strtolower($daddyobb->input['username']))."'");
	...
	if(!$errors)
	{
		$new_leader = array(
			"gid" => $group['gid'],
			"uid" => $user['uid'],
			"canmanagemembers" => intval($daddyobb->input['canmanagemembers']),
			"canmanagerequests" => intval($daddyobb->input['canmanagerequests'])
		);
		...
		// Log admin action
		log_admin_action($user['uid'], $daddyobb->input['username'], $group['gid'], $group['title']);
	...
	else
	{
		...
		$daddyobb->input['action'] = "leaders";
	}
}

// Show a listing of group leaders
if($daddyobb->input['action'] == "leaders")
{
	...
	while($leader = $db->fetch_array($query))
	{
		...
		$table->construct_cell("<a href=\"index.php?module=user/groups&amp;action=delete_leader&amp;lid={$leader['lid']}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_group_leader_deletion}')\">{$lang->delete}</a>", array("width" => 100, "class" => "align_center"));
		$table->construct_row();
	}
	...
	$form_container->output_row($lang->username." <em>*</em>", "", $form->generate_text_box('username', $daddyobb->input['username'], array('id' => 'username')), 'username');
	$form_container->output_row($lang->can_manage_group_members, $lang->can_manage_group_members_desc, $form->generate_yes_no_radio('canmanagemembers', $daddyobb->input['canmanagemembers']));
		$form_container->output_row($lang->can_manage_group_join_requests, $lang->can_manage_group_join_requests_desc, $form->generate_yes_no_radio('canmanagerequests', $daddyobb->input['canmanagerequests']));
...
if($daddyobb->input['action'] == "delete_leader")
{
	...
	$query = $db->query("
		...
		WHERE l.lid='".intval($daddyobb->input['lid'])."'");
...
if($daddyobb->input['action'] == "edit_leader")
{
...
if($daddyobb->input['action'] == "add")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(my_strpos($daddyobb->input['namestyle'], "{username}") === false)
		{
		...
		if(!$errors)
		{
			if($daddyobb->input['stars'] < 1)
			{
				$daddyobb->input['stars'] = 0;
			}
			
			if(!$daddyobb->input['starimage'])
			{
				$daddyobb->input['starimage'] = "images/star.gif";
			}
			...
			$new_usergroup = array(
				...
				"namestyle" => $db->escape_string($daddyobb->input['namestyle']),
				"usertitle" => $db->escape_string($daddyobb->input['usertitle']),
			...
			{
				$query = $db->simple_select("usergroups", "*", "gid='".intval($daddyobb->input['copyfrom'])."'");
			...
			if($daddyobb->input['copyfrom'] > 0)
			{
				$query = $db->simple_select("forumpermissions", "*", "gid='".intval($daddyobb->input['copyfrom'])."'");
			...
			// Log admin action
			log_admin_action($gid, $daddyobb->input['title']);
	...
	$form_container->output_row($lang->username_style, $lang->username_style_desc, $form->generate_text_box('namestyle', $daddyobb->input['namestyle'], array('id' => 'namestyle')), 'namestyle');
	$form_container->output_row($lang->user_title, $lang->user_title_desc, $form->generate_text_box('usertitle', $daddyobb->input['usertitle'], array('id' => 'usertitle')), 'usertitle');
	...
	$form_container->output_row($lang->copy_permissions_from, $lang->copy_permissions_from_desc, $form->generate_select_box('copyfrom', $options, $daddyobb->input['copyfrom'], array('id' => 'copyfrom')), 'copyfrom');
...
if($daddyobb->input['action'] == "edit")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			if($daddyobb->input['joinable'] == 1)
			{
				if($daddyobb->input['moderate'] == 1)
				{
					$daddyobb->input['type'] = "4";
				}
				else
				{
					$daddyobb->input['type'] = "3";
				}
			}
			else
			{
				$daddyobb->input['type'] = "2";
			}

			if($usergroup['type'] == 1)
			{
				$daddyobb->input['type'] = 1;
			}
			...
			$updated_group = array(
				"type" => intval($daddyobb->input['type']),
				"title" => $db->escape_string($daddyobb->input['title']),
				"description" => $db->escape_string($daddyobb->input['description']),
				"namestyle" => $db->escape_string($daddyobb->input['namestyle']),
				"usertitle" => $db->escape_string($daddyobb->input['usertitle']),
				"stars" => intval($daddyobb->input['stars']),
				"starimage" => $db->escape_string($daddyobb->input['starimage']),
				"image" => $db->escape_string($daddyobb->input['image']),
				"isbannedgroup" => intval($daddyobb->input['isbannedgroup']),
				"canview" => intval($daddyobb->input['canview']),
				"canviewthreads" => intval($daddyobb->input['canviewthreads']),
				"canviewprofiles" => intval($daddyobb->input['canviewprofiles']),
				"candlattachments" => intval($daddyobb->input['candlattachments']),
				"canpostthreads" => intval($daddyobb->input['canpostthreads']),
				"canpostreplys" => intval($daddyobb->input['canpostreplys']),
				"canpostattachments" => intval($daddyobb->input['canpostattachments']),
				"canratethreads" => intval($daddyobb->input['canratethreads']),
				"caneditposts" => intval($daddyobb->input['caneditposts']),
				"candeleteposts" => intval($daddyobb->input['candeleteposts']),
				"candeletethreads" => intval($daddyobb->input['candeletethreads']),
				"caneditattachments" => intval($daddyobb->input['caneditattachments']),
				"avatarmaxsize" => $db->escape_string($daddyobb->input['avatarmaxsize']),
				"avatarmaxdimensions" => $db->escape_string($daddyobb->input['avatarmaxdimensions']),
				"profilepicmaxsize" => $db->escape_string($daddyobb->input['profilepicmaxsize']),
				"profilepicmaxdimensions" => $db->escape_string($daddyobb->input['profilepicmaxdimensions']),
				"sigmaxchars" => intval($daddyobb->input['sigmaxchars']),
				"sigallowmycode" => intval($daddyobb->input['sigallowmycode']),
				"sigallowimgcode" => intval($daddyobb->input['sigallowimgcode']),
				"signumimages" =>  intval($daddyobb->input['signumimages']),
				"sigallowsmilies" => intval($daddyobb->input['sigallowsmilies']),
				"sigallowhtml" => intval($daddyobb->input['sigallowhtml']),
				"canpostpolls" => intval($daddyobb->input['canpostpolls']),
				"canvotepolls" => intval($daddyobb->input['canvotepolls']),
				"canusevms" => intval($daddyobb->input['canusevms']),
				"caneditownvms" => intval($daddyobb->input['caneditownvms']),
				"candeleteownvms" => intval($daddyobb->input['candeleteownvms']),
				"canmanagevms" => intval($daddyobb->input['canmanagevms']),
				"canusepms" => intval($daddyobb->input['canusepms']),
				"cansendpms" => intval($daddyobb->input['cansendpms']),
				"cantrackpms" => intval($daddyobb->input['cantrackpms']),
				"candenypmreceipts" => intval($daddyobb->input['candenypmreceipts']),
				"pmquota" => intval($daddyobb->input['pmquota']),
				"maxpmrecipients" => intval($daddyobb->input['maxpmrecipients']),
				"cansendemail" => intval($daddyobb->input['cansendemail']),
				"maxemails" => intval($daddyobb->input['maxemails']),		
				"canviewmemberlist" => intval($daddyobb->input['canviewmemberlist']),
				"canviewcalendar" => intval($daddyobb->input['canviewcalendar']),
				"canaddevents" => intval($daddyobb->input['canaddevents']),
				"canbypasseventmod" => intval($daddyobb->input['canbypasseventmod']),
				"canmoderateevents" => intval($daddyobb->input['canmoderateevents']),
				"canviewonline" => intval($daddyobb->input['canviewonline']),
				"canviewwolinvis" => intval($daddyobb->input['canviewwolinvis']),
				"canviewonlineips" => intval($daddyobb->input['canviewonlineips']),
				"cancp" => intval($daddyobb->input['cancp']),
				"issupermod" => intval($daddyobb->input['issupermod']),
				"cansearch" => intval($daddyobb->input['cansearch']),
				"canusercp" => intval($daddyobb->input['canusercp']),
				"canuploadavatars" => intval($daddyobb->input['canuploadavatars']),
				"canuploadprofilepics" => intval($daddyobb->input['canuploadprofilepics']),
				"canchangename" => intval($daddyobb->input['canchangename']),
				"showforumteam" => intval($daddyobb->input['showforumteam']),
				"usereputationsystem" => intval($daddyobb->input['usereputationsystem']),
				"cangivereputations" => intval($daddyobb->input['cangivereputations']),
				"reputationpower" => intval($daddyobb->input['reputationpower']),
				"maxreputationsday" => intval($daddyobb->input['maxreputationsday']),
				"attachquota" => intval($daddyobb->input['attachquota']),
				"cancustomtitle" => intval($daddyobb->input['cancustomtitle']),
				"canhtmlintitle" => intval($daddyobb->input['canhtmlintitle']),
				"canwarnusers" => intval($daddyobb->input['canwarnusers']),
				"canreceivewarnings" =>intval($daddyobb->input['canreceivewarnings']),
				"maxwarningsday" => intval($daddyobb->input['maxwarningsday']),
				"canmodcp" => intval($daddyobb->input['canmodcp']),
        "modcanannouncements" => intval($daddyobb->input['modcanannouncements']),
			  "modcanviewmodlogs" => intval($daddyobb->input['modcanviewmodlogs']),
				"modcanmanagevms" => intval($daddyobb->input['modcanmanagevms']),
			  "modcanmanageprofiles" => intval($daddyobb->input['modcanmanageprofiles']),
			  "modcanbanusers" => intval($daddyobb->input['modcanbanusers']),
			);

			// Only update the candisplaygroup setting if not a default user group
			if($usergroup['type'] != 1)
			{
				$updated_group['candisplaygroup'] = intval($daddyobb->input['candisplaygroup']);
			}
			...
			// Log admin action
			log_admin_action($usergroup['gid'], $daddyobb->input['title']);
	...
	$stars = "<table cellpadding=\"3\"><tr><td>".$form->generate_text_box('stars', $daddyobb->input['stars'], array('class' => 'field50', 'id' => 'stars'))."</td><td>".$form->generate_text_box('starimage', $daddyobb->input['starimage'], array('id' => 'starimage'))."</td></tr>";
	...
	$form_container->output_row($lang->group_image, $lang->group_image_desc, $form->generate_text_box('image', $daddyobb->input['image'], array('id' => 'image')), 'image');

	$general_options = array();
	if($usergroup['gid'] != "1" && $usergroup['gid'] != "5")
	{
		$general_options[] = $form->generate_check_box("showforumteam", 1, $lang->forum_team, array("checked" => $daddyobb->input['showforumteam']));
	}
	$general_options[] =	$form->generate_check_box("isbannedgroup", 1, $lang->is_banned_group, array("checked" => $daddyobb->input['isbannedgroup']));
	...
	if($usergroup['type'] != 1)
	{
		$public_options = array(
			$form->generate_check_box("joinable", 1, $lang->user_joinable, array("checked" => $daddyobb->input['joinable'])),
			$form->generate_check_box("moderate", 1, $lang->moderate_join_requests, array("checked" => $daddyobb->input['moderate'])),
			$form->generate_check_box("candisplaygroup", 1, $lang->can_set_as_display_group, array("checked" => $daddyobb->input['candisplaygroup'])),
			);
	...
	$admin_options = array(
		$form->generate_check_box("issupermod", 1, $lang->is_super_mod, array("checked" => $daddyobb->input['issupermod'])),
		$form->generate_check_box("canmodcp", 1, $lang->can_access_mod_cp, array("checked" => $daddyobb->input['canmodcp'])),
		$form->generate_check_box("cancp", 1, $lang->can_access_admin_cp, array("checked" => $daddyobb->input['cancp']))
	);
	...
	$viewing_options = array(
		$form->generate_check_box("canview", 1, $lang->can_view_board, array("checked" => $daddyobb->input['canview'])),
		$form->generate_check_box("canviewthreads", 1, $lang->can_view_threads, array("checked" => $daddyobb->input['canviewthreads'])),
		$form->generate_check_box("cansearch", 1, $lang->can_search_forums, array("checked" => $daddyobb->input['cansearch'])),
		$form->generate_check_box("canviewprofiles", 1, $lang->can_view_profiles, array("checked" => $daddyobb->input['canviewprofiles'])),
		$form->generate_check_box("candlattachments", 1, $lang->can_download_attachments, array("checked" => $daddyobb->input['candlattachments'])),
	);
	...
	$posting_options = array(
		$form->generate_check_box("canpostthreads", 1, $lang->can_post_threads, array("checked" => $daddyobb->input['canpostthreads'])),
		$form->generate_check_box("canpostreplys", 1, $lang->can_post_replies, array("checked" => $daddyobb->input['canpostreplys'])),
		$form->generate_check_box("canratethreads", 1, $lang->can_rate_threads, array("checked" => $daddyobb->input['canratethreads']))
	);
	...
	$poll_options = array(
		$form->generate_check_box("canpostpolls", 1, $lang->can_post_polls, array("checked" => $daddyobb->input['canpostpolls'])),
		$form->generate_check_box("canvotepolls", 1, $lang->can_vote_polls, array("checked" => $daddyobb->input['canvotepolls']))
	);
	...
	$attachment_options = array(
		$form->generate_check_box("canpostattachments", 1, $lang->can_post_attachments, array("checked" => $daddyobb->input['canpostattachments'])),
		"{$lang->attach_quota}<br /><small class=\"input\">{$lang->attach_quota_desc}</small><br />".$form->generate_text_box('attachquota', $daddyobb->input['attachquota'], array('id' => 'attachquota', 'class' => 'field50')). "KB"
	);
	...
	$editing_options = array(
		$form->generate_check_box("caneditposts", 1, $lang->can_edit_posts, array("checked" => $daddyobb->input['caneditposts'])),
		$form->generate_check_box("candeleteposts", 1, $lang->can_delete_posts, array("checked" => $daddyobb->input['candeleteposts'])),
		$form->generate_check_box("candeletethreads", 1, $lang->can_delete_threads, array("checked" => $daddyobb->input['candeletethreads'])),
		$form->generate_check_box("caneditattachments", 1, $lang->can_edit_attachments, array("checked" => $daddyobb->input['caneditattachments']))
	);
	...
	$account_options = array(
		$form->generate_check_box("canusercp", 1, $lang->can_access_usercp, array("checked" => $daddyobb->input['canusercp'])),
		$form->generate_check_box("canchangename", 1, $lang->can_change_username, array("checked" => $daddyobb->input['canchangename'])),
		$form->generate_check_box("cancustomtitle", 1, $lang->can_use_usertitles, array("checked" => $daddyobb->input['cancustomtitle'])),
		$form->generate_check_box("canhtmlintitle", 1, $lang->can_use_html_usertitles, array("checked" => $daddyobb->input['canhtmlintitle'])),
	);
	...
	$reputation_options = array(
		$form->generate_check_box("usereputationsystem", 1, $lang->show_reputations, array("checked" => $daddyobb->input['usereputationsystem'])),
		$form->generate_check_box("cangivereputations", 1, $lang->can_give_reputation, array("checked" => $daddyobb->input['cangivereputations'])),
		"{$lang->points_to_award_take}<br /><small class=\"input\">{$lang->points_to_award_take_desc}</small><br />".$form->generate_text_box('reputationpower', $daddyobb->input['reputationpower'], array('id' => 'reputationpower', 'class' => 'field50')),
		"{$lang->max_reputations_daily}<br /><small class=\"input\">{$lang->max_reputations_daily_desc}</small><br />".$form->generate_text_box('maxreputationsday', $daddyobb->input['maxreputationsday'], array('id' => 'maxreputationsday', 'class' => 'field50'))	
	);
	...
	$warning_options = array(
		$form->generate_check_box("canwarnusers", 1, $lang->can_send_warnings, array("checked" => $daddyobb->input['canwarnusers'])),
		$form->generate_check_box("canreceivewarnings", 1, $lang->can_receive_warnings, array("checked" => $daddyobb->input['canreceivewarnings'])),
		"{$lang->warnings_per_day}<br />".$form->generate_text_box('maxwarningsday', $daddyobb->input['maxwarningsday'], array('id' => 'maxwarningsday', 'class' => 'field50'))
	);
	...
	$pm_options = array(
		$form->generate_check_box("canusepms", 1, $lang->can_use_pms, array("checked" => $daddyobb->input['canusepms'])),
		$form->generate_check_box("cansendpms", 1, $lang->can_send_pms, array("checked" => $daddyobb->input['cansendpms'])),
		$form->generate_check_box("cantrackpms", 1, $lang->can_track_pms, array("checked" => $daddyobb->input['cantrackpms'])),
		$form->generate_check_box("candenypmreceipts", 1, $lang->can_deny_reciept, array("checked" => $daddyobb->input['candenypmreceipts'])),
		"{$lang->message_quota}:<br /><small>{$lang->message_quota_desc}</small><br />".$form->generate_text_box('pmquota', $daddyobb->input['pmquota'], array('id' => 'pmquota', 'class' => 'field50')),
		"{$lang->max_recipients}:<br /><small>{$lang->max_recipients_desc}</small><br />".$form->generate_text_box('maxpmrecipients', $daddyobb->input['maxpmrecipients'], array('id' => 'maxpmrecipients', 'class' => 'field50'))
	);
	...
	$vm_options = array(
		$form->generate_check_box("canusevms", 1, $lang->can_use_vms, array("checked" => $daddyobb->input['canusevms'])),
		$form->generate_check_box("caneditownvms", 1, $lang->can_edit_own_vms, array("checked" => $daddyobb->input['caneditownvms'])),
		$form->generate_check_box("candeleteownvms", 1, $lang->can_delete_own_vms, array("checked" => $daddyobb->input['candeleteownvms'])),
		$form->generate_check_box("canmanagevms", 1, $lang->can_manage_vms, array("checked" => $daddyobb->input['canmanagevms'])),
		);
	$form_container->output_row($lang->visitor_messaging, "", "<div class=\"group_settings_bit\">".implode("</div><div class=\"group_settings_bit\">", $vm_options)."</div>");
	
	$form_container->end();
	echo "</div>";
	
	//
	// AVATAR PROFILE PCITURE AND SIGNATURE
	//
	echo "<div id=\"tab_avatar_profilepic_signature\">";	
	$form_container = new FormContainer($lang->avatar_profilepic_signature);

	$avatar_options = array(	
    $form->generate_check_box("canuploadavatars", 1, $lang->can_upload_avatars, array("checked" => $daddyobb->input['canuploadavatars'])),
		"{$lang->avatar_max_size}<br />".$form->generate_text_box('avatarmaxsize', $daddyobb->input['avatarmaxsize'], array('id' => 'avatarmaxsize', 'class' => 'field50')),
		"{$lang->avatar_max_dimensions}<br />".$form->generate_text_box('avatarmaxdimensions', $daddyobb->input['avatarmaxdimensions'], array('id' => 'avatarmaxdimensions', 'class' => 'field50')),
	);
	$form_container->output_row($lang->avatar_options, "", "<div class=\"group_settings_bit\">".implode("</div><div class=\"group_settings_bit\">", $avatar_options)."</div>");
	
	$profilepic_options = array(	
    $form->generate_check_box("canuploadprofilepics", 1, $lang->can_upload_profilepics, array("checked" => $daddyobb->input['canuploadprofilepics'])),
		"{$lang->profilepic_max_size}<br />".$form->generate_text_box('profilepicmaxsize', $daddyobb->input['profilepicmaxsize'], array('id' => 'profilepicmaxsize', 'class' => 'field50')),
		"{$lang->profilepic_max_dimensions}<br />".$form->generate_text_box('profilepicmaxdimensions', $daddyobb->input['profilepicmaxdimensions'], array('id' => 'profilepicmaxdimensions', 'class' => 'field50')),
	);
	$form_container->output_row($lang->profilepic_options, "", "<div class=\"group_settings_bit\">".implode("</div><div class=\"group_settings_bit\">", $profilepic_options)."</div>");
	
	$signature_options = array(	
		"{$lang->sig_max_chars}<br />".$form->generate_text_box('sigmaxchars', $daddyobb->input['sigmaxchars'], array('id' => 'sigmaxchars', 'class' => 'field50')),
		$form->generate_check_box("sigallowmycode", 1, $lang->sig_allow_mycode, array("checked" => $daddyobb->input['sigallowmycode'])),
		$form->generate_check_box("sigallowimgcode", 1, $lang->sig_allow_imgcode, array("checked" => $daddyobb->input['sigallowimgcode'])),
		"{$lang->sig_num_images}<br /><small class=\"input\">{$lang->sig_num_images_descr}</small><br>".$form->generate_text_box('signumimages', $daddyobb->input['signumimages'], array('id' => 'signumimages', 'class' => 'field50')),
		$form->generate_check_box("sigallowsmilies", 1, $lang->sig_allow_smilies, array("checked" => $daddyobb->input['sigallowsmilies'])),
		$form->generate_check_box("sigallowhtml", 1, $lang->sig_allow_html, array("checked" => $daddyobb->input['sigallowhtml'])),
	);
	$form_container->output_row($lang->signature_options, "", "<div class=\"group_settings_bit\">".implode("</div><div class=\"group_settings_bit\">", $signature_options)."</div>");
	...
	$calendar_options = array(
		$form->generate_check_box("canviewcalendar", 1, $lang->can_view_calendar, array("checked" => $daddyobb->input['canviewcalendar'])),
		$form->generate_check_box("canaddevents", 1, $lang->can_post_events, array("checked" => $daddyobb->input['canaddevents'])),
		$form->generate_check_box("canbypasseventmod", 1, $lang->can_bypass_event_moderation, array("checked" => $daddyobb->input['canbypasseventmod'])),
		$form->generate_check_box("canmoderateevents", 1, $lang->can_moderate_events, array("checked" => $daddyobb->input['canmoderateevents']))
	);
	...
	$wol_options = array(
		$form->generate_check_box("canviewonline", 1, $lang->can_view_whos_online, array("checked" => $daddyobb->input['canviewonline'])),
		$form->generate_check_box("canviewwolinvis", 1, $lang->can_view_invisible, array("checked" => $daddyobb->input['canviewwolinvis'])),
		$form->generate_check_box("canviewonlineips", 1, $lang->can_view_ips, array("checked" => $daddyobb->input['canviewonlineips']))
	);
	...
	$modcp_options = array(
		$form->generate_check_box("modcanannouncements", 1, $lang->mod_can_announcements, array("checked" => $daddyobb->input['modcanannouncements'])),
		$form->generate_check_box("modcanviewmodlogs", 1, $lang->mod_can_view_modlogs, array("checked" => $daddyobb->input['modcanviewmodlogs'])),
		$form->generate_check_box("modcanmanagevms", 1, $lang->mod_can_manage_vms, array("checked" => $daddyobb->input['modcanmanagevms'])),
		$form->generate_check_box("modcanmanageprofiles", 1, $lang->mod_can_manage_profiles, array("checked" => $daddyobb->input['modcanmanageprofiles'])),
		$form->generate_check_box("modcanbanusers", 1, $lang->mod_can_ban_users, array("checked" => $daddyobb->input['modcanbanusers'])),
	);
	$form_container->output_row($lang->modcp, "", "<div class=\"group_settings_bit\">".implode("</div><div class=\"group_settings_bit\">", $modcp_options)."</div>");

	$misc_options = array(
		$form->generate_check_box("canviewmemberlist", 1, $lang->can_view_member_list, array("checked" => $daddyobb->input['canviewmemberlist'])),
		$form->generate_check_box("cansendemail", 1, $lang->can_email_users, array("checked" => $daddyobb->input['cansendemail'])),
		"{$lang->max_emails_per_day}<br /><small class=\"input\">{$lang->max_emails_per_day_desc}</small><br />".$form->generate_text_box('maxemails', $daddyobb->input['maxemails'], array('id' => 'maxemails', 'class' => 'field50'))
	);
...
if($daddyobb->input['action'] == "disporder" && $daddyobb->_request_method == "post")
{
	$plugins->run_hooks("admin_user_groups_disporder");
	
	foreach($daddyobb->input['disporder'] as $gid=>$order)
	{
		$gid = intval($gid);
		$order = intval($order);
		if($gid != 0 && $order != 0)
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->request_method == "post")
	{
		if(!empty($daddyobb->input['disporder']))
		{
			foreach($daddyobb->input['disporder'] as $gid => $order)
			{
	...
	while($usergroup = $db->fetch_array($query))
	{
		...
		if($usergroup['type'] > 1)
		{
			$popup->add_item($lang->delete_group, "index.php?module=user/groups&amp;action=delete&amp;gid={$usergroup['gid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_group_deletion}')");
		}

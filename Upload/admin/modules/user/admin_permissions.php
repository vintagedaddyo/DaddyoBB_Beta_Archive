...
if(($daddyobb->input['action'] == "edit" && $daddyobb->input['uid'] == 0) || $daddyobb->input['action'] == "group" || !$daddyobb->input['action'])
{
...
if($daddyobb->input['action'] == "delete")
{
	...
	if(is_super_admin($uid) && $daddyobb->user['uid'] != $uid)
	{
	...
	if(!trim($daddyobb->input['uid']))
	{
		flash_message($lang->error_delete_no_uid, 'error');
		admin_redirect("index.php?module=user/admin_permissions");
	}
	
	$query = $db->simple_select("adminoptions", "COUNT(uid) as adminoptions", "uid = '{$daddyobb->input['uid']}'");
	...
	{
		$page->output_confirm_action("index.php?module=user/admin_permissions&amp;action=delete&amp;uid={$daddyobb->input['uid']}", $lang->confirm_perms_deletion); 
	}
}
...
if($daddyobb->input['action'] == "edit")
{
	...
	if($daddyobb->request_method == "post")
	{
		foreach($daddyobb->input['permissions'] as $module => $actions)
		{
			$no_access = 0;
			...
			{
				unset($daddyobb->input['permissions'][$module]);
			}
		}

		// Does an options row exist for this admin already?
		$query = $db->simple_select("adminoptions", "COUNT(uid) AS existing_options", "uid='".intval($daddyobb->input['uid'])."'");
		$existing_options = $db->fetch_field($query, "existing_options");
		if($existing_options > 0)
		{
			$db->update_query("adminoptions", array('permissions' => $db->escape_string(serialize($daddyobb->input['permissions']))), "uid = '".intval($daddyobb->input['uid'])."'");
		}
		else
		{
			$db->insert_query("adminoptions", array('uid' => intval($daddyobb->input['uid']), 'permissions' => $db->escape_string(serialize($daddyobb->input['permissions']))));
		}
...
if($daddyobb->input['action'] == "group")
{
	...
	while($group = $db->fetch_array($query))
	{
		...
		if($group['permissions'] != "")
		{
			...
			$popup->add_item($lang->revoke_permissions, "index.php?module=user/admin_permissions&amp;action=delete&amp;uid={$uid}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, 'Are you sure you wish to revoke this group\'s permissions?')");
...
if(!$daddyobb->input['action'])
{
	...
	while($admin = $db->fetch_array($query))
	{
		...
		$table->construct_cell(my_date($daddyobb->settings['dateformat'].", ".$daddyobb->settings['timeformat'], $admin['lastactive']), array("class" => "align_center"));
		...
		if($admin['permissions'] != "")
		{
			...
			$popup->add_item($lang->revoke_permissions, "index.php?module=user/admin_permissions&amp;action=delete&amp;uid={$admin['uid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_perms_deletion2}')");
		}
		else
		{

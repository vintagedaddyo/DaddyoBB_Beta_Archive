...
if($daddyobb->input['action'] == "add" || $daddyobb->input['action'] == "merge" || $daddyobb->input['action'] == "search" || !$daddyobb->input['action'])
{
...
require DADDYOBB_ADMIN_DIR."inc/functions_view_manager.php";
if($daddyobb->input['action'] == "views")
{
	view_manager("index.php?module=user/users", "user", $user_view_fields, $sort_options, "user_search_conditions");
}

if($daddyobb->input['action'] == "avatar_gallery")
{
	...
	if($daddyobb->input['avatar'])
	{
		if(file_exists("../".$daddyobb->settings['avatardir']."/".$daddyobb->input['avatar']))
		{
			$dimensions = @getimagesize("../".$daddyobb->settings['avatardir']."/".$daddyobb->input['avatar']);
			$updated_avatar = array(
				"avatar" => $db->escape_string($daddyobb->settings['avatardir']."/".$daddyobb->input['avatar']),
	...
	$gallery = str_replace(array("..", "\x0"), "", $daddyobb->input['gallery']);
	
	$breadcrumb = "<a href=\"index.php?module=user/users&amp;action=avatar_gallery&amp;uid={$user['uid']}\">Default Gallery</a>";

	$daddyobb->settings['avatardir'] = "../".$daddyobb->settings['avatardir'];

	if(!is_dir($daddyobb->settings['avatardir']) && is_dir(DADDYOBB_ROOT."/images/avatars/"))
	{
		$daddyobb->settings['avatardir'] = "../images/avatars/";
	}
	...
	{
		$path = $gallery."/";
		$real_path = $daddyobb->settings['avatardir']."/".$path;
		if(is_dir($real_path))
		{
	...
	{
		$path = "";
		$real_path = $daddyobb->settings['avatardir'];
	}
	...
	if(is_array($sub_galleries))
	{
		foreach($sub_galleries as $gallery)
		{
			...
			{
				$gallery['thumb'] = "{$daddyobb->settings['avatardir']}/{$gallery['thumb']}";
			}
	...
	if(is_array($avatars))
	{
		foreach($avatars as $avatar)
		{
			...
			$top = ceil((80-$scaled_dimensions['height'])/2);
			$left = ceil((80-$scaled_dimensions['width'])/2);
			echo "<li><a href=\"index.php?module=user/users&amp;action=avatar_gallery&amp;uid={$user['uid']}&amp;avatar={$avatar['path']}\"><span class=\"image\"><img src=\"{$daddyobb->settings['avatardir']}/{$avatar['path']}\" alt=\"\" style=\"margin-top: {$top}px;\" height=\"{$scaled_dimensions['height']}\" width=\"{$scaled_dimensions['width']}\" /></span><span class=\"title\">{$avatar['friendly_name']}</span></a></li>\n";
		}
	}
	echo "</ul>\n";
	echo "</div>";
...
if($daddyobb->input['action'] == "activate_user")
{
...
if($daddyobb->input['action'] == "add")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(is_array($daddyobb->input['additionalgroups']))
		{
			foreach($daddyobb->input['additionalgroups'] as $gid)
			{
				if($gid == $daddyobb->input['usergroup'])
				{
					unset($daddyobb->input['additionalgroups'][$gid]);
				}
			}
			$additionalgroups = implode(",", $daddyobb->input['additionalgroups']);
		}
		else
		{
			$additionalgroups = '';
		...
		$new_user = array(
			...
			"password2" => $daddyobb->input['confirm_password'],
			"email" => $daddyobb->input['email'],
			"email2" => $daddyobb->input['email'],
			"usergroup" => $daddyobb->input['usergroup'],
			"additionalgroups" => $additionalgroups,
			"displaygroup" => $daddyobb->input['displaygroup'],
	...
	$form_container->output_row($lang->password." <em>*</em>", "", $form->generate_password_box('password', $daddyobb->input['password'], array('id' => 'password')), 'password');
	$form_container->output_row($lang->confirm_password." <em>*</em>", "", $form->generate_password_box('confirm_password', $daddyobb->input['confirm_password'], array('id' => 'confirm_new_password')), 'confirm_new_password');
	$form_container->output_row($lang->email_address." <em>*</em>", "", $form->generate_text_box('email', $daddyobb->input['email'], array('id' => 'email')), 'email');
	...
	$form_container->output_row($lang->primary_user_group." <em>*</em>", "", $form->generate_select_box('usergroup', $options, $daddyobb->input['usergroup'], array('id' => 'usergroup')), 'usergroup');
	$form_container->output_row($lang->additional_user_groups, $lang->additional_user_groups_desc, $form->generate_select_box('additionalgroups[]', $options, $daddyobb->input['additionalgroups'], array('id' => 'additionalgroups', 'multiple' => true, 'size' => 5)), 'additionalgroups');
	$form_container->output_row($lang->display_user_group." <em>*</em>", "", $form->generate_select_box('displaygroup', $display_group_options, $daddyobb->input['displaygroup'], array('id' => 'displaygroup')), 'displaygroup');

	// Output custom profile fields - required
	output_custom_profile_fields($profile_fields['required'], $daddyobb->input['profile_fields'], $form_container, $form);
...
if($daddyobb->input['action'] == "edit")
{
	...
	if($daddyobb->request_method == "post")
	{
		if(is_super_admin($daddyobb->input['uid']) && $daddyobb->user['uid'] != $daddyobb->input['uid'] && !is_super_admin($daddyobb->user['uid']))
		{
		...
		$updated_user = array(
			...
			"additionalgroups" => $additionalgroups,
			"displaygroup" => $daddyobb->input['displaygroup'],
			"postnum" => $daddyobb->input['postnum'],
			"usertitle" => $daddyobb->input['usertitle'],
			"timezone" => $daddyobb->input['timezone'],
		...
		if($user['usergroup'] == 5 && $daddyobb->input['usergroup'] != 5)
		{
			if($user['coppauser'] == 1)
			{
		...
		if($daddyobb->input['new_password'])
		{
			$updated_user['password'] = $daddyobb->input['new_password'];
			$updated_user['password2'] = $daddyobb->input['confirm_new_password'];
		}
		...
		{
			$updated_user['options']['tpp'] = intval($daddyobb->input['tpp']);
		}

		if($daddyobb->settings['userpppoptions'])
		{
			$updated_user['options']['ppp'] = intval($daddyobb->input['ppp']);
		}
		...
		else
		{
			...
			else if($daddyobb->input['avatar_url'] && $daddyobb->input['avatar_url'] != $user['avatar'])
			{
				$daddyobb->input['avatar_url'] = preg_replace("#script:#i", "", $daddyobb->input['avatar_url']);
				$daddyobb->input['avatar_url'] = htmlspecialchars($daddyobb->input['avatar_url']);
				$ext = get_extension($daddyobb->input['avatar_url']);

				// Copy the avatar to the local server (work around remote URL access disabled for getimagesize)
				$file = fetch_remote_file($daddyobb->input['avatar_url']);
				if(!$file)
				{
				...
				if(empty($avatar_error))
				{
					...
					$extra_user_updates = array(
						"avatar" => $db->escape_string($daddyobb->input['avatar_url']),
						"avatardimensions" => $avatar_dimensions,
						"avatartype" => "remote"
					);
					remove_avatars($user['uid']);
			...
			if(!$errors)
			{
				...
				if($daddyobb->input['update_posts'] == 'enable' || $daddyobb->input['update_posts'] == 'disable')
				{
					$update_signature = array(
						'includesig' => ($daddyobb->input['update_posts'] == 'enable' ? 1 : 0)
					);
				...
				// Log admin action
				log_admin_action($user['uid'], $daddyobb->input['username']);
	...
	if(!$errors)
	{
		...
		$daddyobb->input['profile_fields'] = $db->fetch_array($query);
	}
	...
	if($user['lastactive'])
	{
		$last_active = my_date($daddyobb->settings['dateformat'], $user['lastactive']).", ".my_date($daddyobb->settings['timeformat'], $user['lastactive']);
	}
	else
	{
		$last_active = $lang->never;
	}
	$reg_date = my_date($daddyobb->settings['dateformat'], $user['regdate']).", ".my_date($daddyobb->settings['timeformat'], $user['regdate']);
	if($user['dst'] == 1)
	{
	...
	$local_time = gmdate($daddyobb->settings['dateformat'], TIME_NOW + ($timezone * 3600)).", ".gmdate($daddyobb->settings['timeformat'], TIME_NOW + ($timezone * 3600));
	...
	if($user_permissions['usereputationsystem'] == 1 && $daddyobb->settings['enablereputation'] == 1)
	{
	...
	if($daddyobb->settings['enablewarningsystem'] != 0 && $user_permissions['canreceivewarnings'] != 0)
	{
		$warning_level = round($user['warningpoints']/$daddyobb->settings['maxwarningpoints']*100);
	...
	$form_container->output_row($lang->username." <em>*</em>", "", $form->generate_text_box('username', $daddyobb->input['username'], array('id' => 'username')), 'username');
	$form_container->output_row($lang->new_password, $lang->new_password_desc, $form->generate_password_box('new_password', $daddyobb->input['new_password'], array('id' => 'new_password')), 'new_password');
	$form_container->output_row($lang->confirm_new_password, $lang->new_password_desc, $form->generate_password_box('confirm_new_password', $daddyobb->input['confirm_new_password'], array('id' => 'confirm_new_password')), 'confirm_new_password');
	$form_container->output_row($lang->email_address." <em>*</em>", "", $form->generate_text_box('email', $daddyobb->input['email'], array('id' => 'email')), 'email');
	...
	if(!is_array($daddyobb->input['additionalgroups']))
	{
		$daddyobb->input['additionalgroups'] = explode(',', $daddyobb->input['additionalgroups']);
	}

	$form_container->output_row($lang->primary_user_group." <em>*</em>", "", $form->generate_select_box('usergroup', $options, $daddyobb->input['usergroup'], array('id' => 'usergroup')), 'usergroup');
	$form_container->output_row($lang->additional_user_groups, $lang->additional_user_groups_desc, $form->generate_select_box('additionalgroups[]', $options, $daddyobb->input['additionalgroups'], array('id' => 'additionalgroups', 'multiple' => true, 'size' => 5)), 'additionalgroups');
	$form_container->output_row($lang->display_user_group." <em>*</em>", "", $form->generate_select_box('displaygroup', $display_group_options, $daddyobb->input['displaygroup'], array('id' => 'displaygroup')), 'displaygroup');
	$form_container->output_row($lang->post_count." <em>*</em>", "", $form->generate_text_box('postnum', $daddyobb->input['postnum'], array('id' => 'postnum')), 'postnum');
	...
	$form_container->output_row($lang->custom_user_title, $lang->custom_user_title_desc, $form->generate_text_box('usertitle', $daddyobb->input['usertitle'], array('id' => 'usertitle')), 'usertitle');
	$form_container->output_row($lang->website, "", $form->generate_text_box('website', $daddyobb->input['website'], array('id' => 'website')), 'website');
	$form_container->output_row($lang->icq_number, "", $form->generate_text_box('icq', $daddyobb->input['icq'], array('id' => 'icq')), 'icq');
	$form_container->output_row($lang->aim_handle, "", $form->generate_text_box('aim', $daddyobb->input['aim'], array('id' => 'aim')), 'aim');
	$form_container->output_row($lang->yahoo_messanger_handle, "", $form->generate_text_box('yahoo', $daddyobb->input['yahoo'], array('id' => 'yahoo')), 'yahoo');
	$form_container->output_row($lang->msn_messanger_handle, "", $form->generate_text_box('msn', $daddyobb->input['msn'], array('id' => 'msn')), 'msn');
	// Birthday

	// Output custom profile fields - optional
	output_custom_profile_fields($profile_fields['optional'], $daddyobb->input['profile_fields'], $form_container, $form);
	...
	$login_options = array(
		$form->generate_check_box("invisible", 1, $lang->hide_from_whos_online, array("checked" => $daddyobb->input['invisible'])),
		$form->generate_check_box("remember", 1, $lang->remember_login_details, array("checked" => $daddyobb->input['remember']))
	);
	...
	$messaging_options = array(
		$form->generate_check_box("allownotices", 1, $lang->recieve_admin_emails, array("checked" => $daddyobb->input['allownotices'])),
		$form->generate_check_box("hideemail", 1, $lang->hide_email_from_others, array("checked" => $daddyobb->input['hideemail'])),
		$form->generate_check_box("receivepms", 1, $lang->recieve_pms_from_others, array("checked" => $daddyobb->input['receivepms'])),
		$form->generate_check_box("pmnotice", 1, $lang->alert_new_pms, array("checked" => $daddyobb->input['pmnotice'])),
		$form->generate_check_box("pmnotify", 1, $lang->email_notify_new_pms, array("checked" => $daddyobb->input['pmnotify'])),
		"<label for=\"subscriptionmethod\">{$lang->default_thread_subscription_mode}:</label><br />".$form->generate_select_box("subscriptionmethod", array($lang->do_not_subscribe, $lang->no_email_notification, $lang->instant_email_notification), $daddyobb->input['subscriptionmethod'], array('id' => 'subscriptionmethod'))
	);
	...
	$date_options = array(
		"<label for=\"dateformat\">{$lang->date_format}:</label><br />".$form->generate_select_box("dateformat", $date_format_options, $daddyobb->input['dateformat'], array('id' => 'dateformat')),
		"<label for=\"dateformat\">{$lang->time_format}:</label><br />".$form->generate_select_box("timeformat", $time_format_options, $daddyobb->input['timeformat'], array('id' => 'timeformat')),
		"<label for=\"timezone\">{$lang->time_zone}:</label><br />".build_timezone_select("timezone", $daddyobb->input['timezone']),
		"<label for=\"dstcorrection\">{$lang->daylight_savings_time_correction}:</label><br />".$form->generate_select_box("dstcorrection", array(2 => $lang->automatically_detect, 1 => $lang->always_use_dst_correction, 0 => $lang->never_use_dst_correction), $daddyobb->input['dstcorrection'], array('id' => 'dstcorrection'))
	);
	...
	$forum_options = array(
		"<label for=\"tpp\">{$lang->threads_per_page}:</label><br />".$form->generate_select_box("tpp", $tpp_options, $daddyobb->input['tpp'], array('id' => 'tpp')),
		"<label for=\"daysprune\">{$lang->default_thread_age_view}:</label><br />".$form->generate_select_box("daysprune", $thread_age_options, $daddyobb->input['daysprune'], array('id' => 'daysprune'))
	);
	...
	$thread_options = array(
		$form->generate_check_box("showsigs", 1, $lang->display_users_sigs, array("checked" => $daddyobb->input['showsigs'])),
		$form->generate_check_box("showavatars", 1, $lang->display_users_avatars, array("checked" => $daddyobb->input['showavatars'])),
		$form->generate_check_box("showquickreply", 1, $lang->show_quick_reply, array("checked" => $daddyobb->input['showquickreply'])),
		"<label for=\"ppp\">{$lang->posts_per_page}:</label><br />".$form->generate_select_box("ppp", $ppp_options, $daddyobb->input['ppp'], array('id' => 'ppp')),
		"<label for=\"threadmode\">{$lang->default_thread_view_mode}:</label><br />".$form->generate_select_box("threadmode", array("" => $lang->use_default, "linear" => $lang->linear_mode, "threaded" => $lang->threaded_mode), $daddyobb->input['threadmode'], array('id' => 'threadmode'))
	);
	...
	$other_options = array(
		$form->generate_check_box("showredirect", 1, $lang->show_redirect, array("checked" => $daddyobb->input['showredirect'])),
		$form->generate_check_box("showcodebuttons", "1", $lang->show_code_buttons, array("checked" => $daddyobb->input['showcodebuttons'])),
		"<label for=\"style\">{$lang->theme}:</label><br />".build_theme_select("style", $daddyobb->input['style'], 0, "", 1),
		"<label for=\"language\">{$lang->board_language}:</label><br />".$form->generate_select_box("language", $languages, $daddyobb->input['language'], array('id' => 'language'))
	);
	...
	// SIGNATURE EDITOR
	//
	$signature_editor = $form->generate_text_area("signature", $daddyobb->input['signature'], array('id' => 'signature', 'rows' => 15, 'cols' => '70', 'style' => 'width: 95%'));
	$sig_smilies = $lang->off;
	if($daddyobb->settings['sigsmilies'] == 1)
	{
		$sig_smilies = $lang->on;
	}
	$sig_mycode = $lang->off;
	if($daddyobb->settings['sigmycode'] == 1)
	{
		$sig_mycode = $lang->on;
		$signature_editor .= build_mycode_inserter("signature");
	}
	$sig_html = $lang->off;
	if($daddyobb->settings['sightml'] == 1)
	{
		$sig_html = $lang->on;
	}
	$sig_imgcode = $lang->off;
	if($daddyobb->settings['sigimgcode'] == 1)
	{
		$sig_imgcode = $lang->on;
	}
	...
	if($user['avatartype'] == "upload" || stristr($user['avatar'], $daddyobb->settings['avataruploadpath']))
	{
		$current_avatar_msg = "<br /><strong>{$lang->user_current_using_uploaded_avatar}</strong>";
	}
	else if($user['avatartype'] == "gallery" || stristr($user['avatar'], $daddyobb->settings['avatardir']))
	{
	...
	if($errors)
	{
		$avatar_url = $daddyobb->input['avatar_url'];
	}

	if($daddyobb->settings['maxavatardims'] != "")
	{
		list($max_width, $max_height) = explode("x", my_strtolower($daddyobb->settings['maxavatardims']));
		$max_size = "<br />{$lang->max_dimensions_are} {$max_width}x{$max_height}";
	}

	if($daddyobb->settings['avatarsize'])
	{
		$maximum_size = get_friendly_size($daddyobb->settings['avatarsize']*1024);
...
if($daddyobb->input['action'] == "referrers")
{
	...
	$sub_tabs['referrers'] = array(
		...
		'link' => "index.php?module=user/users&amp;action=referrers&amp;uid={$daddyobb->input['uid']}",
	...
	if($daddyobb->input['type'])
	{
		$admin_view['view_type'] = $daddyobb->input['type'];
	}
	
	$admin_view['conditions'] = unserialize($admin_view['conditions']);
	$admin_view['conditions']['referrer'] = $daddyobb->input['uid'];
...
if($daddyobb->input['action'] == "ipaddresses")
{
	...
	$sub_tabs['ipaddresses'] = array(
		...
		'link' => "index.php?module=user/users&amp;action=ipaddresses&amp;uid={$daddyobb->input['uid']}",
		'description' => $lang->show_ip_addresses_desc
	);
	
	$page->output_nav_tabs($sub_tabs, 'ipaddresses');
	
	$query = $db->simple_select("users", "uid, regip, username, lastip", "uid='{$daddyobb->input['uid']}'", array('limit' => 1));
	$user = $db->fetch_array($query);
...
"index.php?module=user/users&amp;action=search&amp;results=1&amp;conditions=".urlencode(serialize(array("regip" => $user['lastip']))));
	...
	$query = $db->simple_select("posts", "DISTINCT ipaddress", "uid='{$daddyobb->input['uid']}'");
...
if($daddyobb->input['action'] == "merge")
{
	$plugins->run_hooks("admin_user_users_merge");
	
	if($daddyobb->request_method == "post")
	{
		$query = $db->simple_select("users", "*", "LOWER(username)='".$db->escape_string(my_strtolower($daddyobb->input['source_username']))."'");
		...
		$query = $db->simple_select("users", "*", "LOWER(username)='".$db->escape_string(my_strtolower($daddyobb->input['destination_username']))."'");
	...
	$form_container->output_row($lang->source_account." <em>*</em>", $lang->source_account_desc, $form->generate_text_box('source_username', $daddyobb->input['source_username'], array('id' => 'source_username')), 'source_username');
	$form_container->output_row($lang->destination_account." <em>*</em>", $lang->destination_account_desc, $form->generate_text_box('destination_username', $daddyobb->input['destination_username'], array('id' => 'destination_username')), 'destination_username');
	$form_container->end();
...
if($daddyobb->input['action'] == "search")
{
	...
	if($daddyobb->request_method == "post" || $daddyobb->input['results'] == 1)
	{
		// Build view options from incoming search options
		if($daddyobb->input['vid'])
		{
			$query = $db->simple_select("adminviews", "*", "vid='".intval($daddyobb->input['vid'])."'");
			$admin_view = $db->fetch_array($query);
			// View does not exist or this view is private and does not belong to the current user
			if(!$admin_view['vid'] || ($admin_view['visibility'] == 1 && $admin_view['uid'] != $daddyobb->user['uid']))
			{
				unset($admin_view);
			}
		}

		if($daddyobb->input['search_id'] && $admin_session['data']['user_views'][$daddyobb->input['search_id']])
		{
			$admin_view = $admin_session['data']['user_views'][$daddyobb->input['search_id']];
		...
		if($daddyobb->input['conditions'])
		{
			$admin_view['conditions'] = $daddyobb->input['conditions'];
		}
		
		if($daddyobb->input['sortby'])
		{
			$admin_view['sortby'] = $daddyobb->input['sortby'];
		}
		
		if($daddyobb->input['perpage'])
		{
			$admin_view['perpage'] = $daddyobb->input['perpage'];
		}
		
		if($daddyobb->input['order'])
		{
			$admin_view['sortorder'] = $daddyobb->input['order'];
		}
		
		if($daddyobb->input['displayas'])
		{
			$admin_view['view_type'] = $daddyobb->input['displayas'];
		}
	...
	if(!$daddyobb->input['displayas'])
	{
		$daddyobb->input['displayas'] = "card";
	}

	$form = new Form("index.php?module=user/users&amp;action=search", "post");

	user_search_conditions($daddyobb->input, $form);
	...
	$form_container->output_row($lang->sort_results_by, "", $form->generate_select_box('sortby', $sort_options, $daddyobb->input['sortby'], array('id' => 'sortby'))." {$lang->in} ".$form->generate_select_box('order', $sort_directions, $daddyobb->input['order'], array('id' => 'order')), 'sortby');
	$form_container->output_row($lang->results_per_page, "", $form->generate_text_box('perpage', $daddyobb->input['perpage'], array('id' => 'perpage')), 'perpage');
	$form_container->output_row($lang->display_results_as, "", $form->generate_radio_button('displayas', 'table', $lang->table, array('checked' => ($daddyobb->input['displayas'] != "card" ? true : false)))."<br />".$form->generate_radio_button('displayas', 'card', $lang->business_card, array('checked' => ($daddyobb->input['displayas'] == "card" ? true : false))));
	$form_container->end();
...
if(!$daddyobb->input['action'])
{
	...
	$query = $db->simple_select("adminviews", "*", "type='user' AND (visibility=2 OR uid={$daddyobb->user['uid']})", array("order_by" => "title"));
...
{
	global $daddyobb, $db, $cache, $lang, $user_view_fields, $page;
    ...
    {
        $view['conditions']['username'] = $daddyobb->input['username'];
    }
	if($view['vid'])
	{
	...
	if($daddyobb->input['username'])
	{
		$view['url'] .= "&amp;username=".urlencode(htmlspecialchars_uni($daddyobb->input['username']));
	}
	...
	else
	{
		...
		if($daddyobb->input['page'])
		{
			$start = ($daddyobb->input['page'] - 1) * $view['perpage'];
		}
		else
		{
			$start = 0;
		...
		while($user = $db->fetch_array($query))
		{
			...
			$user['view']['regdate'] = my_date($daddyobb->settings['dateformat'], $user['regdate']).", ".my_date($daddyobb->settings['timeformat'], $user['regdate']);
			$user['view']['lastactive'] = my_date($daddyobb->settings['dateformat'], $user['lastactive']).", ".my_date($daddyobb->settings['timeformat'], $user['lastactive']);
			...
			$popup->add_item($lang->delete_user, "index.php?module=user/users&amp;action=delete&amp;uid={$user['uid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->user_deletion_confirmation}')");
			...
			// Fetch the reputation for this user
			if($usergroups[$user['usergroup']]['usereputationsystem'] == 1 && $daddyobb->settings['enablereputation'] == 1)
			{
			...
			if($daddyobb->settings['enablewarningsystem'] != 0 && $usergroups[$user['usergroup']]['canreceivewarnings'] != 0)
			{
				$warning_level = round($user['warningpoints']/$daddyobb->settings['maxwarningpoints']*100);
				if($warning_level > 100)
				{
	...
	{
		$switch_url .= "&amp;page=".intval($daddyobb->input['page']);
	}
	if($view['view_type'] != "card")
	{
	...
	{
		$pagination = draw_admin_pagination($daddyobb->input['page'], $view['perpage'], $num_results, $view['url']."&amp;type={$view['view_type']}");

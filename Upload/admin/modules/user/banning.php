...
if($daddyobb->input['action'] == "lift")
{
	...
	$query = $db->simple_select("banned", "*", "uid='{$daddyobb->input['uid']}'");
	...
	if(is_super_admin($user['uid']) && ($daddyobb->user['uid'] != $user['uid'] && !is_super_admin($daddyobb->user['uid'])))
	{
	...
	if($daddyobb->request_method == "post")
	{
		...
		// Log admin action
		log_admin_action($daddyobb->input['uid'], $user['username']);
...
if($daddyobb->input['action'] == "edit")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		{
			// Ban the user
			if($daddyobb->input['bantime'] == '---')
			{
				$lifted = 0;
			}
			else
			{
				$lifted = ban_date2timestamp($daddyobb->input['bantime'], $ban['dateline']);
			}

			if(count($banned_groups) == 1)
			{
				$group = array_keys($banned_groups);
				$daddyobb->input['usergroup'] = $group[0];
			}

			$update_array = array(
				'gid' => intval($daddyobb->input['usergroup']),
				'dateline' => TIME_NOW,
				'bantime' => $db->escape_string($daddyobb->input['bantime']),
	...
	$form_container->output_row($lang->ban_reason, "", $form->generate_text_box('reason', $daddyobb->input['reason'], array('id' => 'reason')), 'reason');
	if(count($banned_groups) > 1)
	{
		$form_container->output_row($lang->ban_group, $lang->ban_group_desc, $form->generate_select_box('usergroup', $banned_groups, $daddyobb->input['usergroup'], array('id' => 'usergroup')), 'usergroup');
	}
	...
	$form_container->output_row($lang->ban_time, "", $form->generate_select_box('bantime', $length_list, $daddyobb->input['bantime'], array('id' => 'bantime')), 'bantime');	
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->request_method == "post")
	{
		$query = $db->simple_select("users", "uid, usergroup, additionalgroups, displaygroup, username", "LOWER(username)='".$db->escape_string(my_strtolower($daddyobb->input['username']))."'", array('limit' => 1));
		...
		// Is the user we're trying to ban a super admin and we're not?
		else if(is_super_admin($user['uid']) && !is_super_admin($daddyobb->user['uid']))
		{
			$errors[] = $lang->error_no_perm_to_ban;
		}
		else
		...
		if(!$errors)
		{
			...
			{
				$lifted = ban_date2timestamp($daddyobb->input['bantime']);
			}
	...
	while($ban = $db->fetch_array($query))
	{
		...
		$ban_date = my_date($daddyobb->settings['dateformat'], $ban['dateline']);
		...
		else
		{
			...
			$lifts_on = my_date($daddyobb->settings['dateformat'], $ban['lifted']);
		}
		...
		$table->construct_cell("<a href=\"index.php?module=user/banning&amp;action=lift&amp;uid={$ban['uid']}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_lift_ban}');\">{$lang->lift}</a>", array("class" => "align_center"));
		$table->construct_row();
	}
	...
	if($daddyobb->input['uid'] && !$daddyobb->input['username'])
	{
		$user = get_user($daddyobb->input['uid']);
		$daddyobb->input['username'] = $user['username'];
	}
	
	$form_container = new FormContainer($lang->ban_a_user);
	$form_container->output_row($lang->ban_username, $lang->autocomplete_enabled, $form->generate_text_box('username', $daddyobb->input['username'], array('id' => 'username')), 'username');
	$form_container->output_row($lang->ban_reason, "", $form->generate_text_box('reason', $daddyobb->input['reason'], array('id' => 'reason')), 'reason');
	if(count($banned_groups) > 1)
	{
		$form_container->output_row($lang->ban_group, $lang->add_ban_group_desc, $form->generate_select_box('usergroup', $banned_groups, $daddyobb->input['usergroup'], array('id' => 'usergroup')), 'usergroup');
	}
	foreach($ban_times as $time => $period)
	{
		if($time != "---")

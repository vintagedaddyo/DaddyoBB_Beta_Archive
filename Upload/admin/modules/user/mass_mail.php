...
if($daddyobb->input['action'] == "send" || $daddyobb->input['action'] == "archive" || !$daddyobb->input['action'])
{
...
{	
	...
	$query = $db->simple_select("massemails", "*", "mid='".intval($daddyobb->input['mid'])."'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		if($daddyobb->input['delivery_type'] == "now")
		{
		...
		{
			if(strstr($daddyobb->input['deliverytime_time'], "pm"))
			{
				$daddyobb->input['deliveryhour'] += 12;
			}
			
			$exploded = explode(':', $daddyobb->input['endtime_time']);
			$daddyobb->input['deliveryhour'] = intval($exploded[0]);
			
			$exploded = explode(' ', $exploded[1]);
			$daddyobb->input['deliveryminute'] = intval($exploded[0]);
			
			$delivery_date = gmmktime($daddyobb->input['deliveryhour'], $daddyobb->input['deliveryminute'], 0, $daddyobb->input['endtime_month'], $daddyobb->input['endtime_day'], $daddyobb->input['endtime_year']) + $daddyobb->user['timezone']*3600;
		...
		$member_query = build_mass_mail_query($daddyobb->input['conditions']);
		...
		if(!trim($daddyobb->input['subject']))
		{
			$errors[] = $lang->error_missing_subject;
		}
		
		if($daddyobb->input['type'] == 1)
		{
			if(!$daddyobb->input['message'])
			{
		...
		{
			if($daddyobb->input['format'] == 2 && $daddyobb->input['automatic_text'] == 0 && !$daddyobb->input['message'])
			{
				$errors[] = $lang->error_missing_plain_text;
			}
			
			if(($daddyobb->input['format'] == 1 || $daddyobb->input['format'] == 2) && !$daddyobb->input['htmlmessage'])
			{
				$errors[] = $lang->error_missing_html;
			}
			else if($daddyobb->input['format'] == 0 && !$daddyobb->input['message'])
			{
				$errors[] = $lang->error_missing_plain_text;
			}
		}					
		...
		if(!$errors)
		{
			...
			if($daddyobb->input['type'] == 1)
			{
				$daddyobb->input['format'] = 0;
				$daddyobb->input['htmlmessage'] = '';
			}
			// Sending via email
			else
			{
				// Do we need to generate a text based version?
				if($daddyobb->input['format'] == 2 && $daddyobb->input['automatic_text'])
				{
					$daddyobb->input['message'] = create_text_message($daddyobb->input['htmlmessage']);
				}
				else if($daddyobb->input['format'] == 1)
				{
					$daddyobb->input['message'] = '';
				}
				else if($daddyobb->input['format'] == 0)
				{
					$daddyobb->input['htmlmessage'] = '';
				}
			}
			...
			$updated_email = array(
				...
				"totalcount" => $num,
				"conditions" => $db->escape_string(serialize($daddyobb->input['conditions'])),
				"message" => $db->escape_string($daddyobb->input['message']),
				"subject" => $db->escape_string($daddyobb->input['subject']),
				"htmlmessage" => $db->escape_string($daddyobb->input['htmlmessage']),
				"format" => intval($daddyobb->input['format']),
...
if($daddyobb->input['action'] == "send")
{
	...
	if($daddyobb->input['step'])
	{
		$query = $db->simple_select("massemails", "*", "status=0 and mid='".intval($daddyobb->input['mid'])."'");
		$email = $db->fetch_array($query);
		if(!$email['mid'] && $daddyobb->input['step'] != 1)
		{
			flash_message($lang->error_invalid_mid, 'error');
			admin_redirect("index.php?module=user/mass_mail");
		}
	...
	if($daddyobb->input['step'] == 4)
	{
		...
		{
			$format_preview .= "{$lang->text_based} - <a href=\"#\" onclick=\"javascript:DaddyoBB.popupWindow('index.php?module=user/mass_mail&amp;action=preview&amp;mid={$email['mid']}&amp;format=text', 'preview', 450, 450);\">{$lang->preview}</a>";
		}
		if($email['format'] == 2)
		{
		...
		{
			$format_preview.= "{$lang->html_based} - <a href=\"#\" onclick=\"javascript:DaddyoBB.popupWindow('index.php?module=user/mass_mail&amp;action=preview&amp;mid={$email['mid']}', 'preview', 450, 450);\">{$lang->preview}</a>";
		}
	...
	if(!$daddyobb->input['step'] || $daddyobb->input['step'] == 1)
	{
		if($daddyobb->request_method == "post")
		{
			if(!trim($daddyobb->input['subject']))
			...
			if(!$errors)
			{
				...
				if($daddyobb->input['format'] == 2 && $daddyobb->input['automatic_text'] == 1)
				{
					$next = 2;
				}
				else
		...
		if($daddyobb->input['type'] == 0)
		{
			$type_email_checked = true;
			$type_pm_checked = false;
		}
		else if($daddyobb->input['type'] == 1)
		{
			$type_email_checked = false;
			$type_pm_checked = true;
		}
		...
		if($daddyobb->input['format'] == 2)
		{
			if($daddyobb->input['automatic_text'] && !$email['mid'])
			{
				$automatic_text_check = true;
				$text_display = 'display: none';
				$automatic_display = 'display: none;';
		...
		else if($daddyobb->input['format'] == 1 && $daddyobb->input['type'] != 1)
		{
			$text_display = 'display: none;';
		}
		else if($daddyobb->input['format'] == 0 || $daddyobb->input['type'] == 1)
		{
			$html_display = 'display: none';
		}
...
if($daddyobb->input['action'] == "preview")
{
	$query = $db->simple_select("massemails", "*", "mid='".intval($daddyobb->input['mid'])."'");
	$mass_email = $db->fetch_array($query);
	...
	if($daddyobb->input['format'] == 'text' || !$mass_email['message'])
	{
...
if($daddyobb->input['action'] == "resend")
{
...
if($daddyobb->input['action'] == "cancel")
{
...
if($daddyobb->input['action'] == "archive")
{
	...
	while($email = $db->fetch_array($query))
	{
		...
		$delivery_date = my_date($daddyobb->settings['dateformat'], $email['senddate']);
		...
		$table->construct_cell("<a href=\"index.php?module=user/mass_mail&amp;action=delete&amp;mid={$email['mid']}&amp;my_post_key={$daddyobb->post_code}&amp;archive=1\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_deletion_confirmation}')\">{$lang->delete}</a>", array("width" => 100, "class" => "align_center"));
...
if(!$daddyobb->input['action'])
{
	...
	while($email = $db->fetch_array($query))
	{
		...
		if(TIME_NOW >= $email['senddate'] && $email['status'] > 1)
		{
			$table->construct_cell("<a href=\"index.php?module=user/mass_mail&amp;action=cancel&amp;mid={$email['mid']}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_cancel_confirmation}')\">{$lang->cancel}</a>", array("width" => 100, "colspan" => 2, "class" => "align_center"));
		}
		else
		{
			...
			$table->construct_cell("<a href=\"index.php?module=user/mass_mail&amp;action=delete&amp;mid={$email['mid']}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->mass_mail_deletion_confirmation}')\">{$lang->delete}</a>", array("width" => 100, "class" => "align_center"));
		}
		$table->construct_row();
	}

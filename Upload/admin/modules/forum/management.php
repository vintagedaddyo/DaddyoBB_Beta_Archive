...
if($daddyobb->input['action'] == "add" || $daddyobb->input['action'] == "edit" || $daddyobb->input['action'] == "copy" || $daddyobb->input['action'] == "permissions" || !$daddyobb->input['action'])
{
	if($daddyobb->input['fid'] && ($daddyobb->input['action'] == "management" || $daddyobb->input['action'] == "edit" || $daddyobb->input['action'] == "copy" || !$daddyobb->input['action']))
	{
		$sub_tabs['view_forum'] = array(
			'title' => $lang->view_forum,
			'link' => "index.php?module=forum/management&amp;fid=".$daddyobb->input['fid'],
			'description' => $lang->view_forum_desc
		);
	
		$sub_tabs['add_child_forum'] = array(
			'title' => $lang->add_child_forum,
			'link' => "index.php?module=forum/management&amp;action=add&amp;pid=".$daddyobb->input['fid'],
			'description' => $lang->add_child_forum_desc
		);
		
		$sub_tabs['edit_forum_settings'] = array(
			'title' => $lang->edit_forum_settings,
			'link' => "index.php?module=forum/management&amp;action=edit&amp;fid=".$daddyobb->input['fid'],
			'description' => $lang->edit_forum_settings_desc
		);
	
		$sub_tabs['copy_forum'] = array(
			'title' => $lang->copy_forum,
			'link' => "index.php?module=forum/management&amp;action=copy&amp;fid=".$daddyobb->input['fid'],
...
if($daddyobb->input['action'] == "copy")
{
	$plugins->run_hooks("admin_forum_management_copy");
	
	if($daddyobb->request_method == "post")
	{
		$from = intval($daddyobb->input['from']);
		$to = intval($daddyobb->input['to']);
		...
		if($to == -1)
		{
			...
			if($daddyobb->input['pid'] == 0 && $daddyobb->input['type'] == 'f')
			{
			...
			if(!$errors)
			{
				...
				$new_forum['name'] = $db->escape_string($daddyobb->input['title']);
				$new_forum['description'] = $db->escape_string($daddyobb->input['description']);
				$new_forum['type'] = $db->escape_string($daddyobb->input['type']);
				$new_forum['pid'] = intval($daddyobb->input['pid']);
		...
		elseif($daddyobb->input['copyforumsettings'] == 1)
		{
		...
		{
			// Copy permissions
			if(is_array($daddyobb->input['copygroups']) && count($daddyobb->input['copygroups'] > 0))
			{
				foreach($daddyobb->input['copygroups'] as $gid)
				{
					$groups[] = intval($gid);
				}
	...
	{		
		...
		{
			$copy_data['pid'] = intval($daddyobb->input['pid']);
		}
		$copy_data['disporder'] = "1";
		$copy_data['from'] = $daddyobb->input['fid'];
	...
	$form_container->output_row($lang->copy_user_group_permissions, $lang->copy_user_group_permissions_desc, $form->generate_select_box('copygroups[]', $usergroups, $daddyobb->input['copygroups'], array('id' => 'copygroups', 'multiple' => true, 'size' => 5)), 'copygroups');
...
if($daddyobb->input['action'] == "editmod")
{
	$query = $db->simple_select("moderators", "*", "mid='".intval($daddyobb->input['mid'])."'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$update_array = array(
				...
				'canviewips' => intval($daddyobb->input['canviewips']),
				'canopenclosethreads' => intval($daddyobb->input['canopenclosethreads']),
				'canmanagethreads' => intval($daddyobb->input['canmanagethreads']),
				'canmovetononmodforum' => intval($daddyobb->input['canmovetononmodforum'])
			);
			$db->update_query("moderators", $update_array, "mid='".intval($daddyobb->input['mid'])."'");
			...
			admin_redirect("index.php?module=forum/management&fid=".intval($daddyobb->input['fid'])."#tab_moderators");
		}
	}
	...
	$sub_tabs['edit_mod'] = array(
		'title' => $lang->edit_mod,
		'link' => "index.php?module=forum/management&amp;action=editmod&amp;mid=".$daddyobb->input['mid'],
...
if($daddyobb->input['action'] == "permissions")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		{
			if(is_array($daddyobb->input['permissions']))
			{
				$update_array = array();
				...
				foreach($fields_array as $field)
				{
					...
					{
						if(array_key_exists($field['Field'], $daddyobb->input['permissions']))
						{
							$update_array[$db->escape_string($field['Field'])] = intval($daddyobb->input['permissions'][$field['Field']]);
						}
						else
						{
				...
				if($fid && !$pid)
				{
					...
					$update_array['gid'] = intval($daddyobb->input['gid']);
	...
	$sub_tabs['edit_permissions'] = array(
		...
		'link' => "index.php?module=forum/management&amp;action=permissions&amp;fid=".$daddyobb->input['fid'],
		'description' => $lang->forum_permissions_desc
	);
	
	$page->add_breadcrumb_item($lang->forum_permissions2, "index.php?module=forum/management&amp;fid=".$daddyobb->input['fid']."#tab_permissions");
	...
	if($daddyobb->input['pid'] || ($daddyobb->input['gid'] && $daddyobb->input['fid']))
	{
		...
		{
			$page->output_inline_error($errors);
			$permission_data = $daddyobb->input;
		}
		else
		{
			$pid = intval($daddyobb->input['pid']);
...
if($daddyobb->input['action'] == "add")
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if(!$errors)
		{
			...
			$insert_array = array(
				"name" => $db->escape_string($daddyobb->input['title']),
				"description" => $db->escape_string($daddyobb->input['description']),
				"linkto" => $db->escape_string($daddyobb->input['linkto']),
				"type" => $db->escape_string($type),
				"pid" => $pid,
				"parentlist" => '',
				"disporder" => intval($daddyobb->input['disporder']),
				"active" => intval($daddyobb->input['active']),
				"open" => intval($daddyobb->input['open']),
				"allowhtml" => intval($daddyobb->input['allowhtml']),
				"allowmycode" => intval($daddyobb->input['allowmycode']),
				"allowsmilies" => intval($daddyobb->input['allowsmilies']),
				"allowimgcode" => intval($daddyobb->input['allowimgcode']),
				"allowpicons" => intval($daddyobb->input['allowpicons']),
				"allowtratings" => intval($daddyobb->input['allowtratings']),
				"usepostcounts" => intval($daddyobb->input['usepostcounts']),
				"password" => $db->escape_string($daddyobb->input['password']),
				"showinjump" => intval($daddyobb->input['showinjump']),
				"modposts" => intval($daddyobb->input['modposts']),
				"modthreads" => intval($daddyobb->input['modthreads']),
				"mod_edit_posts" => intval($daddyobb->input['mod_edit_posts']),
				"modattachments" => intval($daddyobb->input['modattachments']),
				"style" => intval($daddyobb->input['style']),
				"overridestyle" => intval($daddyobb->input['overridestyle']),
				"rulestype" => intval($daddyobb->input['rulestype']),
				"rulestitle" => $db->escape_string($daddyobb->input['rulestitle']),
				"rules" => $db->escape_string($daddyobb->input['rules']),
				"defaultdatecut" => intval($daddyobb->input['defaultdatecut']),
				"defaultsortby" => $db->escape_string($daddyobb->input['defaultsortby']),
				"defaultsortorder" => $db->escape_string($daddyobb->input['defaultsortorder']),
			);
			...
			$inherit = $daddyobb->input['default_permissions'];
			
			foreach($daddyobb->input['permissions'] as $gid => $permission)
			{
	...
	else
	{
		...
		{
			$forum_data['pid'] = intval($daddyobb->input['pid']);
		}
	...
	{
		$perms = array();
		if(isset($daddyobb->input['default_permissions']))
		{
			if($daddyobb->input['default_permissions'][$usergroup['gid']])
			{
				...
				else if(is_array($cached_forum_perms) && $cached_forum_perms[$daddyobb->input['pid']][$usergroup['gid']])
				{
					$perms = $cached_forum_perms[$daddyobb->input['pid']][$usergroup['gid']];
		...
		foreach($field_list as $forum_permission)
		{
			...
			if($daddyobb->input['permissions'][$usergroup['gid']][$forum_permission])
			{
				$value = $daddyobb->input['permissions'][$usergroup['gid']][$forum_permission];
			}
			
			if(isset($daddyobb->input['permissions']))
			{
				if($daddyobb->input['permissions'][$usergroup['gid']]['all'])
				{
					$all_checked = false;
				}
...
if($daddyobb->input['action'] == "edit")
{
	...
	$query = $db->simple_select("forums", "*", "fid='{$daddyobb->input['fid']}'");
	...
	if($daddyobb->request_method == "post")
	{
		...
		if($pid == $daddyobb->input['fid'])
		{
			$errors[] = $lang->error_forum_parent_itself;
		}
		else
		{
			$query = $db->simple_select("forums", "*", "pid='{$daddyobb->input['fid']}'");
		...
		if(!$errors)
		{
			...
			// Log admin action
			log_admin_action($fid, $daddyobb->input['title']);
...
if($daddyobb->input['action'] == "deletemod")
{
	$plugins->run_hooks("admin_forum_management_deletemod");
	
	$query = $db->simple_select("moderators", "*", "uid='{$daddyobb->input['uid']}' AND fid='{$daddyobb->input['fid']}'");
	...
	if(!$mod['mid'])
	{
		...
		admin_redirect("index.php?module=forum/management&fid=".$daddyobb->input['fid']);
	}
	...
	if($daddyobb->request_method == "post")
	{
		...
		admin_redirect("index.php?module=forum/management&fid=".$daddyobb->input['fid']."#tab_moderators");
	}
	else
	{
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->request_method == "post")
	{
		...
		if($daddyobb->input['update'] == "permissions")
		{
			$inherit = $daddyobb->input['default_permissions'];
			
			if(empty($daddyobb->input['permissions']))
			{
    				$daddyobb->input['permissions'] = array();
			}
		...
		elseif($daddyobb->input['add'] == "moderators")
		{
			...
			$query = $db->simple_select("users", "uid, username", "username='".$db->escape_string($daddyobb->input['username'])."'", array('limit' => 1));
		...
		{
			if(!empty($daddyobb->input['disporder']) && is_array($daddyobb->input['disporder']))
			{
				foreach($daddyobb->input['disporder'] as $update_fid => $order)
				{
	...
	echo $form->generate_hidden_field("fid", $daddyobb->input['fid']);
	...
	if($fid)
	{
		...
		while($moderator = $db->fetch_array($query))
		{
			...
			$form_container->output_cell("<a href=\"index.php?module=forum/management&amp;action=deletemod&amp;uid={$moderator['uid']}&amp;fid={$fid}&amp;my_post_key={$daddyobb->post_code}\" onclick=\"return AdminCP.deleteConfirmation(this, '{$lang->confirm_moderator_deletion}')\">{$lang->delete}</a>", array("class" => "align_center"));
			$form_container->construct_row();
		}
		...
		$form_container->output_row($lang->username." <em>*</em>", $lang->moderator_username_desc, $form->generate_text_box('username', $daddyobb->input['username'], array('id' => 'username')), 'username');
		$form_container->end();
...
{
	global $daddyobb, $lang, $db, $sub_forums;
	...
	foreach($forums_by_parent[$pid] as $children)
	{
		foreach($children as $forum)
		{
			...
			if($forum['type'] == "c" && ($depth == 1 || $depth == 2))
			{
				...
				$popup->add_item($lang->delete_forum, "index.php?module=forum/management&amp;action=delete&amp;fid={$forum['fid']}&amp;my_post_key={$daddyobb->post_code}", "return AdminCP.deleteConfirmation(this, '{$lang->confirm_forum_deletion}')");
			...
			{
				if($donecount < $daddyobb->settings['subforumsindex'])
				{

...
if($daddyobb->input['action'] == "stats" || $daddyobb->input['action'] == "orphans" || !$daddyobb->input['action'])
{
...
if($daddyobb->input['action'] == "delete")
{
	...
	if(!is_array($daddyobb->input['aids']))
	{
		$daddyobb->input['aids'] = array(intval($daddyobb->input['aid']));
	}
	else
	{
		$daddyobb->input['aids'] = array_map("intval", $daddyobb->input['aids']);
	}

	if(count($daddyobb->input['aids']) < 1)
	{
	...
	if($daddyobb->request_method == "post")
	{
		...
		$query = $db->simple_select("attachments", "aid,pid,posthash, filename", "aid IN (".implode(",", $daddyobb->input['aids']).")");
	...
	{
		foreach($daddyobb->input['aids'] as $aid)
		{
			$aids .= "&amp;aids[]=$aid";
		}
...
if($daddyobb->input['action'] == "stats")
{
...
if($daddyobb->input['action'] == "delete_orphans" && $daddyobb->request_method == "post")
{
	$plugins->run_hooks("admin_forum_attachments_delete_orphans");
	
	// Deleting specific attachments from uploads directory
	if(is_array($daddyobb->input['orphaned_files']))
	{
		function clean_filename($string)
		{
			return str_replace(array(".."), "", $string);
		}
		$daddyobb->input['orphaned_files'] = array_map("clean_filename", $daddyobb->input['orphaned_files']);
		foreach($daddyobb->input['orphaned_files'] as $file)
		{
			if(!@unlink(DADDYOBB_ROOT.$daddyobb->settings['uploadspath']."/".$file))
			{
				$error = true;
			}
		}
	...
	if(is_array($daddyobb->input['orphaned_attachments']))
	{
		$daddyobb->input['orphaned_attachments'] = array_map("intval", $daddyobb->input['orphaned_attachments']);
		require_once DADDYOBB_ROOT."inc/functions_upload.php";

		$query = $db->simple_select("attachments", "aid,pid,posthash", "aid IN (".implode(",", $daddyobb->input['orphaned_attachments']).")");
		while($attachment = $db->fetch_array($query))
		{
			if(!$attachment['pid'])
			{
...
if($daddyobb->input['action'] == "orphans")
{
	...
	if($daddyobb->input['step'] == 3)
	{
		$plugins->run_hooks("admin_forum_attachments_step3");
		
		$reults = 0;
		// Incoming attachments which exist as files but not in database
		if($daddyobb->input['bad_attachments'])
		{
			$bad_attachments = unserialize($daddyobb->input['bad_attachments']);
			$results = count($bad_attachments);
		}

		$aids = array();
		if($daddyobb->input['missing_attachment_files'])
		{
			$missing_attachment_files = unserialize($daddyobb->input['missing_attachment_files']);
			$aids = array_merge($aids, $missing_attachment_files);
		}

		if($daddyobb->input['missing_threads'])
		{
			$missing_threads = unserialize($daddyobb->input['missing_threads']);
			$aids = array_merge($aids, $missing_threads);
		}

		if($daddyobb->input['incomplete_attachments'])
		{
			$incomplete_attachments = unserialize($daddyobb->input['incomplete_attachments']);
		...
		if(is_array($bad_attachments))
		{
			...
			{
				$file_path = DADDYOBB_ROOT.$daddyobb->settings['uploadspath']."/".$file;
				...
				$table->construct_cell(my_date($daddyobb->settings['dateformat'], filemtime($file_path)).", ".my_date($daddyobb->settings['timeformat'], filemtime($file_path)), array('class' => 'align_center'));
				$table->construct_row();
			}
		}
		...
		if(count($aids) > 0)
		{
			...
			while($attachment = $db->fetch_array($query))
			{
				...
				{
					$table->construct_cell(my_date($daddyobb->settings['dateformat'], $attachment['dateuploaded']).", ".my_date($daddyobb->settings['timeformat'], $attachment['dateuploaded']), array('class' => 'align_center'));
				}
				else
				{
	...
	else if($daddyobb->input['step'] == 2)
	{
		...
		while($attachment = $db->fetch_array($query))
		{
			...
			if(!file_exists(DADDYOBB_ROOT.$daddyobb->settings['uploadspath']."/{$attachment['attachname']}"))
			{
		...
		if($daddyobb->input['bad_attachments'])
		{
			echo $form->generate_hidden_field("bad_attachments", $daddyobb->input['bad_attachments']);
		}
	...
	else
	{
		...
		{
			global $db, $daddyobb, $bad_attachments, $attachments_to_check;
			
			$real_dir = DADDYOBB_ROOT.$daddyobb->settings['uploadspath'];
			$false_dir = "";
			if($dir)
			{
				$real_dir .= "/".$dir;
...
if(!$daddyobb->input['action'])
{
	...
	if($daddyobb->request_method == "post" || $daddyobb->input['results'] == 1)
	{
		...
		{
			if($daddyobb->input[$search_field])
			{
				$search_sql .= " AND a.{$search_field} LIKE '%".$db->escape_string_like($daddyobb->input[$search_field])."%'";
			}
		}
		...
		{
			$query = $db->simple_select("users", "uid", "LOWER(username)='".$db->escape_string(my_strtolower($daddyobb->input['username']))."'");
		...
		if($daddyobb->input['forum'])
		{
			if(!is_array($daddyobb->input['forum']))
			{
				$daddyobb->input['forum'] = array($daddyobb->input['forum']);
			}

			$fid_in = array();
			foreach($daddyobb->input['forum'] as $fid)
			{
				if(!$forum_cache[$fid])
				{
		...
		if($daddyobb->input['dateuploaded'])
		{
			$daddyobb->input['dateuploaded'] = TIME_NOW-60*60*24;
		}
		if($daddyobb->input['filesize'])
		{
			$daddyobb->input['filesize'] *= 1024;
		}
		...
		foreach($direction_fields as $search_field)
		{
			...
			if($daddyobb->input[$search_field] && $daddyobb->input[$direction_field])
			{
				switch($daddyobb->input[$direction_field])
				{
					case "greater_than":
						$direction = ">";
						break;
				...
				$search_sql .= " AND a.{$search_field}{$direction}'".$db->escape_string($daddyobb->input[$search_field])."'";
			}
		}
		if(!$errors)
		{
		...
		if(!$errors)
		{
			...
			{
				$start = ($daddyobb->input['page'] - 1) * $daddyobb->input['perpage'];
			}
			else
			{
				$start = 0;
			...
			switch($daddyobb->input['sortby'])
			{
				...
				default:
					...
					$daddyobb->input['sortby'] = "filename";
			}

			if($daddyobb->input['sortorder'] != "desc")
			{
				$daddyobb->input['sortorder'] = "asc";
			}
			...
			$query = $db->query("
				...
				LEFT JOIN ".TABLE_PREFIX."users u ON (u.uid=a.uid)
				WHERE {$search_sql}
				ORDER BY {$sort_field} {$daddyobb->input['sortorder']}
				LIMIT {$start}, {$daddyobb->input['perpage']}
			");
			while($attachment = $db->fetch_array($query))
			{
			...
			if($num_results > $daddyobb->input['perpage'])
			{
				...
				{
					if($daddyobb->input[$var])
					{
						$pagination_url .= "&{$var}=".urlencode($daddyobb->input[$var]);
					}
				}
				$pagination = draw_admin_pagination($daddyobb->input['page'], $daddyobb->input['perpage'], $num_results, $pagination_url);
			}
	...
	$form_container->output_row($lang->name_contains, $lang->name_contains_desc, $form->generate_text_box('filename', $daddyobb->input['filename'], array('id' => 'filename')), 'filename');
	$form_container->output_row($lang->type_contains, "", $form->generate_text_box('filetype', $daddyobb->input['filetype'], array('id' => 'filetype')), 'filetype');
	$form_container->output_row($lang->forum_is, "", $form->generate_forum_select('forum[]', $daddyobb->input['forum'], array('multiple' => true, 'size' => 5, 'id' => 'forum')), 'forum');
	$form_container->output_row($lang->username_is, "", $form->generate_text_box('username', $daddyobb->input['username'], array('id' => 'username')), 'username');
	...
	$form_container->output_row($lang->date_posted_is, "", $form->generate_select_box('dateuploaded_dir', $more_options, $daddyobb->input['dateuploaded_dir'], array('id' => 'dateuploaded_dir'))." ".$form->generate_text_box('dateuploaded', $daddyobb->input['dateuploaded'], array('id' => 'dateuploaded'))." {$lang->days_ago}", 'dateuploaded');
	$form_container->output_row($lang->file_size_is, "", $form->generate_select_box('filesize_dir', $greater_options, $daddyobb->input['filesize_dir'], array('id' => 'filesize_dir'))." ".$form->generate_text_box('filesize', $daddyobb->input['filesize'], array('id' => 'filesize'))." {$lang->kb}", 'dateuploaded');
	$form_container->output_row($lang->download_count_is, "", $form->generate_select_box('downloads_dir', $greater_options, $daddyobb->input['downloads_dir'], array('id' => 'downloads_dir'))." ".$form->generate_text_box('downloads', $daddyobb->input['downloads'], array('id' => 'downloads'))."", 'dateuploaded');
	$form_container->end();
...
function build_attachment_row($attachment, &$table, $use_form=false)
{
	...
	{
		$date = my_date($daddyobb->settings['dateformat'], $attachment['dateuploaded']).", ".my_date($daddyobb->settings['timeformat'], $attachment['dateuploaded']);
	}
	else
	{
		$date = $lang->unknown;

...
if($daddyobb->request_method == "post")
{
	...
	{
		$query = $db->simple_select("attachments", "aid, pid", "aid IN (".implode(",", array_map("intval", array_keys($daddyobb->input['attachments'])))."){$flist}");
...
if($daddyobb->input['type'] == "posts" || $daddyobb->input['type'] == "")
{
	...
	else if($daddyobb->input['type'] == "posts")
	{
...
if($daddyobb->input['type'] == "attachments" || $daddyobb->input['type'] == "")
{
	...
	else if($daddyobb->input['type'] == "attachments")
	{
		$page->output_header($lang->moderation_queue);
		$page->output_nav_tabs($sub_tabs, "attachments");

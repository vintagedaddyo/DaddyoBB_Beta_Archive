...
if($daddyobb->settings['portal_showwelcome'] != 0)
{
	if($daddyobb->user['uid'] != 0)
	{
		// Get number of new posts, threads, announcements
		$query = $db->simple_select("posts", "COUNT(pid) AS newposts", "dateline>'".$daddyobb->user['lastvisit']."' $unviewwhere");
		$newposts = $db->fetch_field($query, "newposts");
		if($newposts)
		{ // if there aren't any new posts, there is no point in wasting two more queries
			$query = $db->simple_select("threads", "COUNT(tid) AS newthreads", "dateline>'".$daddyobb->user['lastvisit']."' $unviewwhere");
			$newthreads = $db->fetch_field($query, "newthreads");
			$query = $db->simple_select("threads", "COUNT(tid) AS newann", "dateline>'".$daddyobb->user['lastvisit']."' AND fid IN (".$daddyobb->settings['portal_announcementsfid'].") $unviewwhere");
	...
	{
		$lang->guest_welcome_registration = $lang->sprintf($lang->guest_welcome_registration, $daddyobb->settings['bburl'] . '/member.php?action=register');
		$daddyobb->user['username'] = $lang->guest;
		eval("\$welcometext = \"".$templates->get("portal_welcome_guesttext")."\";");
	}
	$lang->welcome = $lang->sprintf($lang->welcome, $daddyobb->user['username']);
...
if($daddyobb->settings['portal_showpms'] != 0)
{
	if($daddyobb->user['uid'] != 0 && $daddyobb->user['receivepms'] != 0 && $daddyobb->usergroup['canusepms'] != 0 && $daddyobb->settings['enablepms'] != 0)
	{
		switch($db->type)
		{
			case "sqlite2":
			case "sqlite3":
			case "pgsql":
				$query = $db->simple_select("privatemessages", "COUNT(*) AS pms_total", "uid='".$daddyobb->user['uid']."'");
				$messages['pms_total'] = $db->fetch_field($query, "pms_total");
				
				$query = $db->simple_select("privatemessages", "SUM(*) AS pms_unread", "uid='".$daddyobb->user['uid']."' AND IF(status='0' AND folder='1','1','0')");
				$messages['pms_unread'] = $db->fetch_field($query, "pms_unread");
				break;
			default:
				$query = $db->simple_select("privatemessages", "COUNT(*) AS pms_total, SUM(IF(status='0' AND folder='1','1','0')) AS pms_unread", "uid='".$daddyobb->user['uid']."'");
		...
		$lang->pms_received_new = $lang->sprintf($lang->pms_received_new, $daddyobb->user['username'], $messages['pms_unread']);
		eval("\$pms = \"".$templates->get("portal_pms")."\";");
	}
}
// Get Forum Statistics
if($daddyobb->settings['portal_showstats'] != 0)
{
...
// Search box
if($daddyobb->settings['portal_showsearch'] != 0)
{
	eval("\$search = \"".$templates->get("portal_search")."\";");
}

// Get the online users
if($daddyobb->settings['portal_showwol'] != 0)
{
	...
	while($user = $db->fetch_array($query))
	{
		...
		else
		{
			if($doneusers[$user['uid']] < $user['time'] || !$doneusers[$user['uid']])
			{
				...
				if(($user['invisible'] == 1 && ($daddyobb->usergroup['canviewwolinvis'] == 1 || $user['uid'] == $daddyobb->user['uid'])) || $user['invisible'] != 1)
				{
...
if($daddyobb->settings['portal_showdiscussions'] != 0 && $daddyobb->settings['portal_showdiscussionsnum'])
{
	$altbg = alt_trow();
	$threadlist = '';
	$query = $db->query("
		...
		LIMIT 0, ".$daddyobb->settings['portal_showdiscussionsnum']
	);
...
$announcementsfids = explode(',', $daddyobb->settings['portal_announcementsfid']);
...
$query = $db->query("
	...
	LIMIT 0, ".$daddyobb->settings['portal_numannouncements']
);
...
while($announcement = $db->fetch_array($query))
{
	...
	if($announcement['avatar'] != '')
	{
		...
		{
			$announcement['avatar'] = $daddyobb->settings['bburl'] . '/' . $announcement['avatar'];
		}		
	...
	$anndate = my_date($daddyobb->settings['dateformat'], $announcement['dateline']);
	$anntime = my_date($daddyobb->settings['timeformat'], $announcement['dateline']);

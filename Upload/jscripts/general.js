var DaddyoBB = {
	...
	{
		this.detectBrowser();
		Event.observe(document, "dom:loaded", DaddyoBB.pageLoaded);
		return true;
	},
	
	pageLoaded: function()
	{
		DaddyoBB.page_loaded = 1;
		...
		checkall.each(function(element) {
			Event.observe(element, "click", DaddyoBB.checkAll.bindAsEventListener(this));
		});
		...
		if(typeof(use_xmlhttprequest) != "undefined" && use_xmlhttprequest == 1)
		{
			...
			mark_read_imgs.each(function(element) {
				...
				Event.observe(element, "click", DaddyoBB.markForumRead.bindAsEventListener(this));
	...
	{
		DaddyoBB.popupWindow("reputation.php?action=add&uid="+uid, "reputation", 400, 350)
	},
	...
	{
		DaddyoBB.popupWindow("misc.php?action=whoposted&tid=" + tid, "whoPosted", 230, 300)
	},
	...
	markForumRead: function(event)
	{
		...
		new Ajax.Request('misc.php?action=markread&fid='+fid+'&ajax=1', {method: 'get', onComplete: function(request) {DaddyoBB.forumMarkedRead(fid, request); }});
	},
	...
	forumMarkedRead: function(fid, request)
	{
		if(request.responseText == 1)
		{
			...
			Event.stopObserving($('mark_read_'+fid), "click", DaddyoBB.markForumRead.bindAsEventListener(this));

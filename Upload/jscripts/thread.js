var Thread = {
	...
	{
		DaddyoBB.popupWindow("report.php?pid="+pid, "reportPost", 400, 300)
	},
	...
	{
    editreason = $("quickedit_reason_"+pid).value;
		...
		new Ajax.Request('xmlhttp.php?action=edit_post&do=update_post&pid='+pid+"&editreason="+editreason+"&my_post_key="+my_post_key, {method: 'post', postBody: postData, onComplete: function(request) { Thread.quickEditSaved(request, pid); }});
	},
	...
	quickReplyDone: function(request)
	{
		...
		else if(request.responseText.match(/id="post_([0-9]+)"/))
		{
			...
			if(DaddyoBB.browser == "ie" || DaddyoBB.browser == "opera" || DaddyoBB.browser == "safari")
			{

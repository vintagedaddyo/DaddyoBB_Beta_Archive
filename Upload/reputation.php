...
if($daddyobb->settings['enablereputation'] != 1)
{
...
if($daddyobb->input['action'] == "add" || $daddyobb->input['action'] == "do_add")
{
	// This user doesn't have permission to give reputations.
	if($daddyobb->usergroup['cangivereputations'] != 1)
	{
	...
	if($daddyobb->usergroup['maxreputationsday'] != 0 && ($daddyobb->input['action'] != "do_add" || ($daddyobb->input['action'] == "do_add" && !$daddyobb->input['delete'])))
	{
		$timesearch = TIME_NOW - (60 * 60 * 24);
		$query = $db->simple_select("reputation", "*", "adduid='".$daddyobb->user['uid']."' AND dateline>'$timesearch'");
		$numtoday = $db->num_rows($query);

		// Reached the quota - error.
		if($numtoday >= $daddyobb->usergroup['maxreputationsday'])
		{
	...
	$query = $db->simple_select("reputation", "*", "adduid='".$daddyobb->user['uid']."' AND uid='{$uid}'");
...
if($daddyobb->input['action'] == "do_add" && $daddyobb->request_method == "post")
{
	...
	$reputation = intval(str_replace("-", "", $daddyobb->input['reputation']));

	// Deleting our current reputation of this user.
	if($daddyobb->input['delete'])
	{
		$db->delete_query("reputation", "uid='{$uid}' AND adduid='".$daddyobb->user['uid']."'");
	...
	if(trim($daddyobb->input['comments']) == "" || my_strlen($daddyobb->input['comments']) < 10)
	{
		$show_back = 1;
	...
	if($reputation > $daddyobb->usergroup['reputationpower'] || !is_numeric($daddyobb->input['reputation']))
	{
		$show_back = 1;
	...
	$reputation = array(
		"uid" => $uid,
		"adduid" => $daddyobb->user['uid'],
		"reputation" => intval($daddyobb->input['reputation']),
		"dateline" => TIME_NOW,
		"comments" => $db->escape_string($daddyobb->input['comments'])
	);
...
if($daddyobb->input['action'] == "add")
{
	...
	$reputationpower = $daddyobb->usergroup['reputationpower'];
...
if($daddyobb->input['action'] == "delete")
{
	...
	// Fetch the existing reputation for this user given by our current user if there is one.
	$query = $db->simple_select("reputation", "*", "rid='".$daddyobb->input['rid']."'");
	$existing_reputation = $db->fetch_array($query);

	// Only administrators as well as users who gave a specifc vote can delete one.
	if($daddyobb->usergroup['cancp'] != 1 && $existing_reputation['adduid'] != $daddyobb->user['uid'])
	{
		error_no_permission();
	}

	// Delete the specified reputation
	$db->delete_query("reputation", "uid='{$uid}' AND rid='".$daddyobb->input['rid']."'");
...
if(!$daddyobb->input['action'])
{
	...
	{
		$page = $daddyobb->input['page'];
		$start = ($page-1) *$daddyobb->settings['repsperpage'];
		$pages = $reputation_count / $daddyobb->settings['repsperpage'];
		$pages = ceil($pages);
		if($page > $pages)
		{
			$start = 0;
	...
	{
		$multipage = multipage($reputation_count, $daddyobb->settings['repsperpage'], $page, "reputation.php?uid={$user['uid']}");
	}
	...
	$query = $db->query("
		...
		ORDER BY $order
		LIMIT $start, {$daddyobb->settings['repsperpage']}
	");
	while($reputation_vote = $db->fetch_array($query))
	{
		...
		$last_updated_date = my_date($daddyobb->settings['dateformat'], $reputation_vote['dateline']);
		$last_updated_time = my_date($daddyobb->settings['timeformat'], $reputation_vote['dateline']);
		...
		if($daddyobb->usergroup['cancp'] == 1 || ($daddyobb->usergroup['cangivereputations'] == 1 && $reputation_vote['adduid'] == $daddyobb->user['uid'] && $daddyobb->user['uid'] != 0))
		{
			$delete_link = "[<a href=\"reputation.php?action=delete&amp;uid={$reputation_vote['rated_uid']}&amp;rid={$reputation_vote['rid']}\" onclick=\"DaddyoBB.deleteReputation({$reputation_vote['rated_uid']}, {$reputation_vote['rid']}); return false;\">{$lang->delete_vote}</a>]";
		}
		else
		{
			$delete_link = '';
